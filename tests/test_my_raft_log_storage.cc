...
int main(int argc, char **argv) {
    ...
    ksearch::RocksWrapper *rocksdb_instance =
            ksearch::RocksWrapper::get_instance();
    int ret = rocksdb_instance->init(rocks_path);
    if (ret != 0) {
        std::cout << "rocksdb init fail" << std::endl;
        return -1;
    }
    ksearch::RaftLogCompactionFilter::get_instance()->print_map();
    static ksearch::MyRaftLogStorage my_raft_log_storage;
    ...
    }*/
    {
        ...
        for (size_t i = 0; i < read_entry->peers->size(); ++i) {
            std::cout << "log_entry peers id: " << i << " address: " << (*(read_entry->peers))[i].to_string() <<
                    std::endl;
        }
    }
    /*{
    ...
    // add data
    {
        ...
        //construct data
        ksearch::pb::RaftControlResponse response;
        response.set_region_id(10);
        response.set_errcode(ksearch::pb::SUCCESS);
