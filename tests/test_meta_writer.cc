...
protected:
    virtual void SetUp() {
        auto rocksdb = ksearch::RocksWrapper::get_instance();
        if (!rocksdb) {
        ...
        _writer = ksearch::MetaWriter::get_instance();
    ...
    ksearch::MetaWriter *_writer;
};

TEST_F(MetaWriterTest, test_encode) {
    int64_t region_id = 11;
    ksearch::pb::RegionInfo region_info;
    ...
    std::vector<ksearch::pb::RegionInfo> region_infos;
    ...
    for (auto &log_index: log_indexs) {
        ksearch::pb::StoreReq txn;
        txn.set_op_type(ksearch::pb::OP_PREPARE);
