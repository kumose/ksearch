...
DEFINE_bool(insert_to_ksearch, false, "insert test data to Ksearch");
DEFINE_int32(insert_row_cnt, 100000, "insert test data to Ksearch");
DEFINE_bool(query_ksearch, false, "read test data from Ksearch");
DEFINE_string(table, "vec.test_ivf", "Ksearch table name for testing");
...
void query_and_calc_recall(faiss::Index *idx, int d, int nb, int nq, int nl, float *xb, float *xq, float *xl,
                           idx_t *ids, int sn, std::string desc, std::vector<std::set<int> > &flat_res,
                           int ivf_nprobe) {
...
void run_one_test(int d, int nb, int nq, int nl, float *xb, float *xq, float *xl, idx_t *ids, int sn, std::string desc,
                  std::vector<std::set<int> > &flat_res, int ivf_nprobe) {
...
void load_shif1M(unsigned &d, unsigned &nb, unsigned &nq, unsigned &nl, float *&xb, float *&xq, float *&xl,
                 idx_t *&ids) {
    unsigned base_dim = 0;
    ...
    if (base_dim != learn_dim || base_dim != query_dim) {
        std::cout << "dim error: base_dim: " << base_dim << " learn_dim: " << learn_dim << " query_dim: " << query_dim
                << std::endl;
        exit(-1);
    }
    std::cout << "load finish: \n" << "base_num: " << base_num << " learn_num: " << learn_num << " query_num: " <<
            query_num << std::endl;
    d = base_dim;
    nb = base_num;
    nq = query_num;
...
int query(std::string sql, ks::client::Service *_ksearch, ks::client::SmartConnection &connection) {
    int ret = 0;
    int retry = 0;
    int affected_rows = 0;
    do {
        if (connection == nullptr) {
            connection = _ksearch->fetch_connection();
        }
...
TEST(test_query_ksearch, case_all) {
    if (!FLAGS_query_ksearch) {
        return;
    }
    ks::client::Manager _manager;
    ks::client::Service *_ksearch;
    int rc = 0;
    rc = _manager.init("conf", "ks_client.conf");
    if (rc != 0) {
        printf("ks client init fail:%d", rc);
        return;
    }
    _ksearch = _manager.get_service("ksearch");
    if (_ksearch == NULL) {
        printf("ksearch is null");
        return;
    }
    ...
    for (int i = 0; i < nq; ++i) {
        ...
        vec_string.pop_back();
        sql = "select id from " + FLAGS_table + " where match(vec) against ('" + vec_string +
              "' in vector mode) order by __weight " + FLAGS_order + " limit 10;";
        sqls.emplace_back(sql);
    }

    TimeCost total_time;
    TimeCost one_query_time;
    auto connection = _ksearch->fetch_connection();
    for (int i = 0; i < nq; ++i) {
        one_query_time.reset();
        query(sqls[i], _ksearch, connection);
...
TEST(test_insert_ksearch, case_all) {
    if (!FLAGS_insert_to_ksearch) {
        return;
    }
    ks::client::Manager _manager;
    ks::client::Service *_ksearch;
    ...
    for (auto table: tables) {
        ...
        for (int i = 0; i < vec_strings.size(); ++i) {
            one_query_time.reset();
            sql = "replace into " + table + "(id, vec, id2) values (" + std::to_string(i) + ", '" + vec_strings[i] +
                  "', '" + FLAGS_id2_value + "')";
            query(sql, _ksearch, connection);
            if (one_query_time.get_time() > FLAGS_sql_print_time_limit) {
