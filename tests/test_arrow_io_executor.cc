...
namespace ksearch {
    ...
    TEST_F(TestBthreadArrowExecutor, SpawnSlow) {
        ...
        SpawnAdds(pool.get(), 7, task_slow_add<int>{
                      /*seconds=*/0.02
                  });
    }
    ...
    TEST_F(TestBthreadArrowExecutor, StressSpawnSlow) {
        auto pool = this->MakeThreadPool(30);
        SpawnAdds(pool.get(), 1000, task_slow_add<int>{
                      /*seconds=*/0.002
                  });
    }
    ...
    TEST_F(TestBthreadArrowExecutor, StressSpawnSlowThreaded) {
        auto pool = this->MakeThreadPool(30);
        SpawnAddsThreaded(pool.get(), 20, 100, task_slow_add<int>{
                              /*seconds=*/0.002
                          });
    }
    ...
    TEST_F(TestBthreadArrowExecutor, SpawnWithStopTokenCancelled) {
        arrow::StopSource stop_source;
        auto pool = this->MakeThreadPool(3);
        SpawnAddsAndCancel(pool.get(), 100, task_slow_add<int>{
                               /*seconds=*/0.02
                           }, &stop_source);
    }
    ...
    TEST_F(TestBthreadArrowExecutor, StressSpawnThreadedWithStopTokenCancelled) {
        arrow::StopSource stop_source;
        auto pool = this->MakeThreadPool(30);
        SpawnAddsThreadedAndCancel(pool.get(), 20, 100, task_slow_add<int>{
                                       /*seconds=*/0.02
                                   },
                                   &stop_source);
    }
    ...
    TEST_F(TestBthreadArrowExecutor, QuickShutdown) {
        ...
        {
            auto pool = this->MakeThreadPool(3);
            add_tester.SpawnTasks(pool.get(), task_slow_add<int>{
                                      /*seconds=*/0.02
                                  });
        ...
        auto pool = this->MakeThreadPool(3);
        {
            ...
            for (int i = 0; i < n_futures; ++i) {
                ASSERT_OK_AND_ASSIGN(
                    ...
                    auto fut,
                    pool->Submit(stop_token, slow_add<int>, 0.01 /*seconds*/, i, 1)
                );
