    ...
    void arrow_test(std::string db_path, std::string word_file, const char *search_word_file) {
        ...
        while (std::getline(search_file, search_line)) {
            ...
            for (auto i = 0; i < 1; ++i) {
                ...
                std::cout << "valid number [" << valid_num << "] time [" << tc2.get_time() << "] all_time[" << tc_all.
                        get_time() << "]\n";
                txn->commit();
            }
        }
