...
namespace ksearch {
    void construct_schema(pb::SchemaInfo &schema_info) {
        ...
        // Range Partition
        {
            ...
            p_range_partition_info->mutable_less_value()->mutable_nodes(0)->mutable_derive_node()->set_string_val(
                "2023-02-01");
        }
        {
            auto *p_range_partition_info = schema_info.mutable_partition_info()->add_range_partition_infos();
        ...
        }
        {
            ...
            p_range_partition_info->mutable_range()->mutable_left_value()->mutable_nodes(0)->set_node_type(
                pb::STRING_LITERAL);
            ...
            p_range_partition_info->mutable_range()->mutable_left_value()->mutable_nodes(0)->mutable_derive_node()->
                    set_string_val("2023-03-01");

            p_range_partition_info->mutable_range()->mutable_right_value()->add_nodes();
            p_range_partition_info->mutable_range()->mutable_right_value()->mutable_nodes(0)->set_node_type(
                pb::STRING_LITERAL);
            ...
            p_range_partition_info->mutable_range()->mutable_right_value()->mutable_nodes(0)->mutable_derive_node()->
                    set_string_val("2023-04-01");
        }
    }
    ...
    TEST_F(TestPartition, test_pre_process_partition) {
        ...
        ASSERT_EQ(p_schema_info->partition_info().range_partition_infos_size(), 3);
        {
            ...
            EXPECT_EQ(partition_utils::get_partition_value(
                          p_schema_info->partition_info().range_partition_infos(0).range().left_value(), left_value),
                      0);
            EXPECT_EQ(partition_utils::get_partition_value(
                          p_schema_info->partition_info().range_partition_infos(0).range().right_value(), right_value),
                      0);
        ...
        }
        {
            ...
            ExprValue right_value;
            EXPECT_EQ(partition_utils::get_partition_value(
                          p_schema_info->partition_info().range_partition_infos(1).range().left_value(), left_value),
                      0);
            EXPECT_EQ(partition_utils::get_partition_value(
                          p_schema_info->partition_info().range_partition_infos(1).range().right_value(), right_value),
                      0);
        ...
        }
        {
            ...
            ExprValue right_value;
            EXPECT_EQ(partition_utils::get_partition_value(
                          p_schema_info->partition_info().range_partition_infos(2).range().left_value(), left_value),
                      0);
            EXPECT_EQ(partition_utils::get_partition_value(
                          p_schema_info->partition_info().range_partition_infos(2).range().right_value(), right_value),
                      0);
    ...
    TEST_F(TestPartition, test_alter_partition) {
        ...
        {
            auto *p_range_partition_info = request_add_partition.mutable_table_info()->mutable_partition_info()->
        ...
        {
            auto *p_range_partition_info = request_modify_partition.mutable_table_info()->mutable_partition_info()->
                    add_range_partition_infos();
            p_range_partition_info->set_partition_name("p202304");
        ...
        {
            auto *p_range_partition_info = request_drop_partition.mutable_table_info()->mutable_partition_info()->
                    add_range_partition_infos();
            p_range_partition_info->set_partition_name("p202302");
        }
        {
