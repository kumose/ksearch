    ...
    TEST(test_parser, case_load_data) {
        {
            ...
            std::string sql_load =
                    "load data local infile '/home/work/data' replace into table Orders CHARACTER SET 'utf8' fields terminated by '|' enclosed by '\"'"
        ...
        }
        {
            parser::SqlParser parser;
            std::string sql_load =
                    "load data infile '/home/mark/data' replace into table Orders CHARACTER SET 'utf8' fields terminated by ',' enclosed by '\"' "
                    "lines terminated by '\n' ignore 1 lines;";
            parser.parse(sql_load);
            std::cout << parser.syntax_err_str << std::endl;
            ASSERT_EQ(0, parser.error);
