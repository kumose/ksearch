...
namespace parser {
    TEST(test_parser, case_encode) {
        ...
        // utf8
        {
            ...
            while (std::getline(f, line)) {
                ...
                std::string assert_sql = boost::trim_copy(split_vec[1]);
                parser::SqlParser parser;
                parser.charset = ksearch::pb::UTF8;
                parser.parse(sql);
                ASSERT_EQ(0, parser.error);
                ASSERT_EQ(1, parser.result.size());
                parser::SelectStmt *select_stmt = (parser::SelectStmt *) parser.result[0];
    ...
    TEST(test_parser, case_option) {
        ...
        //select
        {
            ...
            //select distict
            std::string sql_opt1 =
                    "select field_a, '1.1', 1+1, count(*), (select 1 +1), (('1.1')), ((1.1+1)), count(*) as A";
            parser.parse(sql_opt1);
            ASSERT_EQ(0, parser.error);
            ASSERT_EQ(1, parser.result.size());
            ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(parser::SelectStmt));
    ...
    TEST(test_parser, case_from_subselect) {
        ...
        {
            parser::SqlParser parser;
            std::string sql_from_subselect =
                    "SELECT a,b FROM (select a,b from t1) as t2 WHERE a=10 AND b=1 ORDER BY a LIMIT 10";
    ...
    TEST(test_parser, case_select_field_subselect) {
        //test union clause
        {
            parser::SqlParser parser;
            std::string sql_from_subselect =
                    "SELECT a,(select max(b) from t1) as maxb FROM (select a,b from t1) as t2 WHERE a=10 AND b=1 ORDER BY a LIMIT 10";
            parser.parse(sql_from_subselect);
            ASSERT_EQ(0, parser.error);
            ASSERT_EQ(1, parser.result.size());
            ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(parser::SelectStmt));
    ...
    TEST(test_parser, case_exists_subselect) {
        ...
        {
            parser::SqlParser parser;
            std::string sql_exists_subselect =
                    "SELECT a,b FROM t1 WHERE EXISTS (SELECT a,b from t2) ORDER BY a LIMIT 10";
        ...
        }
        {
            parser::SqlParser parser;
            std::string sql_exists_subselect =
                    "SELECT a,b FROM t1 WHERE NOT EXISTS (SELECT a,b from t2) ORDER BY a LIMIT 10";
            parser.parse(sql_exists_subselect);
            ASSERT_EQ(0, parser.error);
            ASSERT_EQ(1, parser.result.size());
            ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(parser::SelectStmt));
    ...
    TEST(test_parser, case_anyorall_subselect) {
        ...
        {
            parser::SqlParser parser;
            std::string sql_anyorall_subselect =
                    "SELECT a,b FROM t1 WHERE a > all (SELECT a from t2) ORDER BY a LIMIT 10";
        ...
        }
        {
            parser::SqlParser parser;
            std::string sql_anyorall_subselect =
                    "SELECT a,b FROM t1 WHERE a > any (SELECT a from t2) ORDER BY a LIMIT 10";
            parser.parse(sql_anyorall_subselect);
            ASSERT_EQ(0, parser.error);
            ASSERT_EQ(1, parser.result.size());
            ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(parser::SelectStmt));
        ...
        }
        {
            parser::SqlParser parser;
            std::string sql_anyorall_subselect =
                    "SELECT a,b FROM t1 WHERE a > some (SELECT a from t2) ORDER BY a LIMIT 10";
            parser.parse(sql_anyorall_subselect);
            ASSERT_EQ(0, parser.error);
            ASSERT_EQ(1, parser.result.size());
            ASSERT_TRUE(typeid(*(parser.result[0])) == typeid(parser::SelectStmt));
