...
void print_metadata_info(ksearch::RocksWrapper *rocksdb) {
...
TEST_F(RocksdbTEST, ingest_test7) {
    ...
    for (int i = 0; i < 15; i++) {
        _rocksdb->put(rocksdb::WriteOptions(), _rocksdb->get_data_handle(), rocksdb::Slice(std::to_string(i)),
                      rocksdb::Slice(std::to_string(i + 1)));
    ...
    int i = 1;
    _rocksdb->put(rocksdb::WriteOptions(), _rocksdb->get_data_handle(), rocksdb::Slice(std::to_string(i)),
                  rocksdb::Slice(std::to_string(i + 2)));
    ...
    _rocksdb->remove_range(rocksdb::WriteOptions(), _rocksdb->get_data_handle(), rocksdb::Slice("1"),
                           rocksdb::Slice("3"), false);
...
TEST_F(RocksdbTEST, ingest_test6) {
    ...
    s = _rocksdb->compact_files(rocksdb::CompactionOptions(), _rocksdb->get_data_handle(), {"./rocks_db6/000020.sst"},
                                6);
    if (!s.ok()) {
...
TEST_F(RocksdbTEST, ingest_test5) {
    ...
    s = _rocksdb->compact_files(rocksdb::CompactionOptions(), _rocksdb->get_data_handle(),
                                {"./rocks_db5/000022.sst", "./rocks_db5/000024.sst"}, 1);
    if (!s.ok()) {
        DB_WARNING("compact_files error: code=%d, msg=%s",
                   s.code(), s.ToString().c_str());
    }
    ...
    s = _rocksdb->compact_files(rocksdb::CompactionOptions(), _rocksdb->get_data_handle(), {"./rocks_db5/000025.sst"},
                                5);
    if (!s.ok()) {
        DB_WARNING("compact_files error: code=%d, msg=%s",
                   s.code(), s.ToString().c_str());
    ...
    s = _rocksdb->compact_files(rocksdb::CompactionOptions(), _rocksdb->get_data_handle(), {"./rocks_db5/000026.sst"},
                                5);
    if (!s.ok()) {
        DB_WARNING("compact_files error: code=%d, msg=%s",
                   s.code(), s.ToString().c_str());
    ...
    bthread_usleep(10 * 1000 * 1000);
    s = _rocksdb->compact_files(rocksdb::CompactionOptions(), _rocksdb->get_data_handle(), {"./rocks_db5/000027.sst"},
                                5);
    if (!s.ok()) {
        DB_WARNING("compact_files error: code=%d, msg=%s",
                   s.code(), s.ToString().c_str());
    ...
    int i = 5;
    _rocksdb->put(rocksdb::WriteOptions(), _rocksdb->get_data_handle(), rocksdb::Slice(std::to_string(i)),
                  rocksdb::Slice(std::to_string(i + 3)));
    i = 6;
    _rocksdb->put(rocksdb::WriteOptions(), _rocksdb->get_data_handle(), rocksdb::Slice(std::to_string(i)),
                  rocksdb::Slice(std::to_string(i + 3)));
    ...
    print_metadata_info(_rocksdb);
    bthread_usleep(10 * 1000 * 1000);
    s = _rocksdb->compact_files(rocksdb::CompactionOptions(), _rocksdb->get_data_handle(), {"./rocks_db5/000030.sst"},
                                5);
    if (!s.ok()) {
        DB_WARNING("compact_files error: code=%d, msg=%s",
                   s.code(), s.ToString().c_str());
...
TEST_F(RocksdbTEST, ingest_test4) {
    ...
    read_options.table_filter = [&](const rocksdb::TableProperties &props) {
        if (props.orig_file_number == 20) {
            DB_WARNING("RETRUN FALSE file_number: %lu TableProperties: %s", props.orig_file_number,
                       props.ToString("; ", ": ").c_str());
            return false;
        } else {
            DB_WARNING("RETRUN TRUE file_number: %lu TableProperties: %s", props.orig_file_number,
                       props.ToString("; ", ": ").c_str());
            return true;
        }
    };
...
TEST_F(RocksdbTEST, ingest_test) {
    ...
    if (FLAGS_ingest_file_name != "") {
        ...
        if (!s.ok()) {
            DB_WARNING("ingest fail: %s error: code=%d, msg=%s", FLAGS_ingest_file_name.c_str(), s.code(),
                       s.ToString().c_str());
        } else {
