...
TEST_F(ClusterManagerTest, test_add_and_drop) {
    ...
    for (auto &_logical_physical: _cluster_manager->_logical_physical_map) {
        if (_logical_physical.first == ksearch::FLAGS_default_logical_room) {
    ...
    /* modify_tag 给实例加tag*/
    ksearch::pb::MetaManagerRequest request_tag;
    ...
    ksearch::pb::MetaManagerRequest request_move;
    request_move.set_op_type(ksearch::pb::OP_ADD_LOGICAL);
    ...
    request_move.set_op_type(ksearch::pb::OP_MOVE_PHYSICAL);
    ...
    ksearch::pb::MetaManagerRequest request_remove_logical;
    request_remove_logical.set_op_type(ksearch::pb::OP_DROP_LOGICAL);
    //删除一个有物理机房的逻辑机房， 报错
    ...
    //删除物理机房
    ksearch::pb::MetaManagerRequest request_remove_physical;
    request_remove_physical.set_op_type(ksearch::pb::OP_DROP_PHYSICAL);
    ...
    //删除实例
    ksearch::pb::MetaManagerRequest request_remove_instance;
    request_remove_instance.set_op_type(ksearch::pb::OP_DROP_INSTANCE);
...
TEST_F(ClusterManagerTest, test_load_balance_by_network_segment_rolling) {
    ...
    std::unordered_map<std::string, std::vector<std::string> > resource_ips = {
        ...
        {
            "tag2", {
                "127.100.0.1:8010", "127.100.0.2:8010", "127.101.0.1:8010", "127.101.0.2:8010",
        ...
        {
            "tag3", {
                "127.100.100.1:8010", "127.100.100.2:8010", "127.100.100.3:8010", "127.100.100.4:8010",
    ...
    ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""},
                                                    {"127.0.1.1:8010", "127.0.1.2:8010", "127.0.1.3:8010"},
                                                    selected_instance);
    ASSERT_EQ(0, ret);
...
TEST_F(ClusterManagerTest, test_load_balance_by_network_segment_add_drop_instance) {
    ...
    ksearch::pb::MetaManagerRequest request_change_resource_tag1;
    request_change_resource_tag1.set_op_type(ksearch::pb::OP_UPDATE_INSTANCE);
...
TEST_F(ClusterManagerTest, test_load_balance_by_network_segment_with_gflag) {
    ...
    ksearch::pb::MetaManagerRequest request_change_network_tag1;
    request_change_network_tag1.set_op_type(ksearch::pb::OP_UPDATE_INSTANCE);
    ...
    pick_times.clear();
    for (int i = 0; i < 40; ++i) {
        ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, {"11.0.0.3:8010", "11.0.0.4:8010"},
                                                        selected_instance);
        ASSERT_EQ(0, ret);
        pick_times[selected_instance]++;
    }
    ...
    pick_times.clear();
    for (int i = 0; i < 40; ++i) {
        ret = _cluster_manager->select_instance_rolling({resource_tag, "", ""}, {"11.0.0.1:8010", "11.0.0.4:8010"},
                                                        selected_instance);
        ASSERT_EQ(0, ret);
