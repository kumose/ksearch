...
*/
    ...
    TEST(parquet_read_write, write_read) {
        ...
        boost::filesystem::path output_path2(
            FLAGS_db_path + "_column/" + std::to_string(table_id) + "/" + std::to_string(region_id));
        ...
        std::shared_ptr<ColumnFileInfo> new_file = std::make_shared<ColumnFileInfo>(
            table_id, region_id, 0, 123, 0, file_infos[0]);
    ...
    TEST(check_interval_overlapped, all) {
        {
            ...
            EXPECT_EQ(ParquetFile::check_interval_overlapped(range, false, false, false, file_start_key.data(),
                                                             file_end_key.data()), true);
        }
...

        {
            ...
            MutTableKey file_end_key;
            file_end_key.append_u64(1).append_u64(2);
            EXPECT_EQ(ParquetFile::check_interval_overlapped(range, false, false, true, file_start_key.data(),
                                                             file_end_key.data()), false);
        }
    ...
    TEST(lru_cache, all) {
        {
            ...
            if (s.ok()) {
                ...
                DB_WARNING("value:%s, GetUsage:%lu,GetCharge:%lu", v->value.c_str(), block_cache->GetUsage(handle),
                           block_cache->GetCharge(handle));
            }
