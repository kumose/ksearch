...
#include <ksearch/sqlparser/dms.h>
...
namespace ksearch {
    ...
    TEST(test_dms, case_sql_check) {
        ...
        }
        {
            // 表不存在列field4
            std::string sql =
                    "INSERT INTO test_db.test_table (field1, field4) VALUES (\"field1_val\", \"field4_val\");";
            parser::SqlParser parser;
            parser::StmtNode *stmt = nullptr;
            EXPECT_EQ(task_session.parse_sql(parser, sql, stmt), 0);
            ASSERT_EQ(stmt != nullptr, true);
        ...
        {
            std::string sql =
                    "DELETE FROM test_db.test_table WHERE field1 = \"field1_val\" AND field2 = \"field2_val\";";
            parser::SqlParser parser;
            parser::StmtNode *stmt = nullptr;
            EXPECT_EQ(task_session.parse_sql(parser, sql, stmt), 0);
            ASSERT_EQ(stmt != nullptr, true);
        ...
        }
        {
            // 表不存在列field4
            std::string sql =
                    "DELETE FROM test_db.test_table WHERE field1 = \"field1_val\" AND field4 = \"field4_val\";";
            parser::SqlParser parser;
            parser::StmtNode *stmt = nullptr;
            EXPECT_EQ(task_session.parse_sql(parser, sql, stmt), 0);
            ASSERT_EQ(stmt != nullptr, true);
