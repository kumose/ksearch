...
int main(int argc, char *argv[]) {
    auto val_encoder = ksearch::SchemaFactory::get_instance();
    ksearch::pb::SchemaInfo info;
    ...
    for (int idx = 1; idx < col_cnt; idx++) {
        ksearch::pb::FieldInfo *field_string = info.add_fields();
        field_string->set_field_name("column" + std::to_string(idx));
        field_string->set_field_id(idx);
        if (idx % 5 == 0) {
            field_string->set_mysql_type(ksearch::pb::INT32);
        } else if (idx % 5 == 1) {
            field_string->set_mysql_type(ksearch::pb::UINT32);
        } else if (idx % 5 == 2) {
            field_string->set_mysql_type(ksearch::pb::INT64);
        } else if (idx % 5 == 3) {
            field_string->set_mysql_type(ksearch::pb::UINT64);
        } else if (idx % 5 == 4) {
            field_string->set_mysql_type(ksearch::pb::STRING);
        }
    }

    ksearch::pb::IndexInfo *index_pk = info.add_indexs();
    index_pk->set_index_type(ksearch::pb::I_PRIMARY);
    ...
    //assert(record != NULL);
    sleep(1);
    ksearch::TimeCost cost1;
    ksearch::BthreadCond cond;
    auto cal = [&]() {
        ksearch::SmartRecord record_template = ksearch::TableRecord::new_record(1);
        for (int i = 0; i < 1000000; i++) {
            ksearch::SmartRecord record = record_template->clone(false);
        }
    ...
    sleep(1);
    ksearch::TimeCost cost2;

    auto cal2 = [&]() {
        ksearch::TableInfo info2 = val_encoder->get_table_info(1);
        const Message *message = info2.factory->GetPrototype(info2.tbl_desc);
        for (int i = 0; i < 1000000; i++) {
            //ksearch::SmartRecord record(new ksearch::TableRecord(message->New()));
            ksearch::SmartRecord record_template = ksearch::TableRecord::new_record(1);
        }
        cond.decrease_signal();
    };
