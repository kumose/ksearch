...
int main(int argc, char *argv[]) {
    auto val_encoder = ksearch::RecordEncoder::get_instance();
    ...
    for (int idx = 1; idx < col_cnt; idx++) {
        ksearch::pb::Field *field_string = info.add_fields();
        ...
        if (idx % 5 == 0) {
            ...
            field_string->set_mysql_type(ksearch::pb::DOUBLE);
        }
    }

    ksearch::pb::Index *index_pk = info.add_indexs();
    ...
    std::vector<ksearch::FieldInfo> infos;
