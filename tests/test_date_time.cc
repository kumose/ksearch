...
namespace ksearch {
    ...
    TEST(test_localtime_mktime_fixed_r, case_all) {
        ...
        for (t = -2000000000; t < 2000000000LL; t += 34567 * 345) {
            ...
            if (t < 0) {
                // 负数时间，localtime_r会差几分钟，不知道为啥，用gmtime_r来代替测试
                gmtime_r(&t, &tm_std);
            ...
            if (t < 0) {
                // 负数时间，mktime会差几分钟，不知道为啥，用timegm来代替测试
    ...
    TEST(test_stamp_to_str, case_all) {
        ...
        std::cout << year << " " << mon << " " << mday << " " << hour << " " << min << " " << sec << " " << macrosec <<
                std::endl;
...
*/
    ...
    TEST(test_datetime_time, case_all) {
        ...
        EXPECT_EQ(datetime_to_str(time_to_datetime(str_to_time("19:28:44"))),
                  ExprValue::Now().cast_to(pb::DATE).get_string() + " 19:28:44");
        EXPECT_EQ(datetime_to_str(time_to_datetime(str_to_time("01:28:44"))),
                  ExprValue::Now().cast_to(pb::DATE).get_string() + " 01:28:44");
    ...
    TEST(test_datetime_timestamp, case_all) {
        EXPECT_EQ(timestamp_to_str(datetime_to_timestamp(str_to_datetime("2017-12-03 19:28:44.123456"))),
                  "2017-12-03 19:28:44");
        EXPECT_EQ(timestamp_to_str(datetime_to_timestamp(str_to_datetime("0000-00-00 00:00:00"))),
                  "0000-00-00 00:00:00");
        EXPECT_EQ(timestamp_to_str(datetime_to_timestamp(str_to_datetime("1970-01-01 08:00:00"))),
                  "0000-00-00 00:00:00");
        EXPECT_EQ(timestamp_to_str(datetime_to_timestamp(str_to_datetime("1970-01-01 08:00:01"))),
                  "1970-01-01 08:00:01");
        EXPECT_EQ(timestamp_to_str(datetime_to_timestamp(str_to_datetime("2040-01-01 08:00:01"))),
                  "2040-01-01 08:00:01");
        // < 1970-01-01 08:00:01非法，都当做0
        EXPECT_EQ(timestamp_to_str(datetime_to_timestamp(str_to_datetime("1970-01-01 07:00:01"))),
                  "0000-00-00 00:00:00");
    ...
    TEST(test_hour_date, case_all) {
        ...
        EXPECT_EQ(hour(construct_time_expr_value("2023-06-29")).get_string(), "999");
        // 异常输入，str_to_time()会将2023-06-29转换成999:06:29
