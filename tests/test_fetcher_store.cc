...
protected:
    virtual void SetUp() {
        auto schema_factory = ksearch::SchemaFactory::get_instance();
    ...
    void set_all_instance_normal() {
        for (const auto addr: _instance_info) {
            for (auto i = 0; i < 20; ++i) {
                ksearch::SchemaFactory::get_instance()->update_instance(addr.first, pb::NORMAL, false, false);
            }
        }
    }
...
TEST_F(FetcherStoreTest, test_normal_follower_select) {
    ...
    FLAGS_insulate_fetcher_resource_tag = "";
    {
        ...
        DB_WARNING("ksearch_logical_room: %s", ksearch_logical_room.c_str());
