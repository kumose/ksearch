...
namespace ksearch {
    ...
    public:
        ParquetStreamWriter(std::unique_ptr<::parquet::ParquetFileWriter> writer) : ::parquet::StreamWriter(
            std::move(writer)) {
        }
    ...
    TEST(test_parquet_reader_local, case_all) {
        ...
        std::shared_ptr<ParquetReader> p_reader{
            new(std::nothrow) ParquetReader(
