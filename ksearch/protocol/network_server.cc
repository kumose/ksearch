...
#include <ksearch/expr/internal_functions.h>
#include <ksearch/engine/external_filesystem.h>
#include <ksearch/engine/olap_pre_split.h>
#include <ksearch/logical_plan/kill_planner.h>
...
namespace ksearch {
    ...
    DEFINE_bool(fetch_instance_id, false, "fetch ksearch instace id, used for generate transaction id");
    ...
    DECLARE_string (ks_resource_tag);
    ...
    static const std::string instance_table_name = "INTERNAL.ksearch.__ksearch_instance";
    ...
    int NetworkServer::heartbeat(bool is_sync, bool is_backup) {
        ...
        // 非主Meta
        std::unordered_map<int64_t, pb::KsHeartBeatRequest> meta_request_map;
        cost.reset();
        KsHeartBeat::construct_heart_beat_request(meta_request_map);
        construct_req_cost = cost.get_time();
        DB_WARNING("dblink construct_req_cost: %ld", construct_req_cost);
        for (auto &kv: meta_request_map) {
            const int64_t &meta_id = kv.first;
            const pb::KsHeartBeatRequest &request = kv.second;
            cost.reset();
            if (MetaServerInteract::get_instance()->send_request("ks_heartbeat", request, response, meta_id) == 0) {
                //处理心跳
                if (is_sync) {
                    KsHeartBeat::process_heart_beat_response_sync(response, meta_id);
                    DB_WARNING("sync report_heartbeat, meta_id: %ld, process_res_cost:%ld", meta_id, cost.get_time());
                } else {
                    KsHeartBeat::process_heart_beat_response(response, false, meta_id);
        ...
        if (is_backup && MetaServerInteract::get_backup_instance()->is_inited()) {
            ...
            // 1、construct heartbeat request
            cost.reset();
            KsHeartBeat::construct_heart_beat_request(request, true);
            construct_req_cost = cost.get_time();
            cost.reset();
            // 2、send heartbeat request to meta server
            if (MetaServerInteract::get_backup_instance()->send_request("ks_heartbeat", request, response) == 0) {
                // 处理心跳
                KsHeartBeat::process_heart_beat_response(response, true);
    ...
    void NetworkServer::report_heart_beat() {
        while (!_shutdown) {
            ...
            _heart_beat_count << -1;
            bthread_usleep_fast_shutdown(FLAGS_ks_heartbeat_interval_us, _shutdown);
        }
    }
    ...
    void NetworkServer::report_other_heart_beat() {
        while (!_shutdown) {
            TimeCost cost;
            pb::KsOtherHeartBeatRequest request;
            ...
            cost.reset();
            //2、send heartbeat request to meta server
            if (MetaServerInteract::get_instance()->send_request("ks_other_heartbeat", request, response) == 0) {
                //处理心跳
    ...
    int NetworkServer::insert_agg_sql(const std::string &values) {
        ...
        TimeCost cost;
        // 构造sql
        std::string sql = "REPLACE INTO KsStat.ksearch_trace_info(`time`,`date`,`hour`,`min`,"
        ...
        int retry = 0;
        do {
            ret = _ksearch->query(0, sql, &result_set);
            if (ret == 0) {
                break;
            }
    ...
    int NetworkServer::insert_agg_sql_by_sign(const std::string &values) {
        ...
        TimeCost cost;
        // 构造sql
        std::string sql = "REPLACE INTO KsStat.sign_family_table_sql(`sign`,`family`,`tbl`,`resource_tag`,"
                "`op_type`,`sql`) VALUES ";
        sql += values;
        int ret = 0;
        int retry = 0;
    ...
    int NetworkServer::get_filter_sign_set(const std::map<uint64_t, std::string> &sign_sql_map,
                                           std::set<uint64_t> &need_filter_sign) {
        ks::client::ResultSet result_set;
        TimeCost cost;
        ...
        if (sign_strings.size() > 0) {
            // 构造sql
            std::string sql = "SELECT sign FROM KsStat.sign_family_table_sql WHERE sign in (" + sign_strings + ")";

            int ret = _ksearch->query(0, sql, &result_set);
            if (ret != 0) {
                DB_FATAL("sql_len:%lu query fail : %s", sql.size(), sql.c_str());
                sql += ";\n";
                return -1;
    ...
    int NetworkServer::insert_family_table_tag(const std::string &values) {
        ...
        TimeCost cost;
        // 构造sql
        std::string sql = "REPLACE INTO KsStat.family_table_tag(`family`,`tbl`,`resource_tag`) VALUES ";
        sql += values;
        int ret = 0;
        int retry = 0;
        do {
        ...
        while (!_shutdown) {
            ...
            for (auto &pair: sample.internal_map) {
                ...
                for (auto &pair2: pair.second) {
                    ...
                    SQL_TRACE(
                        "date_hour_min=[%04d-%02d-%02d\t%02d\t%02d] sum_pv_avg_affected_scan_filter_rgcnt_err_size="
                        ...
                        field_desc.c_str(),
                        butil::my_ip_cstr(), FLAGS_ks_port);
                    ...
                    if (FLAGS_insert_agg_sql) {
                        ...
                        if (family == "KsStat" && tbl == "ksearch_trace_info" &&
            ...
            if (FLAGS_insert_agg_sql &&values_size 
            >
            0
            )
            {
            ...
            if (degrade_cost.get_time() > FLAGS_check_degrade_interval_s * 1000 * 1000LL) {
                ...
                if (auto_count > FLAGS_backup_pv_threshold && auto_error * 1.0 / auto_count >
                    FLAGS_backup_error_percent) {
                    AutoInc::need_degrade = true;
                    DB_FATAL("meta_tso_autoinc_degrade, auto inc, auto_count:%ld, auto_error:%ld", auto_count,
                             auto_error);
                }
    ...
    int NetworkServer::query_sql(const std::string &sql, ks::client::ResultSet *result_set) {
        if (result_set == nullptr || _ksearch == nullptr) {
            return -1;
        }
        TimeCost cost;
        int ret = 0;
    ...
    void NetworkServer::load_mpp_signs() {
        ...
        // write db first
        {
            ...
            if (signs.size() > 0) {
                std::string sql =
                        "REPLACE INTO KsStat.mpp_signs (sign, db_resource_tag, date, rows99, bytes99) VALUES ";
                std::string values = "";
                int value_cnt = 0;
        ...
        // load from db
        {
            std::string sql = "SELECT sign, rows99, bytes99 FROM KsStat.mpp_signs "
                              " where date > '" + a_week_ago.get_string() +
                              "' and db_resource_tag = '" + FLAGS_ks_resource_tag + "';";
            ks::client::ResultSet result_set;
    ...
    void NetworkServer::insert_subquery_signs_info(
        std::map<uint64_t, std::set<uint64_t> > &parent_sign_to_subquery_signs, bool is_insert_success) {
    ...
    int NetworkServer::insert_subquery_values(const std::string &values) {
        std::string sql = "REPLACE INTO KsStat.ksearch_subquery_sign_info("
        ...
        while (!_shutdown) {
            ...
            for (auto &pair: info_map) {
                ...
                pb::StoreService_Stub(&channel).health_check(cntl, &req, res,
                                                             brpc::NewCallback(
                                                                 on_health_check_done, res, cntl, addr, old_status));
            }
        }
    }

    //other心跳内容包括：代价统计信息；ksearch参数动态调整
    void NetworkServer::construct_other_heart_beat_request(pb::KsOtherHeartBeatRequest &request) {
        SchemaFactory *factory = SchemaFactory::get_instance();
        auto schema_read_recallback = [&request, factory](const SchemaMapping &schema) {
            ...
            for (auto &info_pair: schema.table_info_mapping) {
                const int64_t meta_id = ::ksearch::get_meta_id(info_pair.first);
                if (meta_id != 0) {
                    // 跳过非主Meta的表
                    continue;
                }
        ...
        std::string resource_tag = "__ksearch:" + FLAGS_ks_resource_tag;
        request.set_ksearch_resource_tag(resource_tag); // ksearch暂时没有resource_tag，使用"__ksearch"会全平台同时更新参数
    }

    void NetworkServer::process_other_heart_beat_response(const pb::KsOtherHeartBeatResponse &response) {
        SchemaFactory *factory = SchemaFactory::get_instance();
            ...
            for (int32_t idx = 0; idx < CONFIG_MPL_EPOLL_MAX_SIZE; ++idx) {
                ...
                if (ctx != nullptr &&
                    ...
                    if (query_time_diff > FLAGS_slow_query_timeout_s) {
                        ...
                        DB_NOTICE(
                            "query is slow, [cost=%d][fd=%d][ip=%s:%d][now=%ld][active=%ld][user=%s][log_id=%lu][conn_id=%ld][sign=%lu][server_addr=%s:%d][sql=%s]",
    ...
    int NetworkServer::insert_slow_query_infos(const std::string &slow_query_info_values) {
        ...
        TimeCost cost;
        // 构造sql
        std::string insert_slow_query_info_sql = "REPLACE INTO KsStat.slow_query_sql_info (`start_time`,"
        ...
        int retry = 0;
        do {
            ret = _ksearch->query(0, insert_slow_query_info_sql, &result_set);
            if (ret == 0) {
                break;
            }
            bthread_usleep(1000000);
    ...
    NetworkServer::NetworkServer() : _is_init(false),
                                     _shutdown(false),
                                     _epoll_info(NULL),
    ...
    int NetworkServer::fetch_instance_info() {
        ...
        DB_NOTICE("ksearch instance_id: %lu", _instance_id);
        return 0;
    }
    ...
    bool NetworkServer::init() {
        ...
        if (FLAGS_insert_agg_sql) {
            int rc = 0;
            rc = _manager.init("conf", "ks_client.conf");
            if (rc != 0) {
                DB_FATAL("ks client init fail:%d", rc);
                return false;
            }
            _ksearch = _manager.get_service("ksearch");
            if (_ksearch == NULL) {
                DB_FATAL("ksearch is null");
                return false;
            }
        }
        _is_init = true;
    ...
    SmartSocket NetworkServer::create_listen_socket() {
        ...
        if (FLAGS_enable_tcp_keep_alive &&set_keep_tcp_alive(sock->fd) 
        !=
        0
        )
        {
        ...
        listen_addr.sin_port = htons(FLAGS_ks_port);
        ...
        auto check_func = [this]() {
            ...
            for (int32_t idx = 0; idx < CONFIG_MPL_EPOLL_MAX_SIZE; ++idx) {
                ...
                if (0 != PhysicalPlanner::execute(kill_ctx.get(), kill_sock->send_buf)) {
                    DB_FATAL("kill_timeout_query PhysicalPlanner execute error. fd=%d, conn_id: %lu", sock->fd,
                             sock->conn_id);
                    continue;
                }
