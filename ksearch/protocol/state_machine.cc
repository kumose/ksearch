...
#include <ksearch/protocol/state_machine.h>
...
namespace ksearch {
    ...
    DEFINE_int64(ks_max_allowed_packet, 268435456LL, "The largest possible packet : 256M");
    ...
    void StateMachine::_print_query_time(SmartSocket client) {
        ...
        if (root != nullptr) {
            ...
            rows = (root->op_type() == pb::OP_SELECT || root->op_type() == pb::OP_UNION)
                       ? stat_info->num_returned_rows
                       : stat_info->num_affected_rows;
        }
            ...
            if (!ctx->use_backup) {
                ...
                */
                ...
                sql_agg_cost << BvarMap(stat_info->sample_sql.str(), index_id, stat_info->table_id,
                                        ...
                                        stat_info->num_scan_rows, stat_info->read_disk_size,
                                        stat_info->num_filter_rows, stat_info->region_count,
        ...
        if (ctx->mysql_cmd == COM_QUERY
            ...
            if (stat_info->total_time > FLAGS_print_time_us || stat_info->error_code != 1000) {
                ...
                DB_NOTICE_LONG(
                    ...
                    ctx->use_backup, butil::my_ip_cstr(), FLAGS_ks_port);
            }
        } else {
    ...
    int StateMachine::_auth_read(SmartSocket sock) {
        ...
        if (RET_SUCCESS != _wrapper->
            protocol_get_char(packet, sock->packet_len + PACKET_HEADER_LEN, off, &charset_num)) {
        ...
        if (len == '\x00') {
            ...
            for (int idx = 0; idx < 20; idx++) {
                if (*(packet + off + idx) != *(sock->user_info->scramble_password + idx)) {
                    DB_WARNING_CLIENT(sock, "client connect ks with wrong password");
                    return RET_AUTH_FAILED;
                }
            }
            off += 20;
        ...
        if (!sock->user_info->allow_addr(sock->ip)) {
            DB_WARNING_CLIENT(sock, "client connect ks with invalid ip");
            return RET_AUTH_FAILED;
        }
    ...
    int StateMachine::_read_packet_header(SmartSocket sock) {
        ...
        if (sock->current_packet_len == (int) PACKET_LEN_MAX) {
            // if packet >= 16M need read next packet
    ...
    int StateMachine::_query_read(SmartSocket sock) {
        ...
        if (COM_PING == command) {
            ...
            return RET_SUCCESS;
        } else if (COM_STMT_EXECUTE == command) {
            // this is COM_EXECUTE Packet
    ...
    int StateMachine::_query_read_stmt_long_data(SmartSocket sock) {
        ...
        uint64_t param_id = 0;
        if (RET_SUCCESS != _wrapper->protocol_get_length_fixed_int(packet, sock->packet_len + PACKET_HEADER_LEN, off, 2,
                                                                   param_id)) {
                ...
                if (iter != prepare_ctx->long_data_vars.end()) {
                    ...
                    if (is_null || prepare_ctx->param_type[idx].mysql_type == MYSQL_TYPE_NULL) {
                        ...
                        if (RET_SUCCESS != _wrapper->decode_binary_protocol_value(
                                ...
                                expr_node)) {
                            DB_WARNING("decode_prepared_stmt_param_value failed num_params:%d, idx:%d", num_params,
                                       idx);
                            return RET_ERROR;
                        }
                    }
    ...
    bool StateMachine::_query_process(SmartSocket client) {
        ...
        if (command == COM_INIT_DB) {
            // 0x02 command: use database, set names, set charset...
            ...
            if (type == SQL_SET_CHARSET_NUM
                ...
                // 防止超大sql文本
                if (sql_len > std::max(FLAGS_ks_max_allowed_packet, (int64_t) 1024)) {
