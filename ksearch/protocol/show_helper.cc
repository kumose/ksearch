...
#include <cctype>
#include <ksearch/protocol/show_helper.h>
...
namespace ksearch {
    ...
    void ShowHelper::init() {
        ...
        _calls[SQL_SHOW_ALL_TABLES] = std::bind(&ShowHelper::_show_all_tables,
                                                this, std::placeholders::_1, std::placeholders::_2);
        _calls[SQL_SHOW_BINLOG_INFO] = std::bind(&ShowHelper::_show_binlog_info, this, std::placeholders::_1,
                                                 std::placeholders::_2);
    ...
    bool ShowHelper::execute(const SmartSocket &client) {
        ...
        if (key == "create") {
            if (split_vec.size() > 2 && (boost::iequals(split_vec[2], "table") ||
                                         boost::iequals(split_vec[2], "view"))) {
    ...
    bool ShowHelper::_show_create_table(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        for (auto &index_id: info.indices) {
            ...
            if (!compatible) {
                ...
                } else if (index_info.type == pb::I_PRIMARY || index_info.type == pb::I_FULLTEXT || index_info.type ==
                           pb::I_VECTOR) {
                    oss << " " << index_map[index_info.type] << " ";
                } else if (index_info.type == pb::I_PRIMARY || index_info.type == pb::I_FULLTEXT || index_info.type ==
                           pb::I_VECTOR || index_info.type == pb::I_ROLLUP) {
                    oss << " " << index_map[index_info.type] << " ";
                } else {
                ...
                if (index_info.type == pb::I_VECTOR) {
                    ...
                    if (lower_vector_description.empty() || lower_vector_description.find("hnsw") !=
                        std::string::npos) {
    ...
    */
    ...
    bool ShowHelper::_show_schema_conf(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        std::unordered_set<std::string> allowed_conf = {
            "need_merge",
    ...
    bool ShowHelper::_process_binlogs_info(const SmartSocket &client, std::unordered_map<int64_t,
        ...
        std::vector<std::string> field_names = {
            "table_id", "current_partition_id", "region_id", "instance_ip", "table_name", "check_point_datetime",
            "max_oldest_datetime",
    ...
    bool ShowHelper::_process_partition_binlogs_info(const SmartSocket &client, std::unordered_map<int64_t,
                                                     ...
                                                     table_id_to_query_info) {
        std::vector<std::string> field_names = {
            "table_id", "partition_index", "check_point_datetime", "oldest_datetime", "table_name"
        };
        std::vector<ResultField> result_fields;
        result_fields.reserve(3);
        for (auto &field_name: field_names) {
    ...
    bool ShowHelper::_show_all_tables(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        std::vector<std::string> database_table;
        std::map<std::string, std::function<bool(const SmartTable &)> >
        type_func_map;
        ...
        std::vector<std::string> names = {
            "namespace", "database_name", "table_name", "binlog_db",
    ...
    bool ShowHelper::_show_virtual_index(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        for (auto &iter: sample.index_id_sample_sqls_map) {
            const int64_t meta_id = ::ksearch::get_meta_id(iter.first);
            if (meta_id != 0) {
                continue;
            }
    ...
    bool ShowHelper::_show_ddl_work(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        if (show_region) {
            ...
            names = {
                "op_type", "index_state", "index_id", "begin_timestamp",
            ...
            for (auto ddl: response.ddlwork_infos()) {
                if (ddl.op_type() == pb::OP_MODIFY_FIELD) {
                    ...
                    if (ddl.job_state() == pb::IS_PUBLIC) {
                        ...
                        row.emplace_back(
                            std::to_string(work_done_count) + "/" + std::to_string(
                                work_done_count + work_to_be_done_count));
                    }
        ...
        for (auto &pair: region_sizes) {
            ...
            for (auto pair2: pair.second) {
                if (first) {
                    ...
                    if (first_size != 0) {
                        if (std::get < 0 > (pair2) * 1.0 / first_size > multiples || first_size * 1.0 / std::get < 0 > (
                                pair2) > multiples) {
                            diff = true;
                        }
                    } else {
    ...
    bool ShowHelper::_show_global_ddl_work(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        std::vector<ResultField> fields;
        std::vector<std::string> names = {
            "region_id", "start_key", "end_key", "status", "op_type", "index_id",
    ...
    bool ShowHelper::_show_switch(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        fields.reserve(4);
        std::vector<std::string> names = {
            "resource tag", "peer load balance", "migrate balance", "network segment balance"
        };
        for (auto &name: names) {
            ResultField field;
            field.name = name;
    ...
    bool ShowHelper::_show_index(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        for (auto &index_id: info.indices) {
            ...
            for (size_t i = 0; i < index_info.fields.size(); ++i) {
                rows.push_back({
                    table, std::to_string(non_unique), key_name, std::to_string(i),
    ...
    void ShowHelper::_query_offline_binlog_info(int64_t table_id, int64_t region_id,
        ...
        auto add_row = [&rows](int64_t region_id, const std::string &peers, int64_t oldest_ts, int64_t newest_ts,
                               const std::string &data_paths, const std::string &binlog_paths,
        ...
        for (const auto &peer: info.peers()) {
            ...
            if (query_one_region) {
                invalid_paths.clear();
                add_row(region_id, peer, peer_oldest_ts, peer_newest_ts, peer_data_paths, peer_binlog_paths,
                        invalid_paths, peer_has_diff);
                peer_has_diff = false;
            }
            is_first_peer = false;
        }
        if (!query_one_region) {
            add_row(region_id, peers, region_oldest_ts, region_newest_ts, data_paths, binlog_paths, invalid_paths,
                    peer_has_diff);
        }
    }
    ...
    bool ShowHelper::_show_offline_binlog(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        // Make fields.
        std::vector<std::string> names = {
            "region_id", "peers", "offline ts", "offline datetime", "data files", "binlog files", "invalid files",
            "diff between peers"
        };
        std::vector<ResultField> fields;
        fields.reserve(8);
    ...
    bool ShowHelper::_show_all_privileges(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        std::unordered_set<std::string> filter_user_set = {"fengmai_user", "f1_user", "ks_user", "root"};
    ...
    void ShowHelper::_parse_sample_sql(std::string sample_sql, std::string &database, std::string &table,
                                       std::string &sql) {
        // Remove comments.
        re2::RE2::Options option;
        option.set_encoding(re2::RE2::Options::EncodingLatin1);;
        ...
        DB_WARNING("sample_sql: %s, database: %s, table: %s, sql: %s", sample_sql.c_str(), database.c_str(),
