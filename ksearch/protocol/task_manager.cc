...
namespace ksearch {
    DEFINE_int32(worker_number, 20, "ksearch worker number.");
    ...
    void TaskManager::process_ddl_work(pb::RegionDdlWork work) {
        DB_NOTICE("begin ddl work task_%ld_%ld : %s", work.table_id(), work.region_id(),
        ...
        DB_NOTICE("ddl work task_%ld_%ld finish ok! %s", work.table_id(), work.region_id(),
                  work.ShortDebugString().c_str());
    }
} // namespace ksearch