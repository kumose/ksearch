...
#include <ksearch/protocol/epoll_info.h>
#include <ksearch/protocol/machine_driver.h>
...
namespace ksearch {
    ...
    private:
        ...
        void construct_other_heart_beat_request(pb::KsOtherHeartBeatRequest &request);

        void process_other_heart_beat_response(const pb::KsOtherHeartBeatResponse &response);
        ...
        int get_filter_sign_set(const std::map<uint64_t, std::string> &sign_sql_map,
                                std::set<uint64_t> &need_filter_sign);

        void insert_subquery_signs_info(std::map<uint64_t, std::set<uint64_t> > &parent_sign_to_subquery_signs,
        ...
        int query_sql(const std::string &sql, ks::client::ResultSet *result_set);
