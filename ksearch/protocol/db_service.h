...
namespace ksearch {
            ...
            do {
                ...
                if (response.errcode() != pb::SUCCESS) {
                    DB_FATAL("mpp execute fail: fragment fail, log_id:%lu, db_address: %s, err:%s, retry times: %d",
                             cntl.log_id(), db_address.c_str(), pb::ErrCode_Name(response.errcode()).c_str(),
                             retry_times);
                    continue;
                } else {
                    break;
    ...
    private:
        ...
        std::map<uint64_t, std::pair<int64_t, int> > _query_start_time_map;
        // key: log_id, value: (start_time, fragment_cnt)
