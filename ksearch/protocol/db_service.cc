...
#define DB_WARNING_TRANSMIT_DATA(_fmt_, args...) \
        ...
        if (!request->is_local_pass_through()) {
            ...
            *schema_str = request->vectorized_schema();
            ...
            std::shared_ptr<arrow::Buffer> buffer = std::make_shared<arrow::Buffer>(
                reinterpret_cast<const uint8_t *>(attachment_str->data()),
                static_cast<int64_t>(attachment_str->size()));
    ...
    void DbService::handle_fragment_start(google::protobuf::RpcController *controller,
        ...
        for (auto &[fragment_instance_id, pb_fragment]: fragment_need_to_build) {
            if (0 != fragment_internal_open(controller,
                                            ...
                                            userinfo)) {
                DB_FATAL(
                    "mpp execute fail: fragment internal build fail, log_id: %lu, fragment_id: %d, fragment_instance_id: %lu, remote_side: %s",
    ...
    int DbService::get_all_fragment_instance_id(google::protobuf::RpcController *controller,
                                                const pb::DAGFragmentRequest *request,
                                                std::unordered_map<uint64_t, pb::FragmentInfo> &
                                                fragment_need_to_build) {
        brpc::Controller *cntl = (brpc::Controller *) controller;
        uint64_t log_id = 0;
        if (cntl->has_log_id()) {
            log_id = cntl->log_id();
        ...
        for (int i = 0; i < request->fragments_size(); i++) {
            ...
            for (int j = 0; j < plan.nodes_size(); j++) {
                ...
                for (int k = 0; k < node.derive_node().exchange_sender_node().fragment_addresses_size(); k++) {
                    ...
                    uint64_t fragment_instance_id = node.derive_node().exchange_sender_node().fragment_addresses(k).
                            fragment_instance_id();
    ...
    int DbService::handle_fragment_scan_nodes(RuntimeState *state, std::vector<ExecNode *> &scan_nodes) {
        for (auto &node: scan_nodes) {
            ...
            if (sm_node != nullptr) {
                ...
                if (sm_node == nullptr) {
                    DB_FATAL("mpp fragment get select manager node fail, log_id: %lu, tuple_id: %d", state->log_id(),
                             scan_node->tuple_id());
                    return -1;
                }
                ...
                if (0 != Joiner::do_plan_router(state, {scan_node}, index_has_null, false, false)) {
                    DB_FATAL("mpp execute fail: fragment plan router fail, log_id: %lu, tuple_id: %d", state->log_id(),
                             scan_node->tuple_id());
                    return -1;
                }
                if (index_has_null) {
                ...
                if (!scan_node->is_file_scan_node() && !scan_node->is_mysql_scan_node()) {
                    DB_WARNING("scan node has no select manager node, log_id: %lu, tuple_id: %d", state->log_id(),
                               scan_node->tuple_id());
                    return -1;
                }
            }
    ...
    int DbService::fragment_internal_open(google::protobuf::RpcController *controller,
        ...
        ON_SCOPE_EXIT(([this, &log_id, &response, &root, &state, &ret, &err_msg, fragment_instance_id]() {
            if (ret < 0) {
                ...
                DB_FATAL("mpp execute fail: log_id: %lu, fragment_instance_id: %lu, error: %s", log_id,
                         fragment_instance_id, err_msg.c_str());
            }
        }));
    ...
    void DbService::fragment_internal_exec(std::vector<std::shared_ptr<FragmentInfo> > &fragment_need_to_exec,
        ...
        std::shared_ptr<ExecQueryFragments> batch_fragments = std::make_shared<ExecQueryFragments>(
            cur_time, is_main_db, fragment_need_to_exec);
        {
            BAIDU_SCOPED_LOCK(_query_start_time_map_lock);
            _query_start_time_map.
                    insert(std::make_pair(log_id, std::make_pair(cur_time, fragment_need_to_exec.size())));
        }
        for (std::shared_ptr<FragmentInfo> fragment_ptr: fragment_need_to_exec) {
            ...
            arrow::Result<std::shared_ptr<arrow::Table> > result_) {
                    ...
                    if (!result_.ok()) {
                        ...
                        DB_FATAL(
                            "mpp execute fail: fragment run fail, log_id: %lu, fragment_id: %d, frag_ins_id: %lu, remote_side: %s, open: %lu, exec: %lu, status: %s",
                            fragment_ptr->log_id,
                    ...
                    }
                    {
                        ...
                        for (auto &er: exchange_receiver_nodes) {
                            _finished_exchange_receiver_keys[static_cast<ExchangeReceiverNode *>(er)->receiver_key()] =
                                    cur_time;
                        }
                    }
                    ...
                    DB_NOTICE(
                        "mpp fragment run success: log_id: %lu, fragment_id: %d, frag_ins_id: %lu, exec_type: %s, remote_side: %s, "
    ...
    void DbService::query_cancel_thread() {
        while (!_shutdown) {
            ...
            {
                BAIDU_SCOPED_LOCK(_finished_exchange_receiver_keys_lock);
                for (auto iter = _finished_exchange_receiver_keys.begin();
                     iter != _finished_exchange_receiver_keys.end();) {
