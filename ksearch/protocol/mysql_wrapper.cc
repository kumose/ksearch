    ...
    int MysqlWrapper::decode_binary_protocol_value(
        ...
        switch (type.mysql_type) {
            ...
            case MYSQL_TYPE_TIMESTAMP: {
                ...
                if (length == 4 || length == 7 || length == 11) {
                    ...
                    if (length == 7 || length == 11) {
                        if (RET_SUCCESS !=
                            protocol_get_length_fixed_int(data, packet_len, offset, 1, time_struct.hour)) {
                            DB_FATAL("decode hour failed type: %d", type.mysql_type);
                            return RET_ERROR;
                        }
                        if (RET_SUCCESS != protocol_get_length_fixed_int(
                                data, packet_len, offset, 1, time_struct.minute)) {
                            DB_FATAL("decode minute failed type: %d", type.mysql_type);
                            return RET_ERROR;
                        }
                        if (RET_SUCCESS != protocol_get_length_fixed_int(
                                data, packet_len, offset, 1, time_struct.second)) {
                    ...
                    if (length == 11) {
                        if (RET_SUCCESS != protocol_get_length_fixed_int(
                                data, packet_len, offset, 4, time_struct.macrosec)) {
            ...
            case MYSQL_TYPE_TIME: {
                ...
                DateTime time_struct = DateTime();
                if (RET_SUCCESS !=
                    protocol_get_length_fixed_int(data, packet_len, offset, 1, time_struct.is_negative)) {
                    DB_FATAL("decode uint8 failed");
                    return RET_ERROR;
                }
