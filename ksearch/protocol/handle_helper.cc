...
#include <ksearch/protocol/handle_helper.h>
...
namespace ksearch {
    ...
    void HandleHelper::update_unhealthy_learners_schema(const pb::QueryRequest &query_req,
        ...
        for (auto &region_info: query_res.region_status_infos()) {
            ...
            *region_iter = update_region_info;
            ...
            //meta元信息清理
            MetaServerInteract::get_instance()->send_request("meta_manager", request_update_region,
                                                             response_update_region);
    ...
    bool HandleHelper::_handle_add_privilege(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        if (start != std::string::npos && end != std::string::npos) {
            ...
            if (db == "resource_tag") {
                ...
                if (permission && client->user_info->username != "ks_olap_additional") {
                    DB_FATAL("param invalid, only ks_olap_additional can set is_request_additional true");
                    client->state = STATE_ERROR;
                    return false;
                }
    ...
    bool HandleHelper::_handle_table_resource_tag(const SmartSocket &client,
        ...
        if (!is_operator_safe) {
            ...
            if (!is_force) {
                DB_FATAL("Table: %s, %s not empty, new resourcetag not safe.", full_name.c_str(),
                         unsafe_item_string.c_str());
                _wrapper->make_err_packet(client, ER_ERROR_COMMON, "%s Not Empty. Operate Not Safe. Need 'force'.",
                                          unsafe_item_string.c_str());
                client->state = STATE_ERROR;
                return false;
            }

            DB_WARNING("Table: %s, %s not empty, new resourcetag not safe, forced to execute.", full_name.c_str(),
                       unsafe_item_string.c_str());
        }
    ...
    bool HandleHelper::_handle_load_balance_switch(const SmartSocket &client,
                                                   const std::vector<std::string> &split_vec) {
        if (!client || !client->user_info) {
            DB_FATAL("param invalid");
            return false;
    ...
    bool HandleHelper::_handle_modify_partition_resourcetag(const SmartSocket &client,
                                                            const std::vector<std::string> &split_vec) {
        if (!client || !client->user_info) {
            DB_FATAL("param invalid");
            return false;
        ...
        int64_t ret = partition_ptr->calc_partitions(client->user_info, left_value, false, right_value, false,
                                                     partition_ids);
        if (ret < 0) {
            DB_FATAL("param invalid, no such partition with start date: %s and end date: %s", start_date.c_str(),
                     end_date.c_str());
            client->state = STATE_ERROR;
            return false;
        }
    ...
    bool HandleHelper::_handle_store_trans_leader(const SmartSocket &client,
                                                  const std::vector<std::string> &split_vec) {
    ...
    bool HandleHelper::_handle_schema_conf(const SmartSocket &client, const std::vector<std::string> &split_vec) {
        ...
        if (key == "need_merge") {
            ...
            if (key.find("blacklist") != key.npos) {
                ...
                DB_WARNING("rolling table_name: %s, table_id: %ld, signs: %s", full_name.c_str(), table_id,
                           value.c_str());
                ...
                DB_WARNING("sign_exec_type table_name: %s, table_id: %ld, signs: %s", full_name.c_str(), table_id,
                           value.c_str());
            }
            ...
            if (key.find("enable") != key.npos || key.find("disable") != key.npos) {
                ...
                if (key.find("snapshot") != key.npos) {
                    if (table->get_field_id_by_short_name("__snapshot__") < 0) {
                        DB_FATAL(" table name: %s, table_id: %ld has no __snapshot__ field", full_name.c_str(),
                                 table_id);
                        client->state = STATE_ERROR;
                        return false;
                    }
                    ...
                    DB_WARNING("snapshot blacklist table_name: %s, table_id: %ld, snapshot: %s, is_add: %d",
                               full_name.c_str(), table_id, value.c_str(), is_add);
                }
            } else {
                DB_FATAL("param invalid");
                client->state = STATE_ERROR;
            ...
            if (is_open && !(table->schema_conf.use_column_storage() && table->schema_conf.enable_column_engine())) {
                DB_FATAL("table name: %s, table_id: %ld need use column storage and enable column engine",
                         full_name.c_str(), table_id);
                client->state = STATE_ERROR;
                return false;
            }
    ...
    bool HandleHelper::_handle_store_compact_region(const SmartSocket &client,
                                                    const std::vector<std::string> &split_vec) {
        ...
        for (const auto &addr: store_addr) {
            ...
            DB_WARNING("compact_region store: %s, req: %s, res: %s", addr.c_str(), req.ShortDebugString().c_str(),
    ...
    bool HandleHelper::_handle_specify_split_keys(const SmartSocket &client,
                                                  const std::vector<std::string> &split_vec) {
