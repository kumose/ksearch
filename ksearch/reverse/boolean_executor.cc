...
namespace ksearch {
    ...
    const PostingNodeT *OrBooleanExecutor::advance(
        ...
        while ((*clauses.begin())->current_id() != nullptr && (*clauses.begin())->current_id()->compare(target_id) <
               0) {
    ...
    // ------------------
    WeightedBooleanExecutor::WeightedBooleanExecutor(bool_executor_type type, BoolArg *arg) : _op_executor(NULL) {
        this->_is_null_flag = false;
