...
namespace ksearch {
    ...
    public:
        ...
        bool ParseFromString(const std::string &val) {
            ...
            auto ret = arrow::ipc::ReadRecordBatch(get_arrow_schema(), nullptr, arrow::ipc::IpcReadOptions::Defaults(),
                                                   &buf_reader);
            if (ret.ok()) {
        ...
        ArrowReverseNode reverse_nodes(int index) const {
            if (index != _current_node_index) {
                ...
                node.set_flag(ksearch::pb::ReverseNodeType(_flags_ptr->Value(index)));
