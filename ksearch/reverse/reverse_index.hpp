...
namespace ksearch {
    ...
    int ReverseIndex<Schema>::get_reverse_list_two(
        ...
        if (schema_info != nullptr) {
            ...
            item_statistic = &schema_info->schema->statistic().term_times[
                schema_info->schema->statistic().term_times.size() - 1];
        }
        ...
        if (is_fast) {
            ...
            std::unique_ptr<myrocksdb::Iterator> iter_first_new(
                new myrocksdb::Iterator(txn->GetIterator(roptions, data_cf)));
    ...
    int ReverseIndex<Schema>::_get_level_reverse_list(
        ...
        if (is_statistic) {
            ...
            if (schema_info && schema_info->schema) {
                item_statistic =
                        &schema_info->schema->statistic().term_times[
                            schema_info->schema->statistic().term_times.size() - 1];
            }
        }
        TimeCost time;
