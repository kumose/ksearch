...
namespace ksearch {
    ...
    rocksdb::CompactionServiceScheduleResponse MyCompactionService::Schedule(
        ...
        if (info.is_l0_compaction == true) {
            DB_WARNING("remote_compaction: %s is L0 compaction, use local compaction",
        ...
        closure->cntl.set_request_code(make_sign32(address)); {
            ...
            if (_doing_map.size() > FLAGS_remote_compaction_client_max_num) {
                DB_WARNING(
                    "remote_compaction: %s, doing remote compaction num: %ld > max num: %d, use local compaction",
...
#endif
        return rocksdb::CompactionServiceScheduleResponse(remote_compaction_id_str,
                                                          rocksdb::CompactionServiceJobStatus::kSuccess);
    }
    ...
    rocksdb::CompactionServiceJobStatus MyCompactionService::Wait(const std::string &remote_compaction_id_str,
                                                                  std::string *result) {
        ...
        rocksdb::Status s = rocksdb::CompactionServiceResult::Read(response.compaction_result_binary(),
                                                                   &compaction_result);
        if (!s.ok()) {
