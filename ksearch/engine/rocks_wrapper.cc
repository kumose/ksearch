...
#include <ksearch/engine/transaction_db_bthread_mutex.h>
...
namespace ksearch {
    ...
    int32_t RocksWrapper::init(const std::string &path) {
        ...
        DB_NOTICE("FLAGS_rocks_transaction_lock_timeout_ms:%d FLAGS_rocks_default_lock_timeout_ms:%d",
        ...
        _data_cf_option.compression_per_level = {
...
#if ROCKSDB_MAJOR >= 7
                auto cache_opt = rocksdb::HyperClockCacheOptions(
                    rocksdb_gflags.rocks_block_cache_size_mb() * 1024 * 1024LL, rocksdb_gflags.rocks_block_size(), 8);
                cache_opt.metadata_charge_policy = rocksdb::kDontChargeCacheMetadata;
                //cache会比rocks_block_cache_size_mb多占用少量内存
                table_options.block_cache = cache_opt.MakeSharedCache();
#endif
            } else {
                table_options.block_cache = rocksdb::NewLRUCache(
                    rocksdb_gflags.rocks_block_cache_size_mb() * 1024 * 1024LL, 8);
            }
        }
    ...
    void RocksWrapper::get_sst_properties(const std::string &start, const std::string &end,
    ...
    void RocksWrapper::get_cold_sst_properties(const std::string &start, const std::string &end,
                                               rocksdb::TablePropertiesCollection &props) {
        std::vector<rocksdb::Range> ranges{rocksdb::Range(start, end)};
    ...
    void RocksWrapper::decode_key_points(const std::string &region_start_key, const std::string &region_end_key,
                                         const std::string &prefix, const rocksdb::TablePropertiesCollection &props,
                                         std::vector<rocksdb::Slice> &out_keys) {
