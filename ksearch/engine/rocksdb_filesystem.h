...
#define FS_LOG(level, _fmt_, args...) \
    ...
    public:
        ...
        rocksdb::IOStatus NewSequentialFile(const std::string &f,
            ...
            FS_LOG(WARNING, "file: %s, iotype: %d, status: %s", f.c_str(), (int)file_opts.io_options.type,
                   s.ToString().c_str());
            return s;
        }
        ...
        rocksdb::IOStatus ReopenWritableFile(const std::string &fname,
            ...
            FS_LOG(WARNING, "file: %s, iotype: %d, status: %s", fname.c_str(), (int)file_opts.io_options.type,
                   s.ToString().c_str());
            return s;
        }
        ...
        rocksdb::IOStatus FileExists(const std::string &f, const rocksdb::IOOptions &io_opts,
            ...
            if (_use_ext_fs && is_sst(f, short_name)) {
                ...
                if (ret < 0) {
                    ...
                    FS_LOG(WARNING, "Found ext file: %s short_name: %s, iotype: %d", f.c_str(), short_name.c_str(),
                           (int)io_opts.type);
                    return rocksdb::IOStatus::OK();
                }
            }
        ...
        rocksdb::IOStatus Truncate(const std::string &fname, size_t size,
            ...
            FS_LOG(WARNING, "file: %s, size: %ld, iotype: %d, status: %s", fname.c_str(), size, (int)options.type,
                   s.ToString().c_str());
            return s;
        }
        ...
        rocksdb::IOStatus GetFileModificationTime(const std::string &fname,
            ...
            if (_use_ext_fs && is_sst(fname, short_name)) {
                ...
                if (ret < 0) {
                    FS_LOG(FATAL, "GetFileModificationTime file: %s short_name: %s failed", fname.c_str(),
                           short_name.c_str());
            ...
            FS_LOG(WARNING, "local file: %s, modify_time: %lu, status: %s", fname.c_str(), *file_mtime,
                   s.ToString().c_str());
            return s;
        }
        ...
        rocksdb::IOStatus GetAbsolutePath(const std::string &db_path, const rocksdb::IOOptions &options,
            ...
            FS_LOG(WARNING, "db_path: %s, output_path: %s, status: %s", db_path.c_str(), output_path->c_str(),
                   s.ToString().c_str());
            return s;
        }
        ...
        rocksdb::IOStatus RenameFile(const std::string &s, const std::string &t,
            ...
            FS_LOG(WARNING, "RenameFile local file: %s => %s, status: %s", s.c_str(), t.c_str(),
        ...
        rocksdb::IOStatus LinkFile(const std::string &s, const std::string &t,
            ...
            if (_use_ext_fs && is_sst(t, t_short_name)) {
                ...
                if (ret < 0) {
                    FS_LOG(FATAL, "LinkFile file: %s => %s, t_short_name: %s failed", s.c_str(), t.c_str(),
                           t_short_name.c_str());
                    return rocksdb::IOStatus::IOError("LinkFile failed");
                }
                FS_LOG(WARNING, "LinkFile ext file: %s => %s, t_short_name: %s", s.c_str(), t.c_str(),
                       t_short_name.c_str());
                return rocksdb::IOStatus::OK();
            }
        ...
        rocksdb::IOStatus AreFilesSame(const std::string &first, const std::string &second,
            ...
            FS_LOG(FATAL, "first: %s, second: %s, res: %d, status: %s", first.c_str(), second.c_str(), *res,
                   s.ToString().c_str());
            return s;
        }
    ...
    public:
        ...
        rocksdb::IOStatus AreFilesSame(const std::string &first, const std::string &second,
                                       const rocksdb::IOOptions &options, bool *res,
                                       rocksdb::IODebugContext *dbg) override {
            FS_LOG(FATAL, "NotSupported AreFilesSame first: %s, second: %s, res: %d", first.c_str(), second.c_str(),
                   *res);
