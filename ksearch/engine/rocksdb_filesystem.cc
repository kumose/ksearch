...
namespace ksearch {
    ...
    rocksdb::IOStatus ExtRandomAccessFile::Read(uint64_t offset, size_t n, const rocksdb::IOOptions &options,
                                                ...
                                                rocksdb::IODebugContext *dbg) const {
        rocksdb::IOStatus s;
        ssize_t r = -1;
        size_t left = n;
        char *ptr = scratch;
    ...
    int SstExtLinker::sst_rename(const std::string &src_short_name, const std::string &dst_short_name) {
        ...
        if (src_iter == _sst_ext_map.end() || dst_iter != _sst_ext_map.end()) {
            DB_FATAL("sst rename failed, src_short_name: %s, dst_short_name: %s", src_short_name.c_str(),
                     dst_short_name.c_str());
            return -1;
        }
        ...
        std::string record = "rename\t" + dst_short_name + "\t" + src_short_name + "\t" + std::to_string(
                                 dst_info.modify_time) + "\n";
        int ret = append_file(record);
        if (ret < 0) {
            // map回滚
    ...
    int SstExtLinker::new_sst_random_access_file(const std::string &short_name,
        ...
        if (ret < 0) {
            DB_FATAL("new_ext_random_access_file: %s, short_name: %s failed", ext_file_name.c_str(),
                     short_name.c_str());
            return -1;
        }
    ...
    */
    ...
    int SstExtLinker::do_rewrite_file() {
        ...
        try {
            ...
            int link_num = 0;
            for (const auto &info: _sst_ext_map) {
                std::string record = "link\t" + info.first + "\t" + info.second.full_name + "\t" + std::to_string(
                                         info.second.size) + "\t"
