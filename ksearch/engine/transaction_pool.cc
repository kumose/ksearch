    ...
    void TransactionPool::clear_transactions(Region *region) {
        ...
        auto call = [this,
            ...
            if (cur_time - txn->begin_time > FLAGS_long_live_txn_interval_ms * 1000LL) {
                if (txn->has_write()) {
                    DB_FATAL(
                        "TransactionWarning: txn %s seq_id: %d  remote_side: %s is alive for %d ms, %ld, %ld, %lds",
            ...
            if (txn->is_primary_region() &&
                ...
                DB_FATAL(
                    "TransactionFatal: primary txn %s seq_id: %d remote_side: %s is idle for %ld ms, %ld, %ld, %lds",
                    txn->get_txn()->GetName().c_str(), txn->seq_id(),
                    txn->remote_side.c_str(),
                    txn_timeout,
                    cur_time,
    ...
    void TransactionPool::clear_orphan_transactions() {
        ...
        while (true) {
            ...
            auto call = [this, &need_erase, &has_process, retry](SmartTransaction &txn) {
                ...
                if (txn->in_process()) {
                    DB_WARNING(
                        "txn is processing region_id:%ld txn_id: %lu remote_side: %s need delay rollback, retry:%d",
