...
namespace ksearch {
    ...
    bool OLAPMergeOperator::FullMergeV2(const rocksdb::MergeOperator::MergeOperationInput &merge_in,
        ...
        ScopeGuard auto_decrease([this, table_id, region_id, &merge_in, merge_out]() {
            merge_out->existing_operand = merge_in.existing_value == nullptr
                                              ? merge_in.operand_list[0]
        ...
        if (table_info == nullptr || (table_info->id != table_id && table_info->rollup_indexs.count(table_id) <= 0) ||
            table_cache_time.get_time() > 60 * 1000 * 1000LL) {
