...
#include <ksearch/common/key_encoder.h>
#include <ksearch/common/common.h>
#include <rocksdb/advanced_cache.h>
#include <ksearch/engine/rocksdb_compaction_service.h>
...
namespace ksearch {
        ...
        rocksdb::Status Finish(rocksdb::UserCollectedProperties *properties) override {
            if (_value_encoded.size() > 0) {
                *properties = rocksdb::UserCollectedProperties{
                ...
                DB_WARNING(
                    "CountingKeyTblPropCollector: total_key_count: %d, total_key_point_count: %d, keypoint total size: %lu",
    ...
    public:
        ...
        rocksdb::Status ingest_to_cold(const std::vector<std::string> &external_files) {
            ...
            // 设置为false时，如果和memtable有重叠则会失败，cold rocksdb不会写入，不会产生memtable所以设置为false也安全
        ...
        rocksdb::Status ingest_offline_binlog_sst(const std::vector<std::string> &external_files,
                                                  bool is_binlog_cf = false) {
            rocksdb::IngestExternalFileOptions options;
            // WARNING! 不要调整以下options
            // WARNING! 不要调整以下options
            // WARNING! 不要调整以下options
        ...
        void get_sst_properties(const std::string &start, const std::string &end,
                                rocksdb::TablePropertiesCollection &props);

        void get_cold_sst_properties(const std::string &start, const std::string &end,
                                     rocksdb::TablePropertiesCollection &props);
