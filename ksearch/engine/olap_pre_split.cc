...
namespace ksearch {
    ...
    int OlapPreSplit::olap_pre_split(int64_t table_id) {
        ...
        if (average_rows < FLAGS_olap_region_pre_split_min_rows) {
            ...
            DB_WARNING(
                "table_name: %s, average rows less than min pre split rows, total[%ld], average rows[%ld], pre_split_region_cnt[%d], sample_date: %s",
        ...
        int ret = DDLPlanner::update_specify_split_keys(table->namespace_, split_vec[0], split_vec[1],
        ...
        DB_NOTICE(
            "END auto pre split success, table: %s, total_rows:%ld, average_rows:%ld, sample_date: %s, split size: %ld, pre_split_region_cnt: %d, keys: %s",
            table->name.c_str(), total_rows, average_rows, start_str.c_str(), pre_split_keys.size(),
            pre_split_region_cnt, boost::join(pre_split_keys, ",").c_str());
        return 0;
    }
}