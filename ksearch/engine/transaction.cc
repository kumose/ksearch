 ...
 */
    ...
    int64_t ttl_decode(rocksdb::Slice &value, const IndexInfo *const index_info, int64_t base_expire_time_us) {
        ...
        if (!valid_timestamp_us(time)) {
            DB_DEBUG("invalid timestamp, index id: %ld, name: %s, ttl time: %ld, data size: %lu, invalid time: %ld",
                     index_info->id, pb::IndexType_Name(index_info->type).c_str(), base_expire_time_us, value.size(),
                     time);
            return base_expire_time_us;
        }
    ...
    rocksdb::Status Transaction::put_kv_without_lock(const std::string &key, const std::string &value,
                                                     int64_t ttl_timestamp_us) {
        TimeCost cost;
        // support ttl
        ...
        DB_DEBUG("key:%s, v:%s, ,use_normal_ttl:%d ttl_timestamp_us:%ld", str_to_hex(key).c_str(),
                 str_to_hex(value).c_str(), _use_normal_ttl, ttl_timestamp_us);
        if (_use_normal_ttl &&ttl_timestamp_us
    ...
    int Transaction::get_update_primary(
        ...
        if (res.ok()) {
            ...
            if (mode == GET_ONLY || mode == GET_LOCK) {
                ...
                if (_use_normal_ttl &&_read_ttl_timestamp_us
                ...
                if (use_memrow) {
                    ...
                    if (reverse_records_vec_ptr != nullptr) {
                        ...
                        if (querywords_slot_id != -1) {
                            ...
                            mem_row->set_value(tuple_id, querywords_slot_id,
                                               reverse_record->get_value(querywords_desc));
                        }
                    }
                    ...
                    if (reverse_records_vec_ptr != nullptr) {
                        ...
                        if (querywords_slot_id != -1) {
                            querywords_desc = reverse_record->get_field_by_idx(querywords_pb_idx);
                            ExprValue *field = row_batch->get_chunk()->
                                    get_tmp_field_value(tuple_id, querywords_slot_id);
    ...
    int Transaction::remove(int64_t region, IndexInfo &index, /*IndexInfo& pk_index,*/
        ...
        if (index.type == pb::I_ROLLUP) {
            ...
            if (rollup_record->encode_value_for_rollup(index, _table_info->fields, _table_info->fields_need_sum, true)
                != 0) {
                DB_FATAL("Fail to encode_rollup, reg:%ld, tab:%ld", region, index.pk);
                return -1;
            }
    ...
    int Transaction::put_primary_columns(const TableKey &primary_key, SmartRecord record,
        ...
        for (auto &field_info: _table_info->fields) {
            ...
            if (update_fields != nullptr // an old row
                && update_fields->count(field_id) == 0) {
                // no need to update
                continue;
            }
            std::string value;
            ...
            if (ret != 0) {
                if (update_fields != nullptr) {
                    // delete when update or replace
