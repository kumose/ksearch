...
namespace ksearch {
    ...
    public:
        ...
        void reset_rate(const int64_t rate) {
            int64_t min_global_extended_tokens = FLAGS_max_tokens_per_second * FLAGS_min_global_extended_percent / 100;
            // 最小全局超额令牌个数
            _valid = true;
            _rate = rate;
            if (rate <= 0) {
            ...
            if (_is_extended) {
                if (rate <= min_global_extended_tokens) {
                    ...
                    _statistics_threshold = (rate - min_global_extended_tokens) / (
                                                1000000/*1s*/ / (FLAGS_token_bucket_burst_window_ms * 1000)) *
                                            _time_per_token;
                }
    ...
    public:
        ...
        int64_t fetch_tokens(const int64_t tokens, int64_t *token_expire_time, bool *is_committed_bucket,
