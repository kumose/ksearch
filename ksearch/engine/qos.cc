...
namespace ksearch {
        ...
        *expire_time = window_end;
        ...
        while (true) {
            ...
            if (_time.compare_exchange_weak(old_time, desired_time)) {
                if (_is_extended &&desired_time
                
                >
                (window_start + _statistics_threshold)
                )
                {
    ...
    int64_t SqlQos::fetch_tokens(const int64_t tokens, int64_t *token_expire_time, bool *is_committed_bucket,
    ...
    int StoreQos::mark_need_reject_sql() {
        ...
        for (auto it = sql_with_multi_index.rbegin(); it != sql_with_multi_index.rend(); it++) {
            ...
            if (index_id <= 0) {
                DB_WARNING(
                    "conditions not met. sign: %lu, log_term [get: %ld, scan: %ld], short_term [get: %ld, scan: %ld]",
                    it->first, it->second.get_long_term, it->second.scan_long_term, it->second.get_short_term,
                    it->second.scan_short_term);
                continue;
            }
            ...
            if (sql_it != ptr->end()) {
                DB_WARNING(
                    "sql use multi index need reject sign: %lu, log_term [get: %ld, scan: %ld], short_term [get: %ld, scan: %ld]",
                    it->first, it->second.get_long_term, it->second.scan_long_term, it->second.get_short_term,
                    it->second.scan_short_term);
