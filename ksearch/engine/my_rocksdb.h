...
#define COLD_DB_CHECK_WRITE \
    do {\
        ...
        class Transaction {
        public:
            explicit Transaction(rocksdb::Transaction *txn, bool use_cold_db,
                                 rocksdb::ColumnFamilyHandle *handle) : _txn(txn), _use_cold_db(use_cold_db),
                                                                        _cold_data_handle(handle) {
            }
