...
#    exit 1
#fi
#mysql -h10.94.160.61 -uroot -pfast -P8121 -Dfengmai -e "select * from ksearch_level" > out
#mysql -hsmartbns.fcdb7113-bmi0000.xdb.all.serv -uksearch_read -p3dJ_1xvrVG2y -P5616 -DFC_Fengmai
WATT_PARAM_HOST="smartbns.fcdb7113-bmi0000.xdb.all.serv"
WATT_PARAM_PORT="5616"
WATT_PARAM_USER="ksearch_read"
WATT_PARAM_PASSWORD="3dJ_1xvrVG2y"
WATT_PARAM_DB="FC_Fengmai"
WATT_PARAM_TABLE="ksearch_level"
...
if [ X$OPT_STREAM_NAME != X ];then
    ...
    mysql -h$WATT_PARAM_HOST -P$WATT_PARAM_PORT -u$WATT_PARAM_USER -p$WATT_PARAM_PASSWORD -D$WATT_PARAM_DB -e "select * from $WATT_PARAM_TABLE where ksearch_dbname in ('$OPT_DATABASE') and ksearch_tablename in ('$OPT_TABLE')" > mysql_out_tmp
    if [ $? -ne 0 ];then
        echo "mysql exec failed"
        fail_quit
    fi
...
DATABASES=($(get_column_multi "ksearch_dbname"))
TABLES=($(get_column_multi "ksearch_tablename"))
