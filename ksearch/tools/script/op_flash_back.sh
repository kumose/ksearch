...
function conf_init() {
    ...
    charset=$(grep "charset" conf/ks_client.conf | awk -F" " '{print $2}')
    ...
    do
        ...
        case $cluster in
            DEFAULT)
                ...
                ;;
            PS)
                meta_bns="group.opera-ps-ksMeta-000-bj.FENGCHAO.all"
                namespace="FENGCHAO"
                ks_client_file="conf/ks_client.conf.ps"
                break
                ;;
            PAP)
                meta_bns="group.opera-pap-ksMeta-000-bj.FENGCHAO.all"
                namespace="FENGCHAO"
                ks_client_file="conf/ks_client.conf.pap"
                break
                ;;
            BAIJIA)
                meta_bns="group.opera-online-ksMeta-000-bj.BZ.all"
                namespace="FENGCHAO"
                ks_client_file="conf/ks_client.conf.baijia"
                break
                ;;
        esac
    done
    ...
    sed -i  "s/^charset:.*/charset: $charset/" conf/ks_client.conf
    ...
    if [ "$cluster" != "DEFAULT" ]; then
        cp $ks_client_file conf/ks_client.conf
    fi
