...
namespace ksearch {
    ...
    BthreadArrowExecutor::BthreadArrowExecutor() : sp_state_(std::make_shared<BthreadArrowExecutor::State>()),
    ...
    void GlobalArrowExecutor::execute(RuntimeState *state, arrow::Result<std::shared_ptr<arrow::Table> > *result) {
        if (state->vectorlized_parallel_execution == false) {
            // 不开启pipeline并行
            *result = arrow::acero::DeclarationToTable(
                arrow::acero::Declaration::Sequence(std::move(state->acero_declarations)),
                /*use_threads=*/false,
            ...
            bool done = false;
            auto table_future = arrow::acero::DeclarationToTableAsync(
                arrow::acero::Declaration::Sequence(std::move(state->acero_declarations)), exec_context);
    ...
    void GlobalArrowExecutor::shutdown() {
        ...
        auto rs = arrow::internal::GetCpuThreadPool()->Shutdown(/*wait=*/false);
        DB_WARNING("GlobalArrowExecutor shutdown finish %s", rs.ToString().c_str());
    }
