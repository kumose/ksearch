...
namespace ksearch {
    ...
    DECLARE_int64 (ksearch_alive_time_s);
    ...
    int64_t RuntimeState::calc_single_store_concurrency(pb::OpType op_type) {
        ...
        int64_t ksearch_alive_time_us = SchemaFactory::get_instance()->get_ksearch_alive_time_us();
        if (ksearch_alive_time_us < FLAGS_ksearch_alive_time_s * 1000 * 1000LL) {
            return single_store_concurrency;
        }
        if (sign == 0) {
            return single_store_concurrency;
    ...
    int RuntimeState::memory_limit_exceeded(int64_t rows_to_check, int64_t bytes) {
        ...
        if (_mem_tracker == nullptr) {
            ...
            if (_mem_tracker == nullptr) {
                _mem_tracker = ksearch::MemTrackerPool::get_instance()->get_mem_tracker(_log_id);
            }
        }
