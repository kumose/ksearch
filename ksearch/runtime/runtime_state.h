...
#include <ksearch/protocol/data_buffer.h>
...
#include <ksearch/reverse/reverse_interface.h>
...
namespace ksearch {
    ...
    public:
        ...
        // for prepared txn recovery in Ksearch
    ...
    public:
        ...
        // -------- 此部分皆为收集统计信息相关 ----------------
        int64_t last_insert_id = INT64_MIN; //存储 ksStore last_insert_id(expr)更新的字段
    ...
    private:
        ...
        int64_t _num_affected_rows = 0; //存储ksearch写影响的行数
        int64_t _num_returned_rows = 0; //存储ksearch读返回的行数
        int64_t _num_scan_rows = 0; //存储 ksStore 扫描行数
        int64_t _num_filter_rows = 0; //存储过滤行数
        int64_t _read_disk_size = 0; // 扫描大小
        int64_t _db_handle_rows = 0; //存储ksearch处理行数
        int64_t _db_handle_bytes = 0; //存储ksearch处理字节数
        uint64_t _log_id = 0;

        bool _single_sql_autocommit = true; // used for ksearch and store
        ...
        NetworkSocket *_client_conn = nullptr; // used for ksearch
        int64_t _single_store_concurrency = -1; // used for ksearch
