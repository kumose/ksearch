...
#include <vector>
#include <memory>
#include <ksearch/runtime/chunk.h>
...
namespace ksearch {
    ...
    int Chunk::finish_and_make_record_batch(std::shared_ptr<arrow::RecordBatch> *out,
    ...
    int Chunk::finish_and_reset_one_column(std::vector<std::shared_ptr<arrow::Array> > &arrays, int &idx) {
        switch (_field_types[idx]) {
            ...
            case FieldDescriptorProto::TYPE_BYTES: {
                // pb::STRING, pb::HLL, pb::BITMAP, pb::TDIGEST
    ...
    int Chunk::append_value(int field_pos, const ExprValue &value) {
        ...
        if (value.type == pb::NULL_TYPE) {
            auto rs = _builders[field_pos]->AppendNull();
            TURBO_UNUSED(rs);
            return 0;
        }
        arrow::Status s;
