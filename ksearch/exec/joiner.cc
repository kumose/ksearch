...
namespace ksearch {
    ...
    int Joiner::strip_out_equal_slots() {
        ...
        for (auto i = 0; i < _outer_equal_slot.size(); ++i) {
            ...
            if (outer_type == inner_type) {
                ...
                if (FLAGS_join_key_cast_like_mysql && !is_string(outer_type) && is_string(inner_type)) {
    ...
    int Joiner::do_plan_router(RuntimeState *state, const std::vector<ExecNode *> &scan_nodes, bool &index_has_null,
                               bool is_explain, bool in_acero) {
        ...
        for (auto &exec_node: scan_nodes) {
            ...
            auto get_slot_id = [ctx](int32_t tuple_id, int32_t field_id) ->
                int32_t {
                return ctx->get_slot_id(tuple_id, field_id);
            };
            auto get_tuple_desc = [ctx](int32_t tuple_id)->
                pb::TupleDescriptor * {
                return ctx->get_tuple_desc(tuple_id);
            };
    ...
    int Joiner::runtime_filter(RuntimeState *state, ExecNode *node, std::vector<ExprNode *> *in_exprs_back,
        ...
        for (auto node: dual_scan_nodes) {
            ...
            bool index_has_null = false;
            if (do_plan_router(sub_query_runtime_state, derived_scan_nodes, index_has_null, _is_explain,
                               in_acero) != 0) {
                DB_WARNING("Fail to do_plan_router");
                return -1;
            }
    ...
    int Joiner::construct_in_condition(RuntimeState *state,
        ...
        if (check_mem) {
            DB_WARNING("logid: %lu, in_values size:%lu, estimate_size:%ld", state->log_id(), in_values.size(),
                       total_estimate_size);
        }
        return 0;
    }
    ...
    int Joiner::show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output, int &next_id,
                             int display_id) {
