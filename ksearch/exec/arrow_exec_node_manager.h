...
namespace ksearch {
    ...
    public:
        explicit MakeDefaultAggRowWhenNoInputOptions(std::unordered_set<std::string> &count_name) : count_names(
            count_name) {
        }
    ...
    public:
        ...
        explicit TopKNodeOptions(arrow::compute::Ordering ordering, int64_t k_num) : ordering(std::move(ordering)),
            k(k_num) {
        }
