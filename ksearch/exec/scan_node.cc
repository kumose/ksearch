...
#include <map>
#include <cfloat>
#include <ksearch/exec/mysql_scan_node.h>
#include <ksearch/exec/scan_node.h>
#include <ksearch/exec/join_node.h>
#include <ksearch/exec/parquet_scan_node.h>
...
#include <ksearch/exec/information_schema_scan_node.h>
#include <ksearch/exec/redis_scan_node.h>
...
namespace ksearch {
    int64_t AccessPathMgr::select_index_common() {
        ...
        for (auto &pair: _paths) {
            ...
            if (_use_force_index &&path
            
            ->
            hint != AccessPath::FORCE_INDEX
            )
            {
                prefix_ratio_index_score = 0;
            }
        ...
        // 优先选命中字段最多的向量索引
        for (auto iter = vector_hit_field_count_mapping.crbegin(); iter != vector_hit_field_count_mapping.crend(); ++
             iter) {
            return iter->second;
        }
        // 优先选倒排
    ...
    int ScanNode::show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
        ...
        if (_merge_index_infos.size() > 0) {
            ...
            std::string tmp;
            //int64_t index_id = select_index_in_ksearch(tmp);
    ...
    int AccessPathMgr::compare_two_path(SmartPath &outer_path, SmartPath &inner_path) {
        ...
        if (outer_path->hit_index_field_ids.size() == inner_path->hit_index_field_ids.size()) {
            ...
            if (!inner_path->need_filter() && outer_path->need_filter() && outer_path->is_sort_index <= inner_path->
                is_sort_index) {
                outer_path->is_possible = false;
                return -1;
            }
            ...
            if (!outer_path->need_filter() && inner_path->need_filter() && inner_path->is_sort_index <= outer_path->
                is_sort_index) {
                inner_path->is_possible = false;
                return -2;
            }
    ...
    int64_t AccessPathMgr::pre_process_select_index() {
        ...
        for (auto outer_loop_iter = _paths.begin(); outer_loop_iter != _paths.end(); outer_loop_iter++) {
            ...
            if (outer_loop_iter->second->index_type == pb::I_PRIMARY ||
                outer_loop_iter->second->index_type == pb::I_UNIQ) {
                if (outer_loop_iter->second->index_field_ids.size() == outer_loop_iter->second->hit_index_field_ids.
                    size()
    ...
    int64_t ScanNode::select_join_index_in_ksearch(const std::string &sample_sql) {
        ...
        if (path->is_virtual && !_join_path.get_explain_hint_flag<ExplainHint::HintType::KEEP_VIRTUAL>()) {
            ...
            return select_join_index_in_ksearch(sample_sql);
        }
    ...
    int64_t ScanNode::select_index_in_ksearch(const std::string &sample_sql) {
        ...
        if (path->is_virtual && !_main_path.get_explain_hint_flag<ExplainHint::HintType::KEEP_VIRTUAL>()) {
            ...
            return select_index_in_ksearch(sample_sql);
        }
            ...
            if (learner_idx != 0 && learner_idx != path->index_id) {
                ...
                if (learner_path->index_info_ptr->is_global) {
                    serialize_index_and_set_router_index(learner_path->pos_index, &learner_path->pos_index,
                                                         learner_path->is_covering_index,
                                                         ScanIndexInfo::U_GLOBAL_LEARNER);
                } else {
                    ...
                    serialize_index_and_set_router_index(learner_path->pos_index,
                                                         &_main_path.path(_table_id)->pos_index,
                ...
                if (get_parent()->node_type() == pb::TABLE_FILTER_NODE ||
                    ...
                    static_cast<FilterNode *>(get_parent())->modifiy_pruned_conjuncts_by_index_learner(
                        learner_condition);
                }
            }
        }
