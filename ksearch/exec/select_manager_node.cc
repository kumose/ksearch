...
#include <ksearch/exec/limit_node.h>
#include <ksearch/exec/agg_node.h>
...
#include <ksearch/exec/join_node.h>
#include <ksearch/exec/vectorize_helpper.h>
#include <ksearch/runtime/arrow_io_excutor.h>
#include <ksearch/exec/arrow_exec_node_manager.h>
#include <ksearch/exec/file_scan_node.h>
...
namespace ksearch {
    ...
    int SelectManagerNode::open(RuntimeState *state) {
        ...
        if ((dual_scan_nodes.size() + scan_nodes.size() != 1) && exchange_receiver_node == nullptr) {
            ...
            DB_WARNING(
                "Invalid SelectManagerNode, dual_scan_nodes_size: %lu, scan_nodes_size: %lu, txn_id: %lu, log_id:%lu",
    ...
    int SelectManagerNode::init(const pb::PlanNode &node) {
        ...
        if (pb_schema.size() > 0) {
            std::shared_ptr<arrow::Buffer> schema_buffer = std::make_shared<arrow::Buffer>(
                reinterpret_cast<const uint8_t *>(pb_schema.data()),
    ...
    void SelectManagerNode::transfer_pb(int64_t region_id, pb::PlanNode *pb_node) {
        ...
        if (!is_dual_scan() && !_has_er_child) {
            ...
            arrow::Result<std::shared_ptr<arrow::Buffer> > schema_ret = arrow::ipc::SerializeSchema(
                *_arrow_schema, GetMemoryPoolForRead());
            if (!schema_ret.ok()) {
    ...
    int SelectManagerNode::build_arrow_declaration(RuntimeState *state) {
        ...
        RocksdbScanNode *scan_node = static_cast<RocksdbScanNode *>(get_node(pb::SCAN_NODE));
        if (_delay_fetcher_store &&_index_collector_cond
        ...
        START_LOCAL_TRACE_WITH_PARTITION_PROPERTY(get_trace(), state->get_trace_cost(), &_partition_property,
                                                  OPEN_TRACE, nullptr);
        int ret = 0;
        // add SourceNode
        std::shared_ptr<FetcherStoreVectorizedReader> vectorized_reader = std::make_shared<
        ...
        std::function < arrow::Iterator<std::shared_ptr<arrow::RecordBatch> >() > iter_maker = [vectorized_reader]() {
            arrow::Iterator<std::shared_ptr<arrow::RecordBatch> > batch_it = arrow::MakeIteratorFromReader(
                vectorized_reader);
            return batch_it;
        };
        if (state->vectorlized_parallel_execution == false) {
            arrow::acero::Declaration dec{
                ...
                arrow::acero::RecordBatchSourceNodeOptions{
                    state->arrow_input_schemas[_scan_tuple_id], std::move(iter_maker)
                }
            };
            ...
            arrow::acero::Declaration dec{
                "record_batch_source",
                arrow::acero::RecordBatchSourceNodeOptions{
                    state->arrow_input_schemas[_scan_tuple_id], std::move(iter_maker), _arrow_io_executor.get()
                }
            };
            LOCAL_TRACE_ARROW_PLAN_WITH_SCHEMA(dec, state->arrow_input_schemas[_scan_tuple_id], &_delay_fetcher_store);
            state->append_acero_declaration(dec);
        ...
        if (!_slot_order_exprs.empty()) {
            ...
            for (int i = 0; i < _slot_order_exprs.size(); ++i) {
                ...
                sort_keys.emplace_back(*field_ref, _is_asc[i]
    ...
    int SelectManagerNode::fetcher_store_run(RuntimeState *state, ExecNode *exec_node) {
        ...
        if (_conditions.size() > 0) {
            ...
            if (0 != VectorizeHelpper::init_conditions(state, _conditions, _vectorize_conditions, _scan_tuple_id,
                                                       nullptr)) {
            ...
            for (auto &pair: fetcher_store->start_key_sort) {
                ...
                if (iter != fetcher_store->region_batch.end()) {
                    ...
                    if (batch.arrow_data != nullptr && batch.arrow_data->num_rows() > 0) {
                        ...
                        _arrow_batch_response.
                                emplace_back(fetcher_store->batch_region_vectorized_response[pair.second]);
    ...
    int SelectManagerNode::open_global_index(FetcherInfo *fetcher, RuntimeState *state, ExecNode *exec_node,
        ...
        ExchangeSenderNode *exchange_sender_node = static_cast<ExchangeSenderNode *>(
            get_node(pb::EXCHANGE_SENDER_NODE));
    ...
    if (_children[0]->node_type() != pb::SCAN_NODE
        ...
        if (need_pushdown && limit != nullptr) {
            ret = construct_primary_possible_index(fetcher->fetcher_store, fetcher->scan_index, state, scan_node,
                                                   main_table_id, limit);
        } else {
            ret = construct_primary_possible_index(fetcher->fetcher_store, fetcher->scan_index, state, scan_node,
                                                   main_table_id);
    ...
    int SelectManagerNode::construct_primary_possible_index(
        ...
        if (fetcher_store.arrow_schema != nullptr) {
            return construct_primary_possible_index_vectorize(fetcher_store,
                                                              scan_index_info, state, exec_node, main_table_id,
                                                              pri_info, limit);
        }
    ...
    int SelectManagerNode::construct_primary_possible_index_vectorize(
        ...
        for (auto &pair: fetcher_store.start_key_sort) {
            ...
            if (row_batch != nullptr) {
                ...
                // chunk复用，内部会reset
                if (0 != row_batch->transfer_rowbatch_to_arrow({tuple}, chunk, nullptr,
            ...
            if (!fetcher_store.arrow_schema->Equals(fetcher_store.region_batch[pair.second].arrow_data->schema())
                && 0 != VectorizeHelpper::change_arrow_record_batch_schema(fetcher_store.arrow_schema,
                                                                           fetcher_store.region_batch[pair.second].
                                                                           arrow_data,
                                                                           &fetcher_store.region_batch[pair.second].
                                                                           arrow_data,
            ...
            std::shared_ptr<arrow::RecordBatch> data_without_partition = fetcher_store.region_batch[pair.second].
                    arrow_data;
        ...
        if (!_mem_row_compare->need_not_compare()) {
            ...
            arrow::Result<std::shared_ptr<arrow::Array> > indices = arrow::compute::SortIndices(
                datum, option, /*ExecContext* ctx = */ nullptr);
            if (!indices.ok()) {
                DB_FATAL("sort indices fail: %s", indices.status().ToString().c_str());
                return -1;
            }
            arrow::Result<arrow::Datum> sorted = arrow::compute::Take(table, *indices,
        ...
        for (auto row_idx = 0; row_idx < table->num_rows(); ++row_idx) {
            ...
            for (auto &field: field_id_to_arrow_array) {
                record->set_value(record->get_field_by_tag(field.first),
                                  VectorizeHelpper::get_vectorized_value(field.second.get(), row_idx));
            }
            auto range = pos_index.add_ranges();
            MutTableKey key;
            if (record->encode_key(*pri_info.get(), key, pri_info->fields.size(), false, false) != 0) {
            ...
            range->set_left_full(key.get_full());
            if (is_partition_table) {
                range->add_partition_ids(
                    VectorizeHelpper::get_vectorized_value(partition_id_array.get(), row_idx).get_numberic<int64_t>());
            }
        }
        //重新做路由选择
        pos_index.SerializeToString(&scan_index_info->raw_index);
    ...
    int SelectManagerNode::merge_fetcher_store_run(RuntimeState *state, ExecNode *exec_node) {
        ...
        auto remove_batch_same_row = [&
                ](std::shared_ptr<RowBatch> &batch, std::shared_ptr<RowBatch> uniq_batch) -> int {
            for (batch->reset(); !batch->is_traverse_over(); batch->next()) {
                std::unique_ptr<MemRow> &mem_row = batch->get_row();
                SmartRecord record = record_template->clone(false);
                for (auto &pri_field: pri_info->fields) {
