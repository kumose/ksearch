...
#include <ksearch/exec/transaction_manager_node.h>
...
namespace ksearch {
    ...
    public:
        ...
        virtual int open(RuntimeState *state) {
            ...
            if (ret < 0) {
                DB_WARNING("TransactionNote: prepare failed, rollback txn_id: %lu log_id:%lu", state->txn_id,
                           state->log_id());
                client_conn->seq_id++;
                ret = exec_rollback_node(state, rollback_node);
                if (ret < 0) {
                    // un-expected case since infinite retry of commit after prepare
                    DB_WARNING("TransactionError: rollback failed. txn_id: %lu log_id:%lu", state->txn_id,
                               state->log_id());
                }
