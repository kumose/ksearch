...
#include <ksearch/exec/exchange_receiver_node.h>
...
#include <ksearch/proto/db_interface.pb.h>
...
namespace ksearch {
    ...
    int ExchangeSenderNode::Channel::send_record_batch(RuntimeState *state,
        ...
        if (combine_cost + serialize_cost + send_cost > FLAGS_print_time_us) {
            DB_WARNING_CHANNEL(
                "combine: %ld, serialize: %ld, send: %ld, rows: %ld, attachment_size: %ld, local_pass_through: %d",
    ...
    int ExchangeSenderNode::Channel::send_record_batch_data(RuntimeState *state,
        ...
        if (!_is_local_pass_through) {
            ...
            data.append_user_data(const_cast<void *>(reinterpret_cast<const void *>(data_buffer->data())),
                                  data_buffer->size(), [](void *) {
                                  });
        } else {
    ...
    int ExchangeSenderNode::Channel::send_version_old(
        const bool is_merge, const int64_t region_id,
        ...
        if (_exchange_receiver_node != nullptr && es_type == pb::HashPartitionType) {
            ...
            if (!receiver_property->hash_partition_propertys.empty()) {
                ...
                for (auto &name: hash_property->need_add_projection_columns) {
                    ...
                    ExprNode::create_pb_expr(es_node->mutable_partition_property()->add_need_project_hash_exprs(),
                                             expr);
                }
    ...
    int ExchangeSenderNode::open(RuntimeState *state) {
        ...
        for (int i = 0; i < _receiver_destinations.size(); ++i) {
            ...
            ->
            localhost_address == _receiver_destinations[i].address()
            )
            {
    ...
    int ExchangeSenderNode::build_arrow_declaration(RuntimeState *state) {
        ...
        if (_partition_property.hash_partition_propertys.size() > 0
            ...
            arrow::acero::Declaration dec{
                "project", arrow::acero::ProjectNodeOptions{generate_projection_exprs, generate_projection_exprs_names}
            };
    ...
    int ExchangeSenderNode::send_record_batch(RuntimeState *state,
        ...
        if (_partition_property.type == pb::SinglePartitionType) {
            ...
            std::vector<std::shared_ptr<arrow::RecordBatch> > need_send_record_batches;
            bool need_send_rpc = false;
            if (0 != _broadcast_record_batch_keepper.add_record_batch(state, record_batch, _is_db_fragment,
                                                                      need_send_record_batches, exchange_state,
                                                                      need_send_rpc)) {
            ...
            // 发送全部上游
            for (auto &channel: _channels) {
                bool is_not_target_channel_for_random_type = (
                    _partition_property.type == pb::RandomPartitionType && _target_channel_idx_for_random_type !=
                    channel_idx);
                ...
                if (0 != channel->send_record_batch_data(state,
                                                         need_send_record_batches,
                                                         schema_buffer,
                                                         is_not_target_channel_for_random_type
                                                             ? _empty_recordbatch_buffer
                                                             : data_buffer,
                                                         exchange_state,
                                                         is_send_query_stat,
                                                         send_rows)) {
            ...
            if (combine_cost + serialize_cost + send_cost > FLAGS_print_time_us) {
                DB_WARNING_ES(
                    "combine cost: %ld, serialize cost: %ld, send cost: %ld, send rows: %ld, single attachment size: %ld",
    ...
    arrow::Status ExchangeSenderNode::repartition(
        ...
        for (int i = 0; i < hash_indices.size(); ++i) {
            if (need_cast_indices[i]) {
                ...
                ARROW_ASSIGN_OR_RAISE(cast_array_map[hash_indices[i]],
                                      arrow::compute::Cast(*array, arrow::large_binary(), cast_options, &exec_ctx));
        ...
        ARROW_RETURN_NOT_OK(stack.Init(GetMemoryPoolForRead(), 8 * max_batch_size * sizeof(uint64_t)));
