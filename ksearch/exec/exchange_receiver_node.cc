...
namespace ksearch {
    ...
    int DataStreamReceiver::handle_version_old(const pb::TransmitDataParam &param) {
        ...
        if (param.region_infos().size() >= 2) {
            ...
            if (!(r.start_key() <= region_info.start_key() && end_key_compare(r.end_key(), region_info.end_key()) >=
                  0)) {
    ...
    int DataStreamReceiver::add_local_pass_through_chunk(uint64_t sender_fragment_instance_id,
        ...
        for (auto &record_batch: filtered_record_batchs) {
            if (record_batch != nullptr) {
                DB_DEBUG(
                    "local pass through record batch num rows: %ld, _fragment_instance_id: %lu, sender_fragment_instance_id: %lu",
    ...
    int ExchangeReceiverNode::build_arrow_declaration(RuntimeState *state) {
        ...
        arrow::acero::Declaration dec{
            ...
            arrow::acero::RecordBatchSourceNodeOptions{
                reader->schema(), std::move(iter_maker), _arrow_io_executor.get()
            }
        };
