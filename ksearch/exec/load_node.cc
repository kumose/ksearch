...
namespace ksearch {
    ...
    int LoadNode::init(const pb::PlanNode &node) {
        ...
        DB_WARNING("load data path:%s size:%ld char_set:%s", _data_path.c_str(), _file_size,
                   pb::Charset_Name(_char_set).c_str());
        return 0;
    }
        ...
        while (!_read_eof) {
            ...
            while (!f.eof()) {
                ...
                if (f.eof()) {
                    ...
                    if (!_has_get_line) {
                        DB_FATAL("path: %s, line_size: %lu > buf_size: %ld", _data_path.c_str(), line.size(),
                                 BUFFER_SIZE);
                        return -1;
                    }
                    if (_file_cur_pos + line.size() == _file_size) {
                        _read_eof = true;
                        DB_WARNING("path: %s, eof, pos: %ld line_size:%ld", _data_path.c_str(), _file_cur_pos,
                                   line.size());
                    }
                    break;
                }
