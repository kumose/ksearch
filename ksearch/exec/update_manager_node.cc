...
namespace ksearch {
    ...
    int UpdateManagerNode::init_update_info(UpdateNode *update_node) {
        ...
        global_affected_indices.insert(global_affected_indices.end(), g_affected_unique_indexs.begin(),
                                       g_affected_unique_indexs.end());
        global_affected_indices.insert(global_affected_indices.end(), g_affected_non_unique_indexs.begin(),
                                       g_affected_non_unique_indexs.end());

        // 如果更新主键或者常规ttl，那么影响了全部索引
        if (_affect_primary || (_table_info->ttl_info.ttl_duration_s > 0 && ttl_field_ptr == nullptr)) {
            _global_affected_index_ids.insert(_global_affected_index_ids.end(), g_unique_indexs.begin(),
                                              g_unique_indexs.end());
            _global_affected_index_ids.insert(_global_affected_index_ids.end(), g_non_unique_indexs.begin(),
                                              g_non_unique_indexs.end());
        } else {
