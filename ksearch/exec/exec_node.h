...
#include <ksearch/exec/mpp_property.h>
...
namespace ksearch {
     ...
     */
        ...
        void add_filter_node_as_child(const std::vector<ExprNode *> &input_exprs,
                                      pb::PlanNodeType type = pb::TABLE_FILTER_NODE);
    ...
    */
        ...
        find_explain_record_by_id(std::vector<std::map<std::string, std::string> > &output, int id) {
            return std::find_if(output.begin(), output.end(),
                                [id](const std::map<std::string, std::string> &explain_map) {
        ...
        bool shrink_partition_property(std::shared_ptr<HashPartitionColumns> hash_columns,
                                       NodePartitionProperty *partition_property);
