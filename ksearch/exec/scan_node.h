...
namespace ksearch {
    ...
    public:
        ...
        void serialize_index_and_set_router_index(const pb::PossibleIndex &pos_index, pb::PossibleIndex *router_index,
                                                  bool covering_index,
        ...
        int64_t select_join_index_in_ksearch(const std::string &sample_sql);

        int64_t select_index_in_ksearch(const std::string &sample_sql);

        virtual int show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
        ...
        void calc_join_index_range() {
            ...
            if (!_join_path.path(_select_index_for_join)->index_info_ptr->is_global && _select_index_for_join !=
                _table_id) {
