...
namespace ksearch {
    ...
    int AggNode::build_arrow_declaration(RuntimeState *state) {
        ...
        for (auto &group_field: _group_exprs) {
            if (0 != vectorize_build_group_by_exprs(state, group_field, group_by_fields, key_field_names,
                                                    generate_projection_exprs, generate_projection_exprs_names)) {
                return -1;
            }
        }
        // 特殊处理multi distinct
        ...
        if (_node_type == pb::AGG_NODE
            || (_node_type == pb::MERGE_AGG_NODE && _parent->node_type() == pb::MERGE_AGG_NODE)
            || is_pushdown_merge_agg) {
            // multi distinct场景，聚合下推生成的merge agg node也需要处理
            transfer_multi_distinct_to_group_by = true;
        }
        for (int i = 0; i < _agg_fn_calls.size(); ++i) {
            if (transfer_multi_distinct_to_group_by && _agg_fn_calls[i]->is_multi_distinct_agg()) {
                for (auto &c: _agg_fn_calls[i]->children()) {
                    if (0 != vectorize_build_group_by_exprs(state, c, group_by_fields, key_field_names,
        ...
        if (need_add_projections) {
            for (int i = 0; i < _agg_fn_calls.size(); ++i) {
                _agg_fn_calls[i]->add_agg_projection_slot_ref(_is_merger, generate_projection_exprs,
                                                              generate_projection_exprs_names);
            }
            arrow::acero::Declaration dec{
                "project",
                arrow::acero::ProjectNodeOptions{
                    std::move(generate_projection_exprs), std::move(generate_projection_exprs_names)
                }
            };
        ...
        if (_group_exprs.size() == 0) {
            ...
            if (packet != nullptr) {
                ...
                arrow::acero::Declaration dec{
                    "make_default_agg_row_when_no_input",
    ...
    int AggNode::open(RuntimeState *state) {
        ...
        if (_hash_map.size() == 0 && _group_exprs.size() == 0) {
            ExecNode *packet = get_parent_node(pb::PACKET_NODE);
            // ksearch才有packet_node;只在ksearch上产生数据
