...
namespace ksearch {
    ...
    int DualScanNode::init(const pb::PlanNode &node) {
        ...
        if (node.derive_node().dual_scan_node().has_sub_query_node()) {
            _has_subquery = true;
            ret = create_tree(node.derive_node().dual_scan_node().sub_query_node(), &_sub_query_node,
                              CreateExecOptions());
            if (ret < 0) {
            ...
            for (derived_batch.reset(); !derived_batch.is_traverse_over(); derived_batch.next()) {
                ...
                if (_agg_tuple_id_pair.first != -1) {
                    ...
                    for (auto slot_id: _agg_slot_ids) {
                        outer_row->set_value(agg_tuple_id, slot_id,
                                             inner_row->get_value(subquery_agg_tuple_id, slot_id));
                    }
                }
            ...
            if (can_predicate_pushdown()) {
                ...
                while (iter != input_exprs.end()) {
                    ...
                    if (can_expr_predicate_pushdown(subquery_has_agg, subquery_has_window,
                                                    group_by_slots, partition_by_slots, *iter,
                                                    can_expr_pushdown) != 0) {
                        return -1;
                    }
    ...
    int DualScanNode::transfer_outer_slot_to_inner(ExprNode **input_expr) {
        if (can_predicate_pushdown()) {
            ...
            if (can_expr_pushdown) {
                const int32_t tuple_id = slot_tuple_id();
                return (*input_expr)->replace_slot_ref_to_expr(tuple_id, _slot_column_mapping,
                                                               _derived_table_projections, *input_expr);
            }
        }
        // 没法下推   
        return -1;
    ...
    int DualScanNode::can_expr_predicate_pushdown(
        ...
        if (tuple_ids.size() == 1 && *tuple_ids.begin() == tuple_id) {
            ...
            expr->get_all_subquery_tuple_slot_ids(tuple_id, _slot_column_mapping, _derived_table_projections,
                                                  tuple_slot_ids);
            if (subquery_has_agg) {
    ...
    int DualScanNode::set_sub_query_node_partition_property(NodePartitionProperty *outer_property) {
        ...
        if (outer_property->type == pb::HashPartitionType && sub_query_property->type == pb::AnyType) {
            if (outer_property->hash_partition_propertys.size() != 1) {
                DB_FATAL("outter partition property invalid size: %ld",
                         outer_property->hash_partition_propertys.size());
                return -1;
            }
            ...
            for (auto &name: outer_property->hash_partition_propertys[0]->ordered_hash_columns) {
                ...
                if (expr->is_slot_ref() && expr->tuple_id() == tuple_id) {
                    ...
                    if (_slot_column_mapping.find(outer_slot_id) == _slot_column_mapping.end()) {
                        DB_WARNING("Fail to get slot column, outer_slot_id: %d, tuple_id: %d", outer_slot_id,
                                   expr->tuple_id());
                        return -1;
                    }
    ...
    int DualScanNode::build_arrow_declaration(RuntimeState *state) {
        ...
        for (auto iter: _slot_column_mapping) {
            ...
            if (is_union_subquery()) {
                ...
                if (0 != build_arrow_expr_with_cast(_derived_table_projections[inter_column_id], slot.slot_type(),
                                                    true)) {
        ...
        if (_agg_tuple_id_pair.first != -1) {
            ...
            pb::TupleDescriptor *subquery_agg_tuple_desc = _sub_query_runtime_state->get_tuple_desc(
                subquery_agg_tuple_id);
            ...
            for (auto slot_id: _agg_slot_ids) {
                ...
                arrow::compute::Expression expr =
                        arrow::compute::field_ref(
                            std::to_string(subquery_agg_tuple_id) + "_" + std::to_string(slot_id));
                if (is_union_subquery()) {
        ...
        if (no_input_node) {
            ...
            if (state->vectorlized_parallel_execution) {
                ...
                scan_dec = arrow::acero::Declaration{
                    "record_batch_source",
                    arrow::acero::RecordBatchSourceNodeOptions{
                        vectorized_reader->schema(), std::move(iter_maker), _arrow_io_executor.get()
                    }
                };
            } else {
                scan_dec = arrow::acero::Declaration{
    ...
    int DualScanNode::child_show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
    ...
    int DualScanNode::show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
                                   int &next_id, int display_id) {
