...
#include <ksearch/exec/fetcher_store.h>

#include <brpc/channel.h>
#include <brpc/selective_channel.h>
#include <gflags/gflags.h>
#include <ksearch/session/binlog_context.h>
#include <ksearch/logical_plan/query_context.h>
#include <ksearch/exec/dml_node.h>
#include <ksearch/exec/scan_node.h>
#include <ksearch/runtime/trace_state.h>
#include <ksearch/exec/rocksdb_scan_node.h>
#include <arrow/util/byte_size.h>
#include <ksearch/exec/filter_node.h>
#include <ksearch/exec/vectorize_helpper.h>
...
namespace ksearch {
    ...
    DEFINE_int64(ksearch_alive_time_s, 10 * 60, "obervation time length in ksearch, default:10 min");
    ...
    OnRPCDone::OnRPCDone(FetcherStore *fetcher_store, RuntimeState *state,
                         ...
                         pb::OpType op_type, bool need_check_memory) : _store_request(store_request),
                                                                       _fetcher_store(fetcher_store),
                                                                       _state(state),
                                                                       _start_seq_id(start_seq_id),
                                                                       _current_seq_id(current_seq_id),
                                                                       _op_type(op_type),
    ...
    ErrorType OnRPCDone::fill_single_request(pb::StoreReq &single_req, pb::RegionInfo &region_info, int64_t region_id,
        ...
        if (_start_seq_id >= 0 && _op_type != pb::OP_COMMIT) {
            for (auto &pair: _client_conn->cache_plans) {
                ...
                if (plan_item.op_type != pb::OP_BEGIN && !_state->single_txn_cached()) {
                    DB_DONE(WARNING, "TranstationNote: copy cache, cache_plan:%s",
                ...
                if (_fetcher_store->shared_plan == nullptr) {
                    ...
                    if (_need_check_memory) {
                        ...
                        if (shared_request_size > 0
                            && 0 != _state->
                            memory_limit_exceeded(std::numeric_limits<int>::max(), shared_request_size)) {
                            _fetcher_store->error = E_BIG_SQL;
                            DB_WARNING_STATE(_state, "Memory limit exceeded when add shared plan size: %ld.",
                                             shared_request_size);
                            return E_BIG_SQL;
                        }
                    }
    ...
    void OnRPCDone::select_addr(pb::RegionInfo &info,
        ...
        if (!insulate_resource_tag.empty() && _op_type == pb::OP_SELECT) {
            return select_resource_insulate_read_addr(info, addr,
                                                      insulate_resource_tag, select_without_leader,
                                                      resource_insulate_read);
        }
        ...
        if (_op_type == pb::OP_SELECT && _state->txn_id == 0 && info.learners_size() > 0 &&
            ...
            if (info.learners_size() > 0) {
                ...
                FetcherStore::choose_opt_instance(info.region_id(), info.learners(), "", _backup, backup_status,
                                                  nullptr);
                ...
                if (_retry_times == 0) {
                    ...
                    FetcherStore::choose_opt_instance(info.region_id(), info.peers(), FLAGS_fetcher_resource_tag, addr,
                                                      addr_status, &_backup, {}, read_random_select_peer);
                }
            }
            select_without_leader = true;
        } else if (_retry_times == 0) {
    ...
    void OnRPCDone::Run(const std::string &version) {
        ...
        if (_is_batch) {
            ...
            if ((query_cost > FLAGS_print_time_us || _retry_times > 0) && _batch_response_ptr->store_res_size() > 0) {
                DB_DONE(WARNING,
                        "batch version:%s time:%ld rpc_time:%ld ip:%s, dynamic_timeout_ms:%ld vectorize:[%d, %d]",
                        version.substr(0, 100).c_str(), _total_cost.get_time(), query_cost, remote_side.c_str(),
                        _fetcher_store->dynamic_timeout_ms, _batch_request.store_req(0).execute_type(),
                        _batch_response_ptr->store_res(0).execute_type());
            }
        } else {
    ...
    ErrorType OnRPCDone::handle_single_response(const std::string &remote_side,
        ...
        if (single_response.errcode() == pb::RETRY_LATER) {
            DB_DONE(WARNING, "request failed, errcode: %s, errmsg: %s",
                    pb::ErrCode_Name(single_response.errcode()).c_str(), single_response.errmsg().c_str());
        ...
        if (single_response.errcode() == pb::REGION_NOT_EXIST || single_response.errcode() == pb::INTERNAL_ERROR) {
            DB_DONE(WARNING, "new_leader:%s, errcode: %s", single_response.leader().c_str(),
                    pb::ErrCode_Name(single_response.errcode()).c_str());
        ...
        if (single_response.errcode() != pb::SUCCESS) {
            ...
            DB_DONE(WARNING, "errcode:%s, mysql_errcode:%d, msg:%s, failed",
                    pb::ErrCode_Name(single_response.errcode()).c_str(), single_response.mysql_errcode(),
                    single_response.errmsg().c_str());
        ...
        if (single_response.records_size() > 0) {
            ...
            }
            {
                ...
                if (!return_str_old_records.empty()) {
                    std::vector<std::string> &str_old_records = _fetcher_store->return_str_old_records[info.
                        partition_id()];
                    str_old_records.insert(str_old_records.end(), return_str_old_records.begin(),
        ...
        if (_op_type != pb::OP_SELECT && _op_type != pb::OP_SELECT_FOR_UPDATE && _op_type != pb::OP_ROLLBACK && _op_type
            != pb::OP_COMMIT) {
        ...
        if (!single_response.leader().empty() && single_response.leader() != "0.0.0.0:0" && single_response.leader() !=
            info.leader()) {
            info.set_leader(single_response.leader());
            schema_factory->update_leader(info);
            if (_state->txn_id != 0) {
        ...
        if (single_response.execute_type() == pb::EXEC_ROW) {
            ...
            bool global_ddl_with_ttl = (single_response.row_values_size() > 0 && single_response.row_values_size() ==
                                        single_response.ttl_timestamp_size())
                                           ? true
                                           : false;
            int ttl_idx = 0;
            ...
            for (auto &pb_row: single_response.row_values()) {
                if (pb_row.tuple_values_size() != single_response.tuple_ids_size()) {
                    ...
                    SQL_TRACE("backup_request size diff, tuple_values_size:%d tuple_ids_size:%d rows:%d",
                              pb_row.tuple_values_size(), single_response.tuple_ids_size(),
                              single_response.row_values_size());
            ...
            if (vector_rows.size() > 0) {
                ...
                std::shared_ptr<arrow::Buffer> schema_buffer = std::make_shared<arrow::Buffer>(
                    reinterpret_cast<const uint8_t *>(vector_schema.data()),
                ...
                std::shared_ptr<arrow::Buffer> buffer = std::make_shared<arrow::Buffer>(
                    reinterpret_cast<const uint8_t *>(vector_rows.data()),
                    static_cast<int64_t>(vector_rows.size()));
                arrow::io::BufferReader buf_reader(buffer);
                auto ret = arrow::ipc::ReadRecordBatch(schema, nullptr, arrow::ipc::IpcReadOptions::Defaults(),
                                                       &buf_reader);
                if (ret.ok()) {
                    batch = *ret;
                ...
                if (_fetcher_store->vectorize_conditions != nullptr) {
                    ...
                    if (0 != VectorizeHelpper::vectorize_filter(batch, _fetcher_store->vectorize_conditions,
                                                                &filtered_batch, &need_keep_response)) {
        ...
        if (_trace_node != nullptr) {
            std::string desc = "ksearch FetcherStore send_request "
        ...
        if (cost.get_time() > FLAGS_print_time_us) {
            if (_fetcher_store->vectorize_conditions == nullptr) {
                ...
                DB_DONE(WARNING, "parse time:%ld rows:%lu,rows_after_filter: %lu", cost.get_time(), returned_row_size,
                        row_size_after_filter);
            }
        }
        return E_OK;
    }
    ...
    OnSingleRPCDone::OnSingleRPCDone(FetcherStore *fetcher_store, RuntimeState *state, ExecNode *store_request,
                                     pb::RegionInfo *info_ptr,
                                     int64_t old_region_id, int64_t region_id, int start_seq_id, int current_seq_id,
                                     pb::OpType op_type, bool need_check_memory) : OnRPCDone(fetcher_store, state,
            store_request, start_seq_id, current_seq_id, op_type, need_check_memory),
        _info(*info_ptr) {
        _old_region_id = old_region_id;
        _region_id = region_id;
        std::string resource_tag = FLAGS_insulate_fetcher_resource_tag.empty()
        ...
        _meta_id = ::ksearch::get_meta_id(_info.table_id());
    ...
    void OnSingleRPCDone::set_region_vectorized_response(int64_t region_id,
                                                         std::shared_ptr<pb::StoreRes> single_response_ptr) {
    ...
    OnBatchRPCDone::OnBatchRPCDone(FetcherStore *fetcher_store,
                                   ...
                                   bool need_check_memory) : OnRPCDone(fetcher_store, state, store_request,
                                                                       start_seq_id, current_seq_id, op_type,
                                                                       need_check_memory),
                                                             _infos(infos),
                                                             _limit_single_store_concurrency_cnts(
                                                                 limit_single_store_concurrency_cnts) {
        for (auto &info: infos) {
    ...
    OnBatchRPCDone::OnBatchRPCDone(FetcherStore *fetcher_store,
        ...
        if (_infos.size() > 0) {
            _meta_id = ::ksearch::get_meta_id(_infos[0]->table_id());
        }
            ...
            if (_fetcher_store->shared_plan == nullptr) {
                ...
                if (_need_check_memory) {
                    ...
                    if (shared_request_size > 0
                        ...
                        _fetcher_store->error = E_BIG_SQL;
                        DB_WARNING_STATE(_state, "Memory limit exceeded when add shared batch plan size: %ld.",
                                         shared_request_size);
                        return E_BIG_SQL;
                    }
                }
    ...
    ErrorType OnBatchRPCDone::handle_response(const std::string &remote_side,
                                              std::vector<OnSingleRPCDone *> &need_retry_tasks) {
        auto &batch_response = *_batch_response_ptr;
        for (int i = 0; i < batch_response.store_res_size(); ++i) {
            std::shared_ptr<pb::StoreRes> smart_single_response_ptr(batch_response.mutable_store_res(i),
                                                                    [](pb::StoreRes *ptr) {
                                                                    });
            ...
            if (ret == E_RETRY) {
                need_retry_tasks.push_back(new OnSingleRPCDone(_fetcher_store, _state, _store_request, &info,
                                                               region_id, info.region_id(), _start_seq_id,
                                                               _current_seq_id, _op_type, _need_check_memory));
            }
    ...
    void OnBatchRPCDone::set_region_vectorized_response(int64_t region_id,
                                                        std::shared_ptr<pb::StoreRes> single_response_ptr) {
        _fetcher_store->region_vectorized_response[region_id] = single_response_ptr;
    ...
    void OnBatchRPCDone::retry_region_task(const std::string &remote_side) {
        ...
        for (auto &info_ptr: _infos) {
            _rpc_ctrl->add_new_task(new OnSingleRPCDone(_fetcher_store, _state, _store_request, info_ptr,
                                                        info_ptr->region_id(), info_ptr->region_id(), _start_seq_id,
                                                        _current_seq_id,
                                                        _op_type, _need_check_memory));
        }
        _rpc_ctrl->task_finish(this);
        ...
        if (!state->force_single_rpc
            ...
            for (auto info: infos) {
                if (!broadcast_all_peer_without_raft) {
                    ...
                    for (auto peer: info->peers()) {
                        auto task = new OnSingleRPCDone(this, state, store_request, info,
                                                        info->region_id(), info->region_id(), start_seq_id,
                                                        current_seq_id,
                                                        op_type, need_check_memory);
                        task->_addr = peer;
                        rpc_ctrl.add_new_task(task);
    ...
    int FetcherStore::run_not_set_state(RuntimeState *state,
        ...
        if ((op_type == pb::OP_COMMIT || op_type == pb::OP_ROLLBACK)
            ...
            do {
                ...
                if (error != E_OK) {
                    DB_FATAL("send optype:%s to region_id:%ld txn_id:%lu failed, log_id:%lu ",
                             pb::OpType_Name(op_type).c_str(),
