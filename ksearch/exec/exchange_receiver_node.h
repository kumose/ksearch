...
#define DB_FATAL_ER(_fmt_, args...) \
    ...
    public:
        ...
        std::unordered_map<int64_t, std::queue<std::shared_ptr<arrow::RecordBatch> > >
        _local_pass_through_record_batchs;
    ...
    public:
        ...
        std::string receiver_key() const {
            return std::to_string(_log_id) + "_" + std::to_string(_fragment_instance_id) + "_" +
                   std::to_string(_node_id);
        }
