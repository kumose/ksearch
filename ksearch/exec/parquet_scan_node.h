...
namespace ksearch {
    ...
    private:
        ...
        int get_qualified_record_batch_readers(
            std::vector<std::shared_ptr<::arrow::RecordBatchReader> > &parquet_file_readers);

        void get_qualified_parquet_file_readers(std::vector<std::shared_ptr<ParquetFileReader> > &parquet_file_readers,
                                                const std::vector<std::shared_ptr<ParquetFile> > &parquet_files,
    ...
    private:
        ...
        // key: ksearch column name, value: parquet column name
        std::unordered_map<std::string, std::string> _column_name_map;
        // key: ksearch column name, value: ksearch column type
