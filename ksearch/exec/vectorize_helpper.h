    ...
    public:
            ...
            for (auto &f: schema->fields()) {
                ...
                if (array == nullptr) {
                    ...
                    if (array->type()->id() != f->type()->id()) {
                        ...
                        auto cast_array_ret = ::arrow::compute::Cast(*array, f->type(),
                                                                     arrow::compute::CastOptions::Unsafe(f->type()),
                                                                     &exec_ctx);
        ...
        static int change_arrow_record_batch_schema(
            ...
            for (const auto &f: schema->fields()) {
                ...
                pb::PrimitiveType to_ksearch_type = pb::INVALID_TYPE;
                if (column_type_map.find(f->name()) != column_type_map.end()) {
                    to_ksearch_type = column_type_map.at(f->name());
                }
                if (array == nullptr) {
                    // 兼容加列场景db/store心跳不一致
                    auto null_array = arrow::MakeArrayOfNull(f->type(), in->num_rows());
                    ...
                    if (array->type()->id() != f->type()->id()) {
                        auto cast_array_ret = Cast(array, array->type(), f->type(), to_ksearch_type);
                        if (!cast_array_ret.ok()) {
                            DB_WARNING("cast array fail, %s", cast_array_ret.status().ToString().c_str());
                            return -1;
                        }
        ...
        static int vectorize_filter(std::shared_ptr<arrow::RecordBatch> record_batch,
            ...
            arrow::Result<arrow::Datum> filter_mask = arrow::compute::ExecuteScalarExpression(
                *conjuncts, exec_batch, /*ExecContext* = */nullptr);
            if (!filter_mask.ok()) {
        ...
        static int concatenate_record_batches(
            ...
            arrow::Result<std::shared_ptr<arrow::Table> > build_table =
        ...
        static std::shared_ptr<arrow::RecordBatch> construct_arrow_record_batch(
            ...
            for (const auto &field: schema->fields()) {
                ...
                auto cast_array_ret = ::arrow::compute::Cast(*array, field->type(),
                                                             arrow::compute::CastOptions::Unsafe(field->type()));
                if (!cast_array_ret.ok()) {
        ...
        static std::shared_ptr<arrow::Schema> construct_arrow_schema(
            ...
            for (const auto &field: table_info->fields) {
                ...
                std::shared_ptr<arrow::Field> arrow_field = construct_arrow_field(
                    name, field.type, arrow_field_metadata);
        ...
        static std::shared_ptr<arrow::Array> make_array_from_str(
            ...
            auto array_ret = arrow::MakeArrayFromScalar(arrow::LargeBinaryScalar(arrow::Buffer::FromString(str)),
                                                        length);
            if (!array_ret.ok()) {
                DB_WARNING("arrow make array from scalar fail, %s", array_ret.status().ToString().c_str());
                return nullptr;
            ...
            std::shared_ptr<arrow::Array> array = *array_ret;
            auto cast_array_ret = ::arrow::compute::Cast(*array, type, arrow::compute::CastOptions::Unsafe(type),
                                                         exec_ctx);
            if (!cast_array_ret.ok()) {
                DB_WARNING("arrow cast array fail, %s", cast_array_ret.status().message().c_str());
                return nullptr;
        ...
        static arrow::Result<std::shared_ptr<arrow::Array> > Cast(
            ...
            const pb::PrimitiveType to_ksearch_type = pb::INVALID_TYPE) {
