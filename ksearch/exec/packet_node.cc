...
#include <ksearch/exec/packet_node.h>
#include <ksearch/exec/full_export_node.h>
...
namespace ksearch {
    ...
    int PacketNode::show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
                                 int &next_id, int display_id) {
    ...
    void PacketNode::pack_trace2(std::vector<std::map<std::string, std::string> > &info,
                                 const pb::TraceNode &trace_node,
                                 int64_t &total_scan_rows, int64_t &total_index_filter, int64_t &total_get_primary,
                                 int64_t &total_where_filter) {
    ...
    int PacketNode::sample_keypoint(RuntimeState *state) {
        ...
        if (partition_key_pks.size() < pre_split_region_cnt) {
            DB_WARNING("partition key pks size:%lu < pre split region count:%d", partition_key_pks.size(),
                       pre_split_region_cnt);
            return -1;
        }
        ...
        int ret = DDLPlanner::update_specify_split_keys(table_ptr->namespace_, split_vec[0], split_vec[1],
                                                        boost::join(pre_split_keys, ","));
        if (ret < 0) {
    ...
    int PacketNode::start_vectorized_execution(RuntimeState *state) {
        ...
        if (final_table.ok()) {
            ...
            DB_FATAL_STATE(state, "arrow execute fail: arrow acero run fail, status: %s",
                           final_table.status().ToString().c_str());
            return -1;
        }
    ...
    int PacketNode::open(RuntimeState *state) {
        ...
        if (state->explain_type == ANALYZE_STATISTICS) {
            ...
            if (scan_nodes.size() != 1) {
                DB_WARNING(
                    "analyze packet node should have one and only one scan node, but now %ld scan node(s) are found.",
                    scan_nodes.size());
                return -1;
            }
            ...
            if (state->max_sample_regions > 0) {
                ...
                main_scan_index->region_infos = sample_analyze_regions(main_scan_index->region_infos,
                                                                       state->max_sample_regions);
            }
        }
        ...
        if (_is_explain &&state
        
        ->
        explain_type == EXPLAIN_NULL
        )
        {
    ...
    int PacketNode::pack_ok(int num_affected_rows, NetworkSocket *client) {
        ...
        int64_t last_insert_id = (op_type() == pb::OP_INSERT || op_type() == pb::OP_UPDATE)
    ...
    int PacketNode::pack_text_row(MemRow *row, RuntimeState *state,
                                  std::vector<std::shared_ptr<arrow::ChunkedArray> > *columns, int row_idx) {
        int start_pos = _send_buf->_size;
        uint8_t bytes[4];
        bytes[0] = '\x01';
        bytes[1] = '\x00';
        ...
        for (auto expr: _projections) {
            ...
            if (row != nullptr) {
                ...
                // TODO, 减少一次拷贝
                expr_value = VectorizeHelpper::get_vectorized_value((*columns)[field_idx].get(), row_idx,
                                                                    expr->float_precision_len()).cast_to(
                    expr->col_type());
            }
    ...
    int PacketNode::pack_binary_row(MemRow *row, RuntimeState *state,
                                    std::vector<std::shared_ptr<arrow::ChunkedArray> > *columns, int row_idx) {
        int start_pos = _send_buf->_size;
        uint8_t bytes[4];
        bytes[0] = '\x01';
                    ...
                    if (!_projections[i]->has_agg() && !_projections[i]->has_window()) {
                        ...
                        for (const auto &[tuple_id, slot_id]: tuple_slot_ids) {
                            ...
                            if (slot_id - 1 < 0 || slot_id - 1 >= tuple->slot_idxes_size()) {
                                DB_WARNING("Invalid slot_id: %d, tuple slot_idxes size: %d", slot_id,
                                           tuple->slot_idxes_size());
                                return -1;
                            }
            ...
            for (auto tuple_id: change_tuple_ids) {
                ...
                for (int i = 0; i < tuple->slots_size(); ++i) {
                    ...
                    if (slot.slot_id() - 1 < 0 || slot.slot_id() - 1 >= tuple->slot_idxes_size()) {
                        DB_WARNING("Invalid slot id: %d, tuple slot idx size: %d", slot.slot_id(),
                                   tuple->slot_idxes_size());
                        return -1;
                    }
