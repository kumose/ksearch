...
namespace ksearch {
    ...
    void ParquetScanNode::get_qualified_parquet_file_readers(
    ...
    int ParquetScanNode::get_qualified_record_batch_readers(
        std::vector<std::shared_ptr<::arrow::RecordBatchReader> > &record_batch_readers) {
        ...
        if (cumulatives_file_count > 0 && has_put_or_delete) {
            ...
            std::shared_ptr<ColumnSchemaInfo> schema_info = ColumnRecord::make_column_schema(
                _table_info->id, _table_info, _pri_info, field_id2info_map);
            ...
            get_qualified_parquet_file_readers(parquet_file_readers, parquet_files,
                                               schema_info->schema_with_order_info);
    ...
    int ParquetVectorizedReader::init(RuntimeState *state, ParquetScanNode *parquet_scan_node) {
        ...
        for (const auto &slot: tuple_desc->slots()) {
            ...
            // 建立tupleid_slotid到列名的映射，用于后面parquet_schema转化成ksearch schema
    ...
    arrow::Status ParquetVectorizedReader::ReadNext(std::shared_ptr<arrow::RecordBatch> *out) {
        ...
        if (_record_batch == nullptr) {
            ...
            while (_reader_iter != _parquet_file_readers->end()) {
                ...
                DB_DEBUG("file ReadNext cost: %ld, num rows: %ld, num columns: %d", /*_reader_iter->first.c_str(), */
                         cost.get_time(),
            ...
            DB_DEBUG("change arrow record batch schema in: %ld, %d out: %ld, %d",
                     record_batch->num_rows(), record_batch->num_columns(), _record_batch->num_rows(),
                     _record_batch->num_columns());
        }
        if (_record_batch == nullptr) {
