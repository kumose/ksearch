 ...
 */
    arrow::Result<arrow::acero::ExecNode *> MakeDefaultAggRowWhenNoInputNode::Make(arrow::acero::ExecPlan *plan,
        ...
        auto schema = inputs[0]->output_schema();
        return plan->EmplaceNode<MakeDefaultAggRowWhenNoInputNode>(plan, std::move(inputs), std::move(schema),
                                                                   opt.count_names);
    }

    arrow::Result<arrow::compute::ExecBatch> MakeDefaultAggRowWhenNoInputNode::ProcessBatch(
    ...
    int MakeDefaultAggRowWhenNoInputNode::Finish(int &total_output_batch_cnt) {
        ...
        for (int i = 0; i < output_schema_->num_fields(); ++i) {
            ...
            if (_count_columns.count(f->name()) > 0) {
                ...
                if (!zero_value.ok()) {
                    DB_FATAL("MakeDefaultAggRowWhenNoInputNode MakeNullScalar fail, %s",
                             zero_value.status().ToString().c_str());
                    return -1;
                }
 ...
 */
    ...
    arrow::Result<arrow::compute::ExecBatch> DebugPrintNode::ProcessBatch(arrow::compute::ExecBatch batch) {
        std::lock_guard<bthread::Mutex> lock(_mutex);
        _passed_row_cnt += batch.length;
        DB_WARNING("DebugPrintNode: %p got batch, batch.length: %ld, _passed_row_cnt: %ld, ", this, batch.length,
                   _passed_row_cnt);
        DB_WARNING("DebugPrintNode: %p got batch, schema: %s, data: %s", this, output_schema_->ToString().c_str(),
