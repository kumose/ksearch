...
#define DB_DONE(level, _fmt_, args...) \
    ...
    public:
        ...
        ErrorType fill_single_request(pb::StoreReq &single_req, pb::RegionInfo &region_info, int64_t region_id,
                                      int64_t old_region_id);

        ErrorType send_async();

        virtual void set_region_vectorized_response(int64_t region_id,
                                                    std::shared_ptr<pb::StoreRes> single_response_ptr) = 0;
    ...
    public:
        OnSingleRPCDone(FetcherStore *fetcher_store, RuntimeState *state, ExecNode *store_request,
        ...
        void pick_addr_for_resource_tag(const std::string &resource_tag, std::string &addr) {
            std::string ksearch_logical_room = SchemaFactory::get_instance()->get_logical_room();
            for (auto &peer: _info.peers()) {
                ...
                if (status.status == pb::NORMAL
                    ...
                    addr = peer;
                    if (status.logical_room == ksearch_logical_room) {
                        break;
                    }
                }
            }
        ...
        void set_region_vectorized_response(int64_t region_id,
                                            std::shared_ptr<pb::StoreRes> single_response_ptr) override;
    ...
    public:
        ...
        static void choose_opt_instance(int64_t region_id, Repeated &&peers, const std::string &resource_tag,
            ...
            std::string ksearch_logical_room = schema_factory->get_logical_room();
            ...
            for (auto &peer: peers) {
                ...
                DB_DEBUG("peer:%s resource_tag:%s status.resource_tag:%s, ksearch_logical_room:%s %s, %d", peer.c_str(),
                         resource_tag.c_str(), status.resource_tag.c_str(), ksearch_logical_room.c_str(),
                         status.logical_room.c_str(), status.status);
                if (status.status == pb::DEAD) {
                    ...
                    if (status.resource_tag == resource_tag) {
                        ...
                        if (!status.logical_room.empty() && status.logical_room == ksearch_logical_room) {
                            if (addr == peer) {
                ...
                } else if (!status.logical_room.empty() && status.logical_room == ksearch_logical_room) {
                    if (addr == peer) {
                        addr_in_candicate = true;
                    } else {
                        candicate_peers.emplace_back(peer);
