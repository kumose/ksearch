...
namespace ksearch {
    ...
    bool JoinNode::suitable_for_broadcast_join(QueryContext *ctx) {
        ...
        if (outer_scan_nodes.size() == 1
            ...
            && (!_use_index_join || ctx->runtime_state->sign_exec_type ==
                SignExecType::SIGN_EXEC_ARROW_FORCE_NO_INDEX_JOIN)) {
    ...
    int JoinNode::build_table_arrow_declaration(RuntimeState *state,
                                                ...
                                                std::unordered_map<std::string, arrow::compute::Expression> &
                                                projection_temp_col,
        ...
        if (node->node_exec_type() == pb::EXEC_ROW) {
            ...
            dec = arrow::acero::Declaration{
                "record_batch_source",
                arrow::acero::RecordBatchSourceNodeOptions{
                    vectorized_reader->schema(), std::move(iter_maker), (*executor).get()
                }
            };
    ...
    void JoinNode::get_need_add_index_collector_cond_nodes(ExecNode *node, std::set<ExecNode *> &need_add_nodes) {
        ...
        if (node->node_type() == pb::JOIN_NODE) {
            return get_need_add_index_collector_cond_nodes(static_cast<JoinNode *>(node)->get_outter_node(),
                                                           need_add_nodes);
        }
        if (node->node_type() == pb::DUAL_SCAN_NODE) {
            return get_need_add_index_collector_cond_nodes(static_cast<DualScanNode *>(node)->sub_query_node(),
                                                           need_add_nodes);
        }
    ...
    void JoinNode::handle_join_equal_key_cast_type(ExprNode *outer_expr,
        ...
        if (FLAGS_join_key_cast_like_mysql &&is_string(inner_type) &&!is_string(outer_type)
        )
        {
        ...
        if (fn_op == parser::FT_EQ) {
            ...
            if (left_child_is_outer) {
                sub_exprs.emplace_back(arrow::compute::call(arrow_func_name,
                                                            {
                                                                arrow::compute::field_ref(outer_child_tmp_col_name),
                                                                arrow::compute::field_ref(inner_child_tmp_col_name)
                                                            }));
            } else {
                sub_exprs.emplace_back(arrow::compute::call(arrow_func_name,
                                                            {
    ...
    int JoinNode::build_arrow_declaration(RuntimeState *state) {
        ...
        for (int i = 0; i < _outer_equal_slot.size(); ++i) {
            ...
            std::string outer_col_name = std::to_string(outer_expr->tuple_id()) + "_" + std::to_string(
                                             outer_expr->slot_id());
            std::string inner_col_name = std::to_string(inner_expr->tuple_id()) + "_" + std::to_string(
                                             inner_expr->slot_id());
            auto outer_type = outer_expr->col_type();
            auto inner_type = inner_expr->col_type();
            if (outer_type == inner_type) {
        ...
        arrow::acero::HashJoinNodeOptions join_opts{
            join_type, outer_keys, inner_keys, /*filter=*/arrow::compute::and_(sub_exprs)
        };
        ...
        while (1) {
            ...
            if (inner_mem_rows != NULL) {
                for (; _result_row_index < inner_mem_rows->size(); ++_result_row_index) {
                    ...
                    bool matched = false;
                    int ret = construct_result_batch(batch, *_outer_iter, (*inner_mem_rows)[_result_row_index],
                                                     matched);
                    if (ret < 0) {
                        DB_WARNING("construct result batch fail");
                        return ret;
        ...
        while (1) {
            ...
            if (outer_mem_rows != NULL) {
                for (; _result_row_index < outer_mem_rows->size(); ++_result_row_index) {
                    ...
                    bool matched = false;
                    int ret = construct_result_batch(batch, (*outer_mem_rows)[_result_row_index], inner_mem_row.get(),
                                                     matched);
                    if (ret < 0) {
                        DB_WARNING("construct result batch fail");
                        return ret;
