...
namespace ksearch {
    DEFINE_string(twoway_repl_mark_prefix, "MysqlKsearchTrans.repl_mark", "twoway relp_mark prefix");
    ...
    int InsertManagerNode::subquery_open(RuntimeState *state) {
        ...
        for (size_t i = 0; i < _select_projections.size(); i++) {
            ...
            if (_table_info->auto_inc_field_id == table_field_map[_selected_field_ids[i]]->id && sub_type ==
                pb::NULL_TYPE) {
                continue;
            }
            ...
            if (is_local) {
                if (_need_ignore || _is_replace || _on_dup_key_update) {
                    ...
                    return binlog_ctx->add_binlog_values(_table_info, ctx->sql, stat_info->sign,
                                                         has_delete_record
                                                             ? pb::MutationType::UPDATE
                                                             : pb::MutationType::INSERT,
                                                         _fetcher_store.return_str_records,
                                                         _fetcher_store.return_str_old_records);
                } else {
                    return binlog_ctx->add_binlog_values(_table_info, ctx->sql, stat_info->sign,
    ...
    */
    ...
    int InsertManagerNode::reverse_main_table(RuntimeState *state) {
        ...
        for (auto &pair: _store_records) {
            ...
            for (auto &record: pair.second) {
                ...
                ret = record->encode_key(*_pri_info, mt_key, -1, false);
                if (ret < 0) {
                    DB_WARNING("encode key failed, log_id:%lu record:%s", state->log_id(),
                ...
                if (pk_record_iter == _primary_record_key_record_map.end()) {
                    ...
                    if (_on_dup_key_update) {
                        ...
                        ret = record->encode_key(*info, mt_key, -1, false);
                        if (ret < 0) {
                            DB_WARNING("encode key failed, index:%ld log_id:%lu ret:%d", index_id, state->log_id(),
                                       ret);
                            return ret;
                        }
                        std::string key = mt_key.data();
    ...
    int InsertManagerNode::get_record_from_store(RuntimeState *state) {
        ...
        while (_execute_child_idx < _children.size() && _children[_execute_child_idx]->node_type() ==
               pb::LOCK_SECONDARY_NODE) {
