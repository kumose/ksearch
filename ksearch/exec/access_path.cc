...
#include <ksearch/exec/access_path.h>
...
namespace ksearch {
    ...
    bool AccessPath::need_add_to_learner_paths() {
        ...
        if (table_info_ptr->learner_resource_tags.size() > 0) {
            if (index_info_ptr->index_hint_status == pb::IHS_NORMAL) {
                if (butil::gettimeofday_us() - index_info_ptr->disable_time < _2h && butil::gettimeofday_us() -
                    index_info_ptr->restore_time < _1h) {
                    // 关闭以后马上打开，可以使用
                    return true;
                ...
                if (butil::gettimeofday_us() - index_info_ptr->restore_time < _2h && butil::gettimeofday_us() -
                    index_info_ptr->disable_time < _1h) {
                    // 打开以后马上关闭，不可以使用
                    return false;
    ...
    bool AccessPath::need_select_learner_index() {
        if (butil::gettimeofday_us() - index_info_ptr->restore_time < FLAGS_index_use_for_learner_delay_s * 1000 *
            1000LL
    ...
    void AccessPath::calc_row_expr_range(std::vector<int32_t> &range_fields, ExprNode *expr, bool in_open,
        ...
        if (index_info_ptr->type == pb::I_KEY) {
            avaliable_key_fields.insert(avaliable_key_fields.end(), index_info_ptr->pk_fields.begin(),
                                        index_info_ptr->pk_fields.end());
        }
    ...
    void AccessPath::calc_normal(Property &sort_property) {
        ...
        for (auto &field: avaliable_key_fields) {
            ...
            {
                get_date_in_values(range.left[0], range.left_open, range.right[0], range.right_open,
                                   range.eq_in_values);
                range.type = IN;
                // 可能会导致seek压力大
            }
            switch (range.type) {
                ...
                case IN:
                    if (range.is_row_expr && in_row_expr_map.count(*range.conditions.begin()) == 1) {
                        ...
                        if (in_records_size > 1 && in_records_size * range.eq_in_values.size() >
                            FLAGS_max_in_records_num) {
                            field_break = true;
                            break;
                        }
                        if (range.is_row_expr) {
                            ...
                            in_row_expr_map[*range.conditions.begin()] = std::pair<uint32_t, uint32_t>(
                                in_records_size, 1);
                        }
    ...
    void AccessPath::get_date_in_values(const ExprValue &left, bool left_open, const ExprValue &right, bool right_open,
    ...
    void AccessPath::calc_cost(std::map<std::string, std::string> *cost_info,
                               std::map<int32_t, double> &filed_selectivity) {
        ...
        if (is_sort_index &&index_other_condition_selectivity
        
        >
        0
        and other_condition_selectivity
        >
        0
        )
        {
        ...
        DB_DEBUG(
            "table_rows:%ld index_read_rows:%ld, selectivity:%f index_other_condition_selectivity:%f other_condition_selectivity:%f cost:%f",
            table_rows, index_read_rows, selectivity, index_other_condition_selectivity, other_condition_selectivity,
            cost);
        if (cost_info != nullptr) {
    ...
    void AccessPath::show_cost(std::map<std::string, std::string> *cost_info,
                               std::map<int32_t, double> &filed_selectivity) {
