...
namespace ksearch {
    ...
    int UnionNode::show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
        ...
        for (auto &it: output) {
            ...
            } else if (children_ids.end() != std::find(children_ids.begin(), children_ids.end(),
                                                       strtoll(it["id"].c_str(), NULL, 10))) {
