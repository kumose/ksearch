...
namespace ksearch {
    ...
    int ApplyNode::open(RuntimeState *state) {
        ...
        DB_WARNING("_use_hash_map:%d _max_one_row:%d _conditions_has_agg:%d", _use_hash_map, _max_one_row,
                   _conditions_has_agg);
    ...
    int ApplyNode::fetcher_inner_table_data(RuntimeState *state,
        ...
        if (_max_one_row &&inner_tuple_data
    ...
    int ApplyNode::show_explain(QueryContext *ctx, std::vector<std::map<std::string, std::string> > &output,
