...
#include <ksearch/vector_index/vector_index.h>
#include <ksearch/common/concurrency.h>
#include <ksearch/exec/vectorize_helpper.h>
#include <ksearch/engine/transaction.h>
...
namespace ksearch {
    ...
    public:
        IDSelectorBitmap(std::shared_ptr<arrow::BooleanArray> filter_bitmap,
                         std::shared_ptr<roaring::Roaring> del_bitmap)
    ...
    private:
        ...
        std::shared_ptr<roaring::Roaring> _del_bitmap;
    };
    ...
    int VectorIndex::init(const pb::RegionInfo &region_info, int64_t region_id, int64_t index_id, int64_t table_id) {
        ...
        if (desc.empty()) {
            ...
            if (std::regex_match(code_string, sm, std::regex("PQ([0-9]+)x4fs?"))) {
                ...
                if (_is_hnsw) {
                    if (nbit != 8) {
                        // faiss高版本支持
                        DB_WARNING("faiss 1.7.4 only support PQx8 for hnsw, but defined nbit: %d, desc: %s", nbit,
        ...
        DB_WARNING("info.vector_description: %s -> _vector_description: %s", info.vector_description.c_str(),
                   _vector_description.c_str());
    ...
    int VectorIndex::add_to_faiss(SmartFaissIndex faiss_index,
        ...
        if (_is_flat) {
            ...
            std::vector<std::shared_ptr<arrow::RecordBatch> > record_batches = {
                faiss_index->scalar_data, arrow_record_batch
            };
            ...
            std::shared_ptr<arrow::Schema> arrow_schema = arrow_record_batch->schema();
            std::vector<std::shared_ptr<arrow::RecordBatch> > record_batches = {
                faiss_index->flat_scalar_data, arrow_record_batch
            };
            std::shared_ptr<arrow::RecordBatch> flat_concat_record_batch;
            if (VectorizeHelpper::concatenate_record_batches(arrow_schema, record_batches, flat_concat_record_batch) !=
                0) {
                DB_WARNING("concatenate_record_batches failed");
                return -1;
            }
         ...
         */
            if (faiss_index->flat_index->ntotal >= 1000 && (
                    !index_need_train || faiss_index->index->is_trained || trigger_train)) {
                ...
                if (trigger_train) {
                    try {
                        ...
                        DB_WARNING(
                            "region_id: %ld, index: %s, dim: %d, index_size: %ld, train cost: %ld using vec size: %ld",
                            _region_id, _vector_description.c_str(), _dimension, cache_idxs.size(), cost.get_time(),
                            _train_vec_size);
                        _begin_train = false;
                    } catch (std::exception &e) {
                ...
                std::shared_ptr<arrow::RecordBatch> concat_record_batch;
                if (VectorizeHelpper::concatenate_record_batches(arrow_schema, record_batches, concat_record_batch) !=
                    0) {
                    DB_WARNING("concatenate_record_batches failed");
                    return -1;
                }
                ...
                faiss_index->flat_del_bitmap = std::make_shared<roaring::Roaring>();
    ...
    int VectorIndex::search(
        ...
        TimeCost total_cost;
        {
            ...
            auto build_arrow_expr_func = [&faiss_index, &arrow_filter_sub_exprs, &pre_filter_exprs
                    ](std::vector<ExprNode *> &filter_exprs) {
                for (auto iter = filter_exprs.begin(); iter != filter_exprs.end();) {
                    ...
                    if (filter_expr->contains_specified_fields(faiss_index->need_not_cache_fields) || !filter_expr->
                        can_use_arrow_vector()) {
            ...
            if (_is_flat) {
                ...
                do {
                    ...
                    if (has_filter) {
                        TimeCost tm;
                        auto arrow_res = arrow::compute::ExecuteScalarExpression(
                            arrow_filter_expr, *(faiss_index->flat_scalar_data->schema()),
                            faiss_index->flat_scalar_data);
                        if (!arrow_res.status().ok()) {
                            DB_WARNING("Fail to ExecuteScalarExpression, %s", arrow_res.status().message().c_str());
                            return -1;
                        ...
                        flat_bitmap_sel = std::make_shared<IDSelectorBitmap>(
                            flat_datum_array, faiss_index->flat_del_bitmap);
                    } else {
                    ...
                    try {
                        if (_is_hnsw) {
                            ...
                            search_params.need_brute_force_when_insufficient = FLAGS_need_brute_force_when_insufficient;
                            // hnsw搜索结果不足topK时退化成暴搜；
                    ...
                    if (total_cost.get_time() > FLAGS_print_time_us) {
                        DB_WARNING(
                            "region_id: %ld, index_id: %ld, separate_value: %lu, search:%ld, faiss.ntotal:%lu,%lu, "
                            "search_cnt: %ld,%ld, prefilter_time_cost: %ld, %ld, do_brute_force: %d",
                            _region_id, _index_id, faiss_index->separate_value, mink, faiss_index->flat_index->ntotal,
        ...
        while ((i < flat_idxs.size() || j < idxs.size())
            ...
            if (i < flat_idxs.size() && j < idxs.size()) {
                if (_metric_compare(flat_dis[i], dis[j])) {
                    // METRIC_L2和METRIC_INNER_PRODUCT相反
        ...
        records.clear();
        int ret = construct_records(txn, faiss_index->separate_value, pk_info, table_info, result_idxs, result_dis,
                                    records);
        if (ret < 0) {
    ...
    int VectorIndex::init_faiss_index(SmartFaissIndex faiss_index) {
        ...
        faiss_index->del_bitmap = std::make_shared<roaring::Roaring>();
    ...
    int VectorIndex::restore_faiss_index(const pb::RegionInfo &region_info, SmartFaissIndex faiss_index) {
        ...
        for (iter->Seek(key); iter->Valid(); iter->Next()) {
            ...
            if (value_slice[0] == V_NORMAL) {
                ...
                ret = txn->get_update_primary(_region_id, *pk_info, TableKey(pk_key), record, field_ids, GET_ONLY,
                                              true);
                if (ret == 0) {
    ...
    int VectorIndex::schema_change_faiss_index(SmartFaissIndex faiss_index) {
        ...
        if ((faiss_index->scalar_data != nullptr && !arrow_schema->Equals(faiss_index->scalar_data->schema(), true)) ||
            (faiss_index->flat_scalar_data != nullptr && !arrow_schema->Equals(
                 faiss_index->flat_scalar_data->schema(), true))) {
            // 直接变更，不需要从Rocksdb重建
            std::shared_ptr<arrow::RecordBatch> scalar_data;
            std::shared_ptr<arrow::RecordBatch> flat_scalar_data;
    ...
    int VectorIndex::compact_faiss_index(const pb::RegionInfo &region_info, SmartFaissIndex faiss_index,
                                         bool is_force) {
        if (faiss_index == nullptr) {
            DB_WARNING("faiss_index is nullptr");
            return -1;
    ...
    bool VectorIndex::need_compact_faiss_index(SmartFaissIndex faiss_index) {
        ...
        if (faiss_index->index != nullptr && need_train()) {
            ...
            if (vec_ntotal >= trigger_train_vec_ntotal) {
                ...
                DB_WARNING(
                    "triger train: vector_description: %s, vec_ntotal: %ld, trained: %d, _train_vec_size: %ld, trigger_train_vec_ntotal: %ld",
                    _vector_description.c_str(),
                    vec_ntotal,
                    faiss_index->index->is_trained,
                    _train_vec_size,
    ...
    int VectorIndex::read_del_bitmap_from_file(SmartFaissIndex faiss_index, const std::string &full_file_name) {
        ...
        try {
            *faiss_index->del_bitmap = roaring::Roaring::readSafe(serialized_str.c_str(), serialized_str.size());
        } catch (...) {
    ...
    int VectorIndex::read_not_cache_fields_from_file(SmartFaissIndex faiss_index, const std::string &full_file_name) {
        ...
        DB_WARNING(
            "succ read_not_cache_fields_from_file, _train_vec_size: %ld, need_not_cache_fields: size: %lu, str: %s",
