...
namespace ksearch {
    ...
    void Backup::process_upload_sst_streaming(brpc::Controller *cntl, bool ingest_store_latest_sst,
        ...
        if (region_ptr != nullptr) {
            ...
            Bthread streaming_work{&BTHREAD_ATTR_NORMAL};
            streaming_work.run(
                [this, region_ptr, ingest_store_latest_sst, data_sst_to_process_size, sd, receiver, backup_info,
                    row_size, id]() {
                    ...
                    region_ptr->update_streaming_result(
                        sd, ret == 0 ? pb::StreamState::SS_SUCCESS : pb::StreamState::SS_FAIL);
                }
            );
        }
    }
    ...
    */
        if (data_sst_to_process_size > 0) {
            if (boost::filesystem::exists(boost::filesystem::path(backup_info.data_info.path))) {
                int64_t data_sst_size = boost::filesystem::file_size(
                    boost::filesystem::path(backup_info.data_info.path));
