...
#include <ksearch/reverse/reverse_index.h>
#include <ksearch/vector_index/vector_index.h>
#include <ksearch/engine/transaction_pool.h>
#include <ksearch/runtime/runtime_state.h>
#include <ksearch/runtime/runtime_state_pool.h>
#include <rapidjson/document.h>
#include <ksearch/raft/rocksdb_file_system_adaptor.h>
#include <ksearch/store/region_control.h>
...
#include <ksearch/store/backup.h>
...
namespace ksearch {
    ...
    public:
        ColumnSnapshotClosure(int64_t region_id, int64_t raft_index, braft::SnapshotWriter *writer,
                              braft::Closure *done) : _raft_index(raft_index), _writer(writer), _done(done) {
        }
    ...
    public:
        ...
        Region(RocksWrapper *rocksdb,
               ...
               int64_t region_id,
               bool is_learner = false) : _rocksdb(rocksdb),
                                          _factory(factory),
                                          _address(address),
        ...
        int select_sample(RuntimeState &state, ExecNode *root, const pb::AnalyzeInfo &analyze_info,
                          pb::StoreRes &response);
        ...
        void get_read_index(const ksearch::pb::GetAppliedIndex *request, pb::StoreRes *response);
        ...
        int flush_hot_index_to_cold(int64_t index_id,
        ...
        bool use_userid_statis(
            bool is_eq, const google::protobuf::RepeatedPtrField<pb::PossibleIndex::Range> &key_ranges);

        std::vector<std::shared_ptr<ColumnFileInfo> > column_link_files(const std::vector<ColumnFileMeta> &file_infos,
                                                                        int64_t min_version, int64_t max_version);
    ...
    private:
        ...
        std::string binlog_get_str_val(const std::string &name,
                                       const std::map<std::string, ExprValue> &field_value_map);

        void binlog_get_scan_fields(std::map<int32_t, FieldInfo *> &field_ids, BinlogReadFields &binlog_fields,
        ...
        void binlog_query_primary_region(const int64_t &start_ts, const int64_t &txn_id, pb::RegionInfo &region_info,
                                         int64_t rollback_ts);

        void binlog_fill_exprvalue(const pb::BinlogDesc &binlog_desc, pb::OpType op_type,
        ...
        uint32_t get_time_interval_from_oldest_timestamp_to_now() {
            uint32_t timestamp = tso::get_timestamp_internal(
                std::max(_binlog_param.oldest_ts, _rocksdb->get_oldest_ts_in_binlog_cf()));
    ...
    private:
        ...
        //split后缓存分裂出去的region信息供ksearch使用
        ...
        //merge后该region为空，记录目标region，供ksearch使用，只会merge一次，不必使用vector
        ...
        struct NotLeaderAlarm {
            ...
            NotLeaderAlarm(int64_t region_id, const braft::PeerId &node_id) : type(ALARM_INIT), region_id(region_id),
                                                                              node_id(node_id) {
            }
