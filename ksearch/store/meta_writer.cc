...
namespace ksearch {
    ...
    int MetaWriter::read_transcation_rollbacked_tag(int64_t region_id, uint64_t txn_id) {
        ...
        auto status = _rocksdb->get(options, _meta_cf, rocksdb::Slice(rollbacked_transcation_key(region_id, txn_id)),
                                    &value);
        if (!status.ok()) {
            return -1;
        }
    ...
    int MetaWriter::write_meta_after_commit(int64_t region_id, int64_t num_table_lines,
                                            int64_t applied_index, int64_t data_index, uint64_t txn_id,
                                            bool need_write_rollback) {
        if (applied_index == 0) {
            return 0;
        }
    ...
    int MetaWriter::write_meta_begin_index(int64_t region_id, int64_t log_index, int64_t data_index, uint64_t txn_id,
        ...
        batch.Put(_meta_cf, transcation_log_index_key(region_id, txn_id),
                  encode_transcation_log_index_value(log_index));
        if (has_column_engine) {
            batch.Put(_meta_cf, column_txn_log_index_key(region_id, txn_id),
                      encode_rollup_region_init_index(log_index));
        }
        return write_batch(&batch, region_id);
    }
    ...
    int64_t MetaWriter::read_meta_begin_index(int64_t region_id, uint64_t txn_id) {
        std::string value;
        rocksdb::ReadOptions options;
        auto status = _rocksdb->get(options, _meta_cf, rocksdb::Slice(column_txn_log_index_key(region_id, txn_id)),
                                    &value);
        if (!status.ok()) {
...
int MetaWriter::ingest_meta_sst(const std::string& meta_sst_file, int64_t region_id) {
    ...
    void MetaWriter::read_applied_index(int64_t region_id, const rocksdb::ReadOptions &options, int64_t *applied_index,
                                        int64_t *data_index) {
        std::string value;
    ...
    rocksdb::Status MetaWriter::get_watt_stats_version(int64_t region_id, uint64_t watt_stats_version) const {
        std::string value;
        rocksdb::ReadOptions options;
        return _rocksdb->get(options, _meta_cf, rocksdb::Slice(watt_stats_version_key(region_id, watt_stats_version)),
                             &value);
    }
    ...
    rocksdb::Status MetaWriter::get_skip_watt_stats_version(int64_t region_id, uint64_t raft_index) const {
        std::string value;
        rocksdb::ReadOptions options;
        return _rocksdb->get(options, _meta_cf, rocksdb::Slice(skip_watt_stats_version_key(region_id, raft_index)),
                             &value);
    }
    ...
    int MetaWriter::clear_rollup_batch_version(int64_t region_id) {
        ...
        while (true) {
            ...
            if (s.ok()) {
                ...
                DB_WARNING(
                    "clear_rollup_batch_version fail region_id: %lu, rollup_batch_version: %lu, code: %d, msg: %s,"
    ...
    int MetaWriter::write_region_offline_binlog_info(int64_t region_id,
                                                     const pb::RegionOfflineBinlogInfo &offline_binlog_info) {
    ...
    int MetaWriter::read_region_offline_binlog_info(int64_t region_id,
