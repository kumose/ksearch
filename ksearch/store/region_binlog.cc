...
#define IF_DONE_SET_RESPONSE_FATAL(done, errcode, err_message) \
    ...
    void Region::binlog_query_primary_region(const int64_t &start_ts, const int64_t &txn_id,
                                             pb::RegionInfo &region_info, int64_t rollback_ts) {
        pb::StoreReq request;
    ...
    void Region::binlog_fill_exprvalue(const pb::BinlogDesc &binlog_desc, pb::OpType op_type,
    ...
    int64_t Region::binlog_get_int64_val(const std::string &name,
    ...
    std::string Region::binlog_get_str_val(const std::string &name,
                                           const std::map<std::string, ExprValue> &field_value_map) {
        auto iter = field_value_map.find(name);
        if (iter == field_value_map.end()) {
            return "";
    ...
    void Region::binlog_get_scan_fields(std::map<int32_t, FieldInfo *> &field_ids, BinlogReadFields &binlog_fields,
    ...
    void Region::binlog_update_map_when_scan(const std::map<std::string, ExprValue> &field_value_map) {
        ...
        _binlog_param.check_point_ts = _binlog_param.ts_binlog_map.empty()
                                           ? ts
    ...
    int Region::binlog_update_check_point() {
        ...
        int64_t check_point_ts = _binlog_param.ts_binlog_map.empty()
                                     ? _binlog_param.max_ts_applied
                                     : _binlog_param.ts_binlog_map.begin()->first;
    ...
    BinlogReadMgr::BinlogReadMgr(int64_t region_id, int64_t begin_ts, const std::string &capture_ip, uint64_t log_id,
        ...
        _mem_tracker = ksearch::MemTrackerPool::get_instance()->get_mem_tracker(_region_id);
    }
    ...
    int BinlogReadMgr::multiget(std::map<int64_t, std::string> &start_binlog_map) {
        ...
        if (!_read_offline_binlog) {
            ...
            if (cold_db == nullptr || cold_binlog_cf == nullptr) {
                DB_FATAL("region_id: %ld read offline binlog data but cold db or cold binlog cf is nullptr",
                         _region_id);
                return -1;
            }
    ...
    int BinlogReadMgr::get_prewrite_binlog(int64_t start_ts, std::map<int64_t, std::string> &start_binlog_map,
                                           bool &batch_finish, bool read_finished = false) {
        int ret = 0;
        batch_finish = false;
        if (!read_finished) {
    ...
    void BinlogReadMgr::print_log() {
        if (_binlog_total_row_cnts > 0 && _time.get_time() > FLAGS_print_time_us) {
            DB_WARNING(
                ...
                _region_id, _begin_ts, ts_to_datetime_str(_begin_ts).c_str(), _total_binlog_size, _mode, _need_read_cnt,
                _fake_binlog_cnt,
    ...
    void Region::read_binlog(const pb::StoreReq *request,
        ...
        binlog_get_scan_fields(field_ids, binlog_fields, field_slot, binlog_table, binlog_pri,
                               request->binlog_desc().flash_back_read());
        ...
        switch (request->op_type()) {
            case pb::OP_READ_BINLOG: {
                SmartMemTracker mem_tracker =
                        ksearch::MemTrackerPool::get_instance()->get_mem_tracker(_region_id);
                // 内存超限拒绝请求
 ...
 */
    struct OfflineSSTInfo {
        ...
        int parse_remote_offline_binlog_sst(std::string external_file) {
            ...
            if (vec.back() == "binlogsst" || vec.back() == "datasst") {
                ...
                DB_FATAL("not offline binlog file: %s with abnormal file type: %s", external_file.c_str(),
                         vec.back().c_str());
                return -1;
            }
            return 0;
    ...
    int Region::ingest_offline_binlog_sst(const std::vector<std::string> &data_ssts,
        ...
        if (!data_ssts.empty()) {
            ...
            if (!s.ok()) {
                DB_FATAL("region_id: %ld ingest data_ssts failed, err: %s, cost: %ld", _region_id, s.ToString().c_str(),
                         cost.get_time());
                return -1;
            }
        }
        if (!binlog_ssts.empty()) {
            ...
            if (!s.ok()) {
                DB_FATAL("region_id: %ld ingest binlog_ssts failed, err: %s, cost: %ld", _region_id,
                         s.ToString().c_str(), cost.get_time());
                return -1;
            }
        }
    ...
    void Region::update_offline_binlog_info(const pb::StoreReq &request, braft::Closure *done) {
        ...
        DB_WARNING(
            "region_id: %ld, update backup binlog range: [%ld, %ld]:[%s, %s] -> [%ld, %ld]:[%s, %s], request: %s, cost: %ld",
            _region_id,
            oldest_ts,
            newest_ts,
    ...
    void Region::recover_offline_binlog_info(const pb::StoreReq *request, pb::StoreRes *response) {
        ...
        DB_WARNING(
            "region_id: %ld, recovery backup binlog range: [%ld, %ld]:[%s, %s] -> [%ld, %ld]:[%s, %s], request: %s, cost: %ld",
            _region_id,
            oldest_ts,
            newest_ts,
            ts_to_datetime_str(oldest_ts).c_str(),
    ...
    bool Region::need_clear_offline_binlog_sst() {
        ...
        if (backup_days <= 0 && (!_offline_binlog_param.data_ssts.empty() || !_offline_binlog_param.binlog_ssts.
                                 empty())) {
            return true;
        }
        return false;
     ...
     */
        ...
        ScopeGuard auto_decrease([this, &data_sst_writer, &binlog_sst_writer]() {
            ...
            for (const std::string &external_file: data_sst_writer.external_files()) {
                DB_WARNING("region_id: %ld backup fail, delete data external_file: %s", _region_id,
                           external_file.c_str());
                fs->delete_path(external_file, false);
            }
            for (const std::string &external_file: binlog_sst_writer.external_files()) {
                DB_WARNING("region_id: %ld backup fail, delete binlog external_file: %s", _region_id,
                           external_file.c_str());
                fs->delete_path(external_file, false);
            }
        });
    ...
    void Region::do_backup_binlog() {
        ...
        ScopeGuard auto_decrease([this]() {
            ...
            for (const std::string &file: _offline_binlog_task.data_ssts) {
                DB_WARNING("region_id: %ld backup fail, maybe need to delete afs by hand, data external_file: %s",
                           _region_id, file.c_str());
            }
            for (const std::string &file: _offline_binlog_task.binlog_ssts) {
                DB_WARNING("region_id: %ld backup fail, maybe need to delete afs by hand, binlog external_file: %s",
                           _region_id, file.c_str());
            }
        });
        if (!is_leader()) {
    ...
    void Region::delete_remote_expired_file() {
        ...
        if (ttl_remote_files.empty()) {
            DB_WARNING(
                "region_id: %ld, ttl_timestamp: %ld: %s > offline_binlog_oldest_ts: %ld: %s, no expired afs file",
                _region_id, ttl_ts, ts_to_datetime_str(ttl_ts).c_str(),
                offline_binlog_oldest_ts, ts_to_datetime_str(offline_binlog_oldest_ts).c_str());
            return;
        }
        DB_WARNING(
            "region_id: %ld, ttl_timestamp: %ld: %s > offline_binlog_oldest_ts: %ld: %s, with some expired afs files, need ttl",
            _region_id, ttl_ts, ts_to_datetime_str(ttl_ts).c_str(),
            offline_binlog_oldest_ts, ts_to_datetime_str(offline_binlog_oldest_ts).c_str());
        pb::StoreReq req;
        req.set_region_version(_version);
