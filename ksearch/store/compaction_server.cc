...
#include <ksearch/engine/rocksdb_merge_operator.h>
#include <butil/file_util.h>
#include <ksearch/engine/my_listener.h>
...
namespace ksearch {
    ...
    int remote_copy_file(const std::string &local_file,
        ...
        const static int64_t BUF_SIZE{FLAGS_remote_copy_buf_size_mb * 1024 * 1024LL};
        ...
        if (write_size != length) {
            DB_FATAL("sst_file: %s external_file: %s, write_size: %ld, length: %ld", local_file.c_str(),
                     external_file.c_str(), write_size, length);
            return -1;
        }
    ...
    void CompactionServer::do_compaction(google::protobuf::RpcController *controller,
        ...
        //         remote_compaction_id_str.c_str(), Concurrency::get_instance()->remote_compaction_server_concurrency.count());
        int ret = Concurrency::get_instance()->remote_compaction_server_concurrency.increase_timed_wait(
            10 * 1000 * 1000LL);
        if (ret != 0) {
        ...
        rocksdb::Status s =
                rocksdb::CompactionServiceInput::Read(request->compaction_input_binary(), &compaction_input);
        if (!s.ok()) {
        ...
        if (CompactionSstExtLinker::get_instance()->register_job(remote_compaction_id_str, ext_fs,
                                                                 compaction_input.input_files) != 0) {
        ...
        std::unique_ptr<rocksdb::Env> compaction_env = rocksdb::NewCompositeEnv(
            std::make_shared<CompactionFileSystemWrapper>(request->remote_compaction_id()));
        ...
        if (open_and_compaction_cost > FLAGS_remote_compaction_request_timeout * 1000LL) {
            DB_WARNING(
                "remote_compaction_id: %s, remote_address: %s, time_cost: %ld open and do compaction cost too long, more than %d",
                remote_compaction_id_str.c_str(), request->address().c_str(),
        ...
        if (CompactionSstExtLinker::get_instance()->get_linker_data(remote_compaction_id_str, linker_data) != 0 ||
            linker_data == nullptr) {
        ...
        if ((linker_data->cache_size + linker_data->not_cache_size) != 0) {
            remote_compaction_cache_rate << (10000 * linker_data->cache_size / (
                                                 linker_data->cache_size + linker_data->not_cache_size));
        }
        // 任务结束, 删除文件
        auto_decrease.release();
        response->set_errcode(pb::SUCCESS);
        DB_NOTICE(
            "remote_compaction_id=%s, remote_address=%s, total_cost=%ld|%ld|%ld, file_num=%ld|%ld, cache=%ld|%ld|%ld, read_file_time=%ld",
