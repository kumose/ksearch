...
#include <ksearch/store/region.h>
#include <algorithm>
#include <fstream>
#include <boost/filesystem.hpp>
#include <boost/algorithm/string.hpp>
#include <ksearch/common/table_key.h>
...
#include <ksearch/raft/my_raft_log_storage.h>
#include <ksearch/raft/log_entry_reader.h>
#include <ksearch/raft/raft_log_compaction_filter.h>
#include <ksearch/engine/split_compaction_filter.h>
#include <ksearch/store/rpc_sender.h>
#include <ksearch/common/concurrency.h>
#include <ksearch/store/store.h>
#include <ksearch/store/closure.h>
#include <rapidjson/rapidjson.h>
#include <ksearch/engine/qos.h>
#include <ksearch/runtime/arrow_io_excutor.h>
#include <ksearch/exec/exchange_sender_node.h>
#include <ksearch/exec/ttl_delete_node.h>
...
namespace ksearch {
    ...
    ScopeProcStatus::~ScopeProcStatus() {
        if (_region != NULL) {
            ...
            ksearch::Store::get_instance()->sub_split_num();
        }
    }
    ...
    int Region::init(bool new_region, int32_t snapshot_times) {
        ...
        if (new_region) {
            ...
            if (_region_info.peers_size() > 0) {
                ...
                if (_is_learner &&_meta_writer
                ->
                write_learner_key(_region_info.region_id(), _is_learner) != 0
                )
                {
        ...
        if (ttl_info.ttl_duration_s > 0) {
            ...
            if (global_index == nullptr) {
                DB_FATAL("init region failed, missing region global index info. region_id:%ld, global_index_id:%ld",
                         _region_id, _global_index_id);
                return -1;
            }
            if (update_ttl_info() != 0) {
                DB_FATAL("init region failed, update ttl info failed. region_id:%ld, global_index_id:%ld", _region_id,
                         _global_index_id);
                return -1;
            }
        }
    ...
    int Region::validate_version(const pb::StoreReq *request, pb::StoreRes *response, bool &is_valid_version) {
        if (request->region_version() < get_version()) {
            ...
            // MPP场景，通知上游ER处理
            if (request->plan().nodes().size() > 0 && request->plan().nodes(0).node_type() ==
                pb::EXCHANGE_SENDER_NODE) {
                if (mpp_send_version_old(request->plan().nodes(0).derive_node().exchange_sender_node(),
                                         response->is_merge(),
    ...
    void Region::exec_in_txn_query(google::protobuf::RpcController *controller,
        ...
        if (txn == nullptr) {
            ...
            if (ret == 0) {
                //查询meta存在说明已经ROLLBACK，ksearch直接返回TXN_IS_ROLLBACK错误码
        ...
        // 多语句事务局部回滚
        if (FLAGS_apply_partial_rollback &&txn 
        !=
        nullptr
        ...
        )
        {
            ...
            if (need_rollback) {
                if (apply_partial_rollback(controller, txn, request, response) != 0 || response->errcode() !=
                    pb::SUCCESS) {
        ...
        if (!is_leader() && !request->extra_req().no_raft_log()) {
            ...
            if (request->extra_req().use_read_idx()) {
                ...
                if (read_index_total_cost > FLAGS_print_time_us) {
                    DB_WARNING(
                        "region_id: %ld, log_id: %lu, read_index_total_cost: %ld, ask_rdx_time: %ld, wait_rdx_time: %ld, read_index: %ld",
                        _region_id, log_id,
        ...
        switch (request->op_type()) {
            ...
            case pb::OP_ROLLUP_REGION_FAILED: {
                ...
                if (_rollup_region_init_index != -1
                    ...
                    DB_WARNING(
                        "doing rollup ddl not support exclude insert merge int, op_type:%s region_id: %ld, log_id:%lu",
                        pb::OpType_Name(op_type).c_str(), _region_id, log_id);
                }
    ...
    void Region::dml_2pc(const pb::StoreReq &request,
        ...
        if (op_type != pb::OP_BEGIN && txn != nullptr) {
            ...
            // 而导致当前region为follow_up, 每次都需要从ksearch拉取cached命令
            txn->set_seq_id(seq_id);
        ...
        if (/*txn_info.autocommit() && */(op_type == pb::OP_UPDATE || op_type == pb::OP_INSERT || op_type ==
                                          pb::OP_DELETE)) {
    ...
    int Region::select(const pb::StoreReq &request, pb::StoreRes &response) {
        ...
        if (FLAGS_open_new_sign_read_concurrency && (StoreQos::get_instance()->is_new_sign(sqlqos_ptr))) {
            ...
            // sign一级并发控制
            if (reject_timeout > 0) {
                sign_concurrency_ret = sqlqos_ptr->increase_timed_wait_with_max_wait_cnt(
                    FLAGS_max_sign_concurrency_wait_cnt, reject_timeout);
            } else {
            ...
            if (sign_concurrency_ret == 0) {
                if (t.get_time() > 10 * 1000) {
                    // 全局二级并发控制
                    if (reject_timeout > 0) {
                        global_concurrency_ret = Concurrency::get_instance()->global_select_concurrency.
                                increase_timed_wait_with_max_wait_cnt(FLAGS_max_sign_concurrency_wait_cnt,
                                                                      reject_timeout);
                    } else {
                        Concurrency::get_instance()->global_select_concurrency.increase_wait_with_max_wait_cnt(
                            FLAGS_max_sign_concurrency_wait_cnt);
                    }
            ...
            if (sign_concurrency_ret != 0 || global_concurrency_ret != 0) {
                DB_FATAL(
                    "sign: %lu, log_id: %lu, get concurrency quota fail, sign_concurrency_ret: %d, global_concurrency_ret: %d, reject_timeout: %ld",
    ...
    int Region::select_vectorized(RuntimeState &state, ExecNode *root, const pb::StoreReq &request,
                                  pb::StoreRes &response) {
        ...
        if (ret != 0) {
            DB_FATAL("arrow execute fail: build arrow acero plan fail, region: %ld, log_id: %lu", _region_id,
                     request.log_id());
            return -1;
        }
        // execute
        ...
        if (table->num_rows() == 0) {
            ...
            if (exchange_sender_node != nullptr && runtime > FLAGS_print_time_us) {
                DB_NOTICE(
                    "mpp request, region_id: %ld, time_cost: %ld, log_id: %lu, sign: %lu, rows: %ld, scan_rows: %ld, repartition_cost: %ld",
        ...
        t.reset();
        arrow::Result<std::shared_ptr<arrow::Buffer> > schema_ret = arrow::ipc::SerializeSchema(
            *(record_batch->schema()), pool);
        if (!schema_ret.ok()) {
        ...
        arrow::Result<std::shared_ptr<arrow::Buffer> > data_ret = arrow::ipc::SerializeRecordBatch(
    ...
    int Region::select(const pb::StoreReq &request,
        ...
        pb::TraceNode trace_node;
        std::string desc = "ksStore select";
        TimeCost cost;
        bool is_trace = false;
        if (request.has_is_trace()) {
            is_trace = request.is_trace();
    ...
    int Region::select_normal(RuntimeState &state, ExecNode *root, const pb::StoreReq &request,
                              pb::StoreRes &response) {
        bool eos = false;
        int rows = 0;
        int ret = 0;
        ...
        if (total_packet_time > FLAGS_print_time_us) {
            DB_WARNING("region_id: %ld, log_id: %lu, total_packet_time: %ld", _region_id, request.log_id(),
                       total_packet_time);
        }
        return rows;
    }
    ...
    //抽样采集
    int Region::select_sample(RuntimeState &state, ExecNode *root, const pb::AnalyzeInfo &analyze_info,
                              pb::StoreRes &response) {
        bool eos = false;
        int ret = 0;
        int count = 0;
        ...
        if (analyze_info.sample_rows() >= analyze_info.table_rows()) {
            ...
            sample_cnt = analyze_info.sample_rows() * 1.0 / std::max(analyze_info.table_rows(), (int64_t) 1) *
    ...
    void Region::construct_heart_beat_request(pb::StoreHeartBeatRequest &request, bool need_peer_balance,
        ...
        std::vector<braft::PeerId> peers;
        bool need_report = FLAGS_report_all_to_meta || meta_need_report || need_peer_balance || request.
                           need_leader_balance();
        // 只有完全正常才不上报
        if (is_merged() || is_learner() || _is_binlog_region || _version == 0 || _region_control.get_status() !=
            pb::IDLE) {
            need_report = true;
        }
    ...
    void Region::do_apply(int64_t term, int64_t index, const pb::StoreReq &request, braft::Closure *done) {
        ...
        switch (op_type) {
            ...
            case pb::OP_ROLLUP_REGION_INIT: {
                _data_index = _applied_index;
                if (_rollup_region_init_index != -1) {
                    DB_FATAL("rollup_region_init_index: %ld is not expect, OP_ROLLUP_REGION_INIT already exist",
                             _rollup_region_init_index);
                    if (done != nullptr) {
                        ((DMLClosure *) done)->response->set_errcode(pb::EXEC_FAIL);
                        ((DMLClosure *) done)->applied_index = _applied_index;
                    }
                ...
                if (_olap_state.load() == pb::OLAP_IMMUTABLE) {
                    DB_WARNING("rollup_region_init_index: %ld, _olap_state is OLAP_IMMUTABLE, try again",
                               _rollup_region_init_index);
                    if (done != nullptr) {
                        ((DMLClosure *) done)->response->set_errcode(pb::EXEC_FAIL);
                        ((DMLClosure *) done)->applied_index = _applied_index;
                ...
                if (ret != 0 && done != nullptr) {
                    ...
                    ((DMLClosure *) done)->applied_index = _applied_index;
                    DB_FATAL(
                        "clear_rollup_batch_version fail op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                        pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term,
                        _rollup_region_init_index);
                }
                ...
                if (ret != 0 && done != nullptr) {
                    ...
                    ((DMLClosure *) done)->applied_index = _applied_index;
                    DB_FATAL(
                        "delete_hot_local_rocksdb fail op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                        pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term,
                        _rollup_region_init_index);
            ...
            case pb::OP_ROLLUP_REGION_FINISH: {
                _data_index = _applied_index;
                int ret = exec_rollup_region_finish_request(request, _rollup_region_init_index + 1, _applied_index,
                                                            done, term, false);
                if (ret != 0) {
                    DB_FATAL(
                        "exec_rollup_region_finish_request op_type=%s, region_id: %ld, _applied_index:%ld, term:%ld, rollup_region_init_index: %ld",
                        pb::OpType_Name(request.op_type()).c_str(), _region_id, _applied_index, term,
                        _rollup_region_init_index);
                    if (done != nullptr) {
                        ((DMLClosure *) done)->response->set_errcode(pb::EXEC_FAIL);
    ...
    void Region::apply_kv_out_txn(const pb::StoreReq &request, braft::Closure *done,
        ...
        if (watt_stats_version != 0) {
            ...
            if (s.IsNotFound()) {
                ...
                DB_WARNING("repeat write region_id: %ld watt_stats_version: %lu, raft_index: %ld", _region_id,
                           watt_stats_version, index);
                // 找到说明已经写入不可以再写
    ...
    void Region::transfer_leader_after_split() {
        ...
        for (auto &peer: peers) {
            ...
            int get_apply_index_ret = RpcSender::get_peer_applied_index(peer_string, _region_id, peer_applied_index,
                                                                        peer_dml_latency);
    ...
    void Region::add_version_for_split_region(const pb::StoreReq &request, braft::Closure *done, int64_t applied_index,
                                              int64_t term) {
    ...
    void Region::on_snapshot_save(braft::SnapshotWriter *writer, braft::Closure *done) {
            ...
            for (auto pair: _vector_index_map) {
                ...
                if (idx->is_separate()) {
                    ...
                    if (idx->write_to_separate_files(_snapshot_path, faiss_name_vec) != 0) {
                        ...
                        DB_WARNING("Error while faiss write_to_separate_files, region_id: %ld, index_id: %ld",
                                   _region_id, index_id);
                        return;
                    }
                    ...
                    if (idx->write_to_file(_snapshot_path, faiss_name_vec) != 0) {
                        ...
                        DB_WARNING("Error while faiss write_to_file, region_id: %ld, index_id: %ld", _region_id,
                                   index_id);
                        return;
                    }
                    faiss_files.insert(faiss_files.end(), faiss_name_vec.begin(), faiss_name_vec.end());
        ...
        _snapshot_path += buf;
        DB_WARNING(
            "region_id: %ld snapshot save complete, _snapshot_index:%ld _applied_index:%ld time_cost: %ld, _snapshot_path:%s",
    ...
    void Region::snapshot(braft::Closure *done) {
        ...
        DB_WARNING("region_id: %ld do snapshot, snapshot_num_table_lines:%ld, num_table_lines:%ld "
                   ...
                   _snapshot_index, _applied_index, _rollup_region_init_index,
                   _snapshot_time_cost.get_time() / 1000 / 1000);
        done_guard.release();
        if (is_learner()) {
        ...
        for (int64_t index_id: indices) {
            auto read_and_write = [this, &pk_info, &write_sst_lines,
                ...
                DB_WARNING("scan index:%ld, cost=%ld, file_size=%lu, file_num=%ld, lines=%ld, skip:%ld, region_id: %ld "
                           ...
                           index_id, cost.get_time(), file_size, file_num, num_write_lines, skip_write_lines,
                           _region_id,
    ...
    void Region::complete_split() {
        ...
        //分离完成后立即发送一次心跳
        ksearch::Store::get_instance()->send_heart_beat();
    ...
    int Region::exec_rollup_region_finish_request(const pb::StoreReq &request,
                                                  ...
                                                  const int64_t unappiled_end_index, braft::Closure *done, int64_t term,
                                                  bool is_failed) {
        if (_rollup_region_init_index == -1) {
        ...
        begin_log_data_key.append_i64(_region_id).append_u8(MyRaftLogStorage::LOG_DATA_IDENTIFY).append_i64(
            unappiled_begin_index);
        MutTableKey end_log_data_key;
        end_log_data_key.append_i64(_region_id).append_u8(MyRaftLogStorage::LOG_DATA_IDENTIFY).append_i64(
            unappiled_end_index);
        ...
        for (int i = 0; iter->Valid() && (unappiled_begin_index + i) < unappiled_end_index; iter->Next(), i++) {
            ...
            LogHead head(iter->value());
            if ((braft::EntryType) head.type != braft::ENTRY_TYPE_DATA) {
                DB_WARNING("log entry is not data, log_index:%ld, region_id: %ld, type: %d", unappiled_begin_index + i,
                           _region_id, (braft::EntryType) head.type);
                continue;
            }
            value_slice.remove_prefix(MyRaftLogStorage::LOG_HEAD_SIZE);
            pb::StoreReq store_req;
            if (!store_req.ParseFromArray(value_slice.data(), value_slice.size())) {
                DB_FATAL(
                    "Fail to parse request fail, exec_rollup_region_finish_request fail, log_index:%ld, region_id: %ld, type: %d",
        ...
        if (!is_failed && _olap_state.load() >= pb::OLAP_FLUSHED) {
            ...
            if (olap_info.has_new_olap_index_info()) {
                DB_FATAL("region_id: %ld, index_id: %ld is flushing cold", _region_id,
                         olap_info.new_olap_index_info().index_id());
                return -1;
            }
            for (auto &olap_index_info: olap_info.olap_index_info_list()) {
                if (olap_index_info.index_id() == ddl_index_id) {
                    DB_FATAL("region_id: %ld, index_id: %ld is flushed", _region_id,
                             olap_info.new_olap_index_info().index_id());
                    return -1;
                }
            }
    ...
    int Region::get_log_entry_for_split(const int64_t split_start_index,
        ...
        butil::IOBuf attachment_data;
        MutTableKey log_data_key;
        log_data_key.append_i64(_region_id).append_u8(MyRaftLogStorage::LOG_DATA_IDENTIFY).
                append_i64(split_start_index);
        rocksdb::ReadOptions read_options;
        read_options.prefix_same_as_start = true;
        read_options.total_order_seek = false;
        read_options.fill_cache = false;
    ...
    bool Region::can_use_approximate_split() {
        if (FLAGS_use_approximate_size &&
            ...
            get_num_table_lines() 
        >
        FLAGS_min_split_lines &&
                get_last_split_time_cost()
        >
        60 * 60 * 1000 * 1000LL &&
        ...
        )
        {
            if (_approx_info.last_version_region_size > 0 &&
                ...
                double last_avg_size = 1.0 * _approx_info.last_version_region_size / _approx_info.
                                       last_version_table_lines;
                // avg_size差不多时才能做
    ...
    int Region::update_ttl_info() {
        ...
        if (!use_ttl()) {
            ...
            if (ttl_field != nullptr) {
                ...
                if (_ttl_field_type == TTLFieldType::T_NON_TTL) {
                    ...
                    DB_FATAL("No ttl field find in main_table or global index, region_id: %ld, index_id: %ld",
                             _region_id, _global_index_id);
                    return -1;
                }
            }
        ...
        if (ttl_info.ttl_duration_s > 0) {
            ...
            // online TTL
            if (ttl_info.online_ttl_expire_time_us > 0 && ttl_info.online_ttl_expire_time_us !=
    ...
    int Region::get_ttl_timestamp(IndexInfo &index_info,
        ...
        if (need_decode_ttl_field()) {
            ...
            if (_ttl_field_type == TTLFieldType::T_FIELD_KEY) {
                ...
                if (ret != 0) {
                    DB_WARNING("ttl decode field value from key failed, index_id:%ld, field_id:%d", index_info.id,
                               _ttl_field->id);
                    return ret;
                }
                ...
                if (ret != 0) {
                    DB_WARNING("ttl decode field value from pk value failed, index_id:%ld, field_id:%d", index_info.id,
                               _ttl_field->id);
                    return ret;
                }
                ...
                if (ret != 0) {
                    DB_WARNING("ttl decode field value from guidx value failed, index_id:%ld, field_id:%d",
                               index_info.id, _ttl_field->id);
                    return ret;
                }
            ...
            ttl_timestamp = ttl_decode(rocksdb_value_cpy, &index_info,
                                       is_field_ttl() ? 0 : _online_ttl_base_expire_time_us);
        }
        return 0;
    }
        ...
        for (int64_t index_id: indices) {
            ...
            for (iter->Seek(table_prefix.data()); iter->Valid(); iter->Next()) {
                ...
                if (is_binlog_region()) {
                    ...
                    if (oldest_tso > 0) {
                        //需要吗?
                        oldest_tso = timestamp_to_ts(tso::get_timestamp_internal(oldest_tso) - 7200);
                        // data cf 比binlog cf延迟两小时删除
                ...
                if (!is_binlog_region()) {
                    ...
                    uint64_t ttl_timestamp = std::numeric_limits<std::uint64_t>::max();
                    int ret = get_ttl_timestamp(index_info, iter->key(), rocksdb::Slice(value), record_template,
                                                ttl_timestamp);
                    if (ret < 0) {
                        continue;
                    }
    ...
    void Region::get_read_index(const ksearch::pb::GetAppliedIndex *request, pb::StoreRes *response) {
        if (_shutdown
            || !_init_success
            || (is_learner() && _learner == nullptr)
            || (is_learner() && !learner_ready_for_read())) {
    ...
    void NoOpTimer::run() {
        if (_region != nullptr && !_region->is_shutdown() && _region->is_leader() && !_region->is_splitting() && _region
            ->get_version() > 0) {
            // 发no op
