...
namespace ksearch {
    ...
    int Store::init_before_listen(std::vector<std::int64_t> &init_region_ids) {
        ...
        if (0 != create_dir_if_not_exist(FLAGS_db_path + "_tmp") || 0 != create_dir_if_not_exist(
                FLAGS_db_path + "_column")) {
...
*/
    ...
    void Store::restore_region(google::protobuf::RpcController *controller,
                               const ksearch::pb::RegionIds *request,
                               pb::StoreRes *response,
                               google::protobuf::Closure *done) {
        brpc::ClosureGuard done_guard(done);
        response->set_errcode(pb::SUCCESS);
    ...
    void Store::get_applied_index(google::protobuf::RpcController *controller,
                                  const ksearch::pb::GetAppliedIndex *request,
                                  pb::StoreRes *response,
                                  google::protobuf::Closure *done) {
        brpc::ClosureGuard done_guard(done);
        response->set_errcode(pb::SUCCESS);
    ...
    void Store::manual_split_region(google::protobuf::RpcController *controller,
        ...
        auto fun = [this, region_ids, userids, response]() {
            ...
            for (int i = 0; i < region_ids.size(); i++) {
                ...
                SmartRegion region = get_region(region_id);
                if (region == nullptr || region->get_version() == 0 || region->is_binlog_region() || region->
                    is_learner() ||
                ...
                if (userid == 0) {
                    bool is_cold = false;
                    bool is_olap_table = _factory->is_olap_table(region->get_table_id(), region->get_partition_id(),
                                                                 &is_cold);
                    if (is_olap_table) {
                        if (is_cold) {
                            continue;
    ...
    void Store::query_region(google::protobuf::RpcController *controller,
        ...
        if (request->query_apply_index()) {
            ...
            for (const auto &md: metadata) {
                ...
                if (smallest_region_id != largest_region_id) {
                    DB_FATAL("cold sst: %s has diff region_id %ld vs %ld", md.relative_filename.c_str(),
                             smallest_region_id, largest_region_id);
                    continue;
                }
            ...
            for (int64_t region_id: region_ids) {
                ...
                if (olap_info.state() < pb::OLAP_FLUSHED || olap_info.external_full_path_size() <= 0) {
                    DB_WARNING("region_id: %ld diff state info: %s", region->get_region_id(),
                               olap_info.ShortDebugString().c_str());
                ...
                if (iter == region_sst_map.end()) {
                    ...
                    DB_FATAL("region_id: %ld cant find cold sst, olap_info: %s", region->get_region_id(),
                             olap_info.ShortDebugString().c_str());
                    continue;
                }
        ...
        if (request->query_olap_keypoint()) {
            ...
            for (int64_t region_id: request->region_ids()) {
                ...
                _rocksdb->decode_key_points(region_start_key, region_end_key, rocksdb_start_key.data().substr(0, 16),
                                            props, keys);
        ...
        while (!_shutdown) {
            ...
            traverse_copy_region_map([this](const SmartRegion &region) {
                ...
                int64_t random_remove_data_timeout = (FLAGS_region_delay_remove_timeout_s +
                                                      (int64_t)(
                                                          butil::fast_rand() % FLAGS_region_delay_remove_timeout_s)) *
                                                     1000 * 1000LL;
                if (region->removed() &&
    ...
    void Store::flush_memtable_thread() {
        ...
        while (!_shutdown) {
            ...
            _rocksdb->get_db()->GetIntProperty(_rocksdb->get_data_handle(), "rocksdb.oldest-snapshot-time",
                                               &snapshot_time);
            if (snapshot_time > 0) {
            ...
            if (force_flush || mata_count > FLAGS_rocks_cf_flush_remove_range_times || last_file_number != _rocksdb->
                flush_file_number()) {
    ...
    void Store::cold_region_check() {
        ...
        for (const auto &md: metadata) {
            ...
            if (smallest_region_id != largest_region_id) {
                // 报警
                DB_FATAL("cold sst: %s has diff region_id %ld vs %ld", md.relative_filename.c_str(), smallest_region_id,
                         largest_region_id);
                continue;
            }
            region_sst_map[smallest_region_id].insert(md.relative_filename);
        ...
        for (SmartRegion region: regions) {
            ...
            if (region->column_status() == pb::CS_COLD) {
                ...
                if (total_lines != column_lines && (total_lines - index_lines) != column_lines) {
                    DB_COLUMN_FATAL(
                        "region_id: %ld diff lines row vs column total_lines:%lu, index_lines:%ld, column_lines:%ld",
    ...
    void Store::hot_region_check() {
        ...
        for (const auto &iter: region_size_map) {
            ...
            if (iter.second > FLAGS_region_size_alarm_threshold_G * 1024 * 1024 * 1024LL) {
                ...
                if (it != regions.end()) {
                    DB_WARNING("table_id: %ld region_id: %ld size: %ld too big", it->second->get_table_id(), iter.first,
                               iter.second);
                }
            }
        }
    ...
    int64_t Store::get_tso() {
        ...
        if (tso_physical != 0 && tso_logical != 0 && tso_count > 0 && gen_tso_time.get_time() <
            FLAGS_gen_tso_interval_us) {
    ...
    void Store::process_split_request(int64_t table_id, int64_t region_id, bool tail_split,
    ...
    void Store::whether_split_thread() {
        ...
        while (!_shutdown) {
            ...
            for (size_t i = 0; i < region_ids.size(); ++i) {
                ...
                bool is_cold = false;
                bool is_olap_table = _factory->is_olap_table(ptr_region->get_table_id(), ptr_region->get_partition_id(),
                                                             &is_cold);
                //设置计算存储分离开关
                if (is_olap_table) {
                    // olap表默认开启kv分离
                ...
                if (ptr_region->is_leader()
                    ...
                    if (ptr_region->get_log_index() != ptr_region->get_log_index_lastcycle()) {
                        ...
                        DB_WARNING("region:%ld is none, num_table_lines:%ld, used_size:%ld, log_index:%ld reset time",
                                   region_ids[i], ptr_region->get_num_table_lines(), ptr_region->get_used_size(),
                                   ptr_region->get_log_index());
                        continue;
                    }
                    if (ptr_region->get_log_index() == ptr_region->get_log_index_lastcycle()
                        && ptr_region->get_lastcycle_timecost() > FLAGS_none_region_merge_interval_us) {
                        DB_WARNING(
                            "region:%ld is none, num_table_lines:%ld, used_size:%ld, log_index:%ld process merge",
                            region_ids[i], ptr_region->get_num_table_lines(), ptr_region->get_used_size(),
                            ptr_region->get_log_index());
            ...
            if (FLAGS_compact_delete_sst_keys > 0) {
                ...
                for (const auto &[sst_file_name, prop]: props) {
                    ...
                    if (FLAGS_compact_delete_sst_keys > 0
                        ...
                        RocksWrapper::get_instance()->compact_files(compact_options, data_cf, {sst_file_name},
                                                                    target_level);
                        compact_sst_cnt++;
                        DB_WARNING(
                            "compact sst file: %s finish, num_deletions: %ld, from level#%d to level#%d, cost: %ld",
                            sst_file_name.c_str(), prop->num_deletions, sst_level, target_level, cost.get_time());
                    }
                }
                DB_WARNING("compact_region_by_sst_delete_keys, compact_sst_cnt: %d cost: %ld", compact_sst_cnt,
                           cost.get_time());
            }
        }
        return;
    ...
    void Store::update_schema_info(const pb::SchemaInfo &table,
                                   ...
                                   std::unordered_map<int64_t, std::unordered_map<int64_t, pb::IndexState> > *
                                   vector_index_map) {
        ...
        for (size_t idx = 0; idx < table.indexs_size(); ++idx) {
            ...
            if (index_info.index_type() == pb::I_VECTOR) {
                if (vector_index_map != nullptr) {
                    (*vector_index_map)[table.table_id()].emplace(
                        std::make_pair(index_info.index_id(), index_info.state()));
                }
            }
        }
    }
    ...
    void Store::construct_heart_beat_request(
        ...
        if (FLAGS_store_rocks_hang_check &&count 
        >
        2
        )
        {
        ...
        instance_info->set_version(KSEARCH_REVISION);
#endif
