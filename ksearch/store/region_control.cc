...
#include <ksearch/raft/raft_control.h>
...
namespace ksearch {
    ...
    int RegionControl::remove_data(int64_t drop_region_id, int64_t drop_index_id) {
        ...
        auto data_cf = rocksdb->get_data_handle();
        if (data_cf == nullptr) {
            DB_WARNING("get rocksdb data column family failed, region_id: %ld, index_id: %ld", drop_region_id,
                       drop_index_id);
            return -1;
        }
        TimeCost cost;
        ...
        DB_WARNING("region clear data, remove_range cost:%ld, region_id: %ld, index_id: %ld", cost.get_time(),
                   drop_region_id, drop_index_id);
        return 0;
    }
    ...
    int RegionControl::remove_cold_index_data(int64_t drop_region_id, int64_t drop_index_id) {
        ...
        auto s = rocksdb->remove_cold_range(begin, end);
        if (!s.ok()) {
            DB_FATAL("region_id: %ld, index_id: %ld delete files in range failed %s", drop_region_id, drop_index_id,
                     s.ToString().c_str());
            return -1;
        }
        DB_WARNING("region_id: %ld, index_id: %ld delete files in range, cost: %ld", drop_region_id, drop_index_id,
                   cost.get_time());
        return 0;
    }
    ...
    int RegionControl::remove_cold_binlog(int64_t drop_region_id) {
        ...
        if (!s.ok()) {
            DB_FATAL("region_id: %ld delete files in range failed, cold_binlog cf %s", drop_region_id,
                     s.ToString().c_str());
            return -1;
        }
    ...
    int RegionControl::remove_expired_offline_data(int64_t region_id, int64_t table_id, int64_t ttl_ts,
                                                   int64_t newest_ts) {
        TimeCost cost;
        auto rocksdb = RocksWrapper::get_instance();
        // cold_data_cf, delete < oldest_ts
        {
            ...
            if (!s.ok()) {
                DB_FATAL(
                    "region_id: %ld, table_id: %ld, ttl_ts: %ld, %s delete cold_data_cf files in range failed %s, cost: %ld",
                    region_id, table_id, ttl_ts, ts_to_datetime_str(ttl_ts).c_str(), s.ToString().c_str(),
                    cost.get_time());
                return -1;
            }
        }
        // cold_data_cf, delete >= newest_ts
        {
            ...
            if (!s.ok()) {
                DB_FATAL(
                    "region_id: %ld, table_id: %ld, newest_ts: %ld, %s delete cold_data_cf files in range failed %s, cost: %ld",
                    region_id, table_id, newest_ts, ts_to_datetime_str(newest_ts).c_str(), s.ToString().c_str(),
                    cost.get_time());
                return -1;
            }
        }
        ...
        // cold_binlog_cf, delete >= newest_ts
        {
            ...
            if (!s.ok()) {
                DB_FATAL("region_id: %ld, newest_ts: %ld, %s delete cold_binlog_cf files in range failed %s, cost: %ld",
                         region_id, ttl_ts, ts_to_datetime_str(newest_ts).c_str(), s.ToString().c_str(),
                         cost.get_time());
                return -1;
            }
        }
        DB_WARNING(
            "binlog region_id: %ld, table_id: %ld, ttl_ts: %ld, %s, newest_ts: %ld, %s delete expired offline data in range, cost: %ld",
            region_id, table_id, ttl_ts, ts_to_datetime_str(ttl_ts).c_str(), newest_ts,
            ts_to_datetime_str(newest_ts).c_str(), cost.get_time());
        return 0;
    }
