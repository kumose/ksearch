...
#include <ksearch/common/backup_stream.h>
...
namespace ksearch {
    ...
    private:
        struct ProgressiveAttachmentWritePolicy {
            ProgressiveAttachmentWritePolicy(butil::intrusive_ptr<brpc::ProgressiveAttachment> attach) : attachment(
                attach) {
            }
    ...
    private:
        ...
        int send_file(BackupInfo &backup_info, WritePolicy *pa, int64_t log_index) {
            ...
            int64_t all_streaming_size = sizeof(int64_t) * 2 + sizeof(int8_t) + backup_info.meta_info.size + (
                                             backup_info.data_info.size == 0
                                                 ? 0
                                                 : (sizeof(int64_t) + backup_info.data_info.size)
                                         );
