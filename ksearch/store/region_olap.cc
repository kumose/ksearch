...
#define IF_DONE_SET_RESPONSE_FATAL(done, errcode, err_message) \
    ...
    bool Region::need_flush_to_cold_rocksdb() {
        ...
        for (auto it = table_ptr->partition_info.range_partition_infos().rbegin();
             it != table_ptr->partition_info.range_partition_infos().rend(); ++it) {
...
*/
    ...
    void print_metadata_info(const rocksdb::LiveFileMetaData &metadata) {
        ...
        if (metadata.column_family_name == RocksWrapper::COLD_BINLOG_CF) {
            ...
            DB_WARNING("LiveFileMetaData[cf_name: %s level: %d];"
                       ...
                       metadata.smallest_seqno, metadata.largest_seqno, metadata.num_reads_sampled,
                       (int) metadata.being_compacted,
                       metadata.num_entries, metadata.num_deletions, metadata.file_creation_time, metadata.name.c_str(),
                       metadata.db_path.c_str(),
                       metadata.relative_filename.c_str(), metadata.directory.c_str(), metadata.file_number,
                       metadata.size);
        } else {
            int64_t smallest_region_id = smallestkey.extract_i64(0);
    ...
    int Region::modify_olap_region_num_table_lines() {
        ...
        DB_WARNING("region_id: %ld num_table_lines: %ld => %ld", get_region_id(), _num_table_lines.load(),
                   estimate_lines);
    ...
    int Region::ingest_cold_sst_on_snapshot_load() {
        ...
        DB_NOTICE("region_id: %ld olap_info: %s cost: %ld", _region_id, olap_info.ShortDebugString().c_str(),
                  cost.get_time());
        return 0;
    }
    ...
    int Region::doing_cold_data_rollup(int64_t index_id) {
        ...
        while (iter->Valid()) {
            ...
            if (0 != record->decode_key(*pk_info, table_key, pos)) {
                DB_WARNING("DDL_LOG table_id:%ld index:%ld region_id:%ld decode key failed", get_table_id(), index_id,
                           _region_id);
                return -1;
            }
            ...
            if (0 != tuple_record.decode_fields(field_ids, record)) {
                DB_WARNING("DDL_LOG table_id:%ld index:%ld region_id:%ld decode value failed", get_table_id(), index_id,
                           _region_id);
                return -1;
            }
            ...
            if (0 != rollup_key.append_index(rollup_index_info, record.get(), -1, false)) {
                DB_FATAL("DDL_LOG table_id:%ld index:%ld region_id:%ld fail to append_index", get_table_id(), index_id,
                         _region_id);
                return -1;
            }
            ...
            if (rollup_record == nullptr) {
                DB_FATAL("DDL_LOG table_id:%ld index:%ld region_id:%ld fail to clone", get_table_id(), index_id,
                         _region_id);
                return -1;
            }
            if (rollup_record->encode_value_for_rollup(rollup_index_info, table_info_ptr->fields,
                                                       table_info_ptr->fields_need_sum) != 0) {
                DB_FATAL("DDL_LOG table_id:%ld index:%ld region_id:%ld fail to encode_rollup", get_table_id(), index_id,
                         _region_id);
                return -1;
            }
            if (0 != rollup_record->encode(rollup_value)) {
                DB_FATAL("DDL_LOG table_id:%ld index:%ld region_id:%ld fail to encode_record", get_table_id(), index_id,
                         _region_id);
                return -1;
            }
            ...
            if (line_nums % 100 == 0) {
                ...
                if (!s.ok()) {
                    DB_FATAL("DDL_LOG table_id:%ld index:%ld region_id:%ld merge fail", get_table_id(), index_id,
                             _region_id);
                    return -1;
                }
                batch.Clear();
        ...
        DB_NOTICE(
            "DDL_LOG doing cold data rollup end, cost: %ld table_id:%ld, index:%ld, region_id: %ld, num_table_lines: %d",
    ...
    int Region::flush_index_to_cold_rocksdb(const pb::OlapRegionInfo &olap_info) {
        ...
        if (olap_info.new_olap_index_info().state() == pb::OLAP_IMMUTABLE) {
            if (!has_index_sst_data(olap_info.new_olap_index_info().index_id(), nullptr)) {
                DB_WARNING("DDL_LOG region_id: %ld no index %ld sst data", _region_id,
            ...
            if (ret < 0) {
                DB_FATAL("DDL_LOG region_id: %ld, index %ld flush_hot_to_cold failed", _region_id,
                         olap_info.new_olap_index_info().index_id());
                return -1;
            }
        }
    ...
    int Region::sync_olap_info(pb::OlapRegionStat state,
        ...
        DB_NOTICE("region_id: %ld sync olap info: %s cost: %ld", _region_id, req.ShortDebugString().c_str(),
                  tc.get_time());
        return 0;
    }
    ...
    int Region::sync_olap_index_info(const pb::OlapRegionInfo &old_olap_info, pb::OlapRegionStat state,
        ...
        DB_NOTICE("region_id: %ld sync olap index info: %s cost: %ld", _region_id, req.ShortDebugString().c_str(),
                  tc.get_time());
        return 0;
    }

    std::string make_make_relative_path_without_filename(const std::string &prefix, int64_t table_id,
                                                         int64_t partition_id) {
        //                                                            partition     
        //  ks_olap/meta_bns/database_name/table_name/table_id/20230301_20230331
    ...
    std::string make_relative_path(int64_t table_id, int64_t partition_id, int64_t region_id, uint64_t size,
                                   uint64_t lines) {
        //                                                            partition     regionid_lines_size_timestamp.extsst
        //  ks_olap/meta_bns/database_name/table_name/table_id/20230301_20230331/26783_1024_1234.extsst
        std::string path = make_make_relative_path_without_filename("ks_olap", table_id, partition_id);
        if (path == "") {
            return "";
        }
        uint32_t now = (uint32_t) time(nullptr); // 暂时使用时间戳，可能发生重复，后续可以改成拿自增id OLAPTODO
        path += "/" + std::to_string(region_id) + "_" + std::to_string(lines) + "_" + std::to_string(size) + "_" +
                std::to_string(now) + ".extsst";
        return path;
    }

    // 执行失败暂不删除
    int copy_file(const std::string &local_file, const std::string &user_define_path, std::string &external_file,
                  uint64_t size) {
        TimeCost cost;
        ...
        if (write_size != length) {
            DB_FATAL("sst_file: %s external_file: %s diff size %ld vs %ld", local_file.c_str(), external_file.c_str(),
                     size, write_size);
            return -1;
        }
        ...
        DB_NOTICE("copy %s to %s size: %ld, cost: %ld", local_file.c_str(), external_file.c_str(), size,
                  cost.get_time());
        return 0;
    }
    ...
    public:
        ColdFileSstWriter(int64_t table_id, int64_t partition_id, int64_t region_id, int64_t index_id,
                          int64_t count_per_sst) : _table_id(table_id), _partition_id(partition_id),
                                                   _region_id(region_id), _index_id(index_id),
                                                   _count_per_sst(count_per_sst) {
    ...
    int ColdFileSstWriter::finish() {
        ...
        DB_NOTICE("local file: %s external_file: %s size: %lu cost: %ld", _tmp_file_name.c_str(), external_file.c_str(),
                  size, _cost.get_time());
        _tmp_file_name.clear();
        return 0;
    }
    ...
    int ColdFileSstWriter::write_kv(const rocksdb::Slice &key, const rocksdb::Slice &value) {
        ...
        if (_writer == nullptr) {
            ...
            _write_count = 0;
            _cost.reset();
            _tmp_file_name = FLAGS_db_path + "_tmp/" + std::to_string(_region_id) + "_" + std::to_string(
            ...
            if (!s.ok()) {
                DB_FATAL("region_id: %ld, index_id: %ld, open sst file: %s fail", _region_id, _index_id,
                         _tmp_file_name.c_str());
                return -1;
            }
        }
    ...
    int Region::flush_hot_to_cold(std::vector<std::string> &external_files,
        ...
        for (int64_t index_id: index_ids) {
            ...
            int64_t total_cnt = 0;
            int64_t fail_cnt = 0;
            ColdFileSstWriter sst_writer(get_table_id(), get_partition_id(), _region_id, index_id,
                                         FLAGS_num_lines_per_sst);
            ...
            for (iter->Seek(prefix); iter->Valid(); iter->Next()) {
                ...
                if (_need_snapshot_filter && 0 != table_info_ptr->snapshot_blacklist.size() && snapshot_desc !=
                    nullptr) {
                    ...
                    if (table_info_ptr->snapshot_blacklist.count(val._u.uint64_val) != 0) {
                        DB_DEBUG("table: %ld, flush hot to cold snapshot_blacklist filter val: %lu", get_table_id(),
                                 val._u.uint64_val);
                        continue;
                    }
                }
            ...
            DB_NOTICE("region_id: %ld flush total_cnt: %ld, fail_cnt: %ld, cost: %ld", _region_id, total_cnt, fail_cnt,
                      cost.get_time());
        }
        return 0;
    }
    ...
    int Region::fits_snapshot_blacklist() {
        ...
        for (auto it = table_info_ptr->partition_info.range_partition_infos().rbegin();
             it != table_info_ptr->partition_info.range_partition_infos().rend(); ++it) {
    ...
    int Region::flush_hot_index_to_cold(int64_t index_id,
        ...
        for (iter->Seek(prefix); iter->Valid(); iter->Next()) {
            ...
            if (_need_snapshot_filter && 0 != table_info_ptr->snapshot_blacklist.size() && snapshot_desc != nullptr) {
                ...
                ExprValue val = record->get_value(snapshot_desc);
                if (table_info_ptr->snapshot_blacklist.count(val._u.uint64_val) != 0) {
                    DB_DEBUG("table: %ld, index: %ld, flush hot to cold snapshot_blacklist filter val: %lu",
                             get_table_id(), index_id, val._u.uint64_val);
                    continue;
                }
            }
        ...
        DB_NOTICE("region_id: %ld, index_id: %ld, flush total_cnt: %ld, fail_cnt: %ld, cost: %ld", _region_id, index_id,
                  total_cnt, fail_cnt, cost.get_time());
        return 0;
    }
    ...
    int Region::manual_link_external_sst() {
        ...
        std::string done_path = make_make_relative_path_without_filename("ks_olap", _table_id, get_partition_id());
        if (done_path.empty()) {
            return -1;
        }
        ...
        if (eof && len > 0 && len < buf_len) {
            ...
            DB_FATAL("read done file: %s failed, len: %ld, buf_len: %u, eof: %d", external_done_file.c_str(), len,
                     buf_len, eof);
            return -1;
        }
