...
namespace ksearch {
    ...
    int IndexSelector::analyze(QueryContext *ctx) {
        ...
        SortNode *sort_node = static_cast<SortNode *>(root->get_last_node(pb::SORT_NODE));
        // 窗口函数场景可能包含多个sort节点，需要获取最后一个
            ...
            case pb::FUNCTION_CALL: {
                ...
                switch (fn_op) {
                    ...
                    case parser::FT_GT: {
                        ...
                        tmp_type = RANGE;
                        if (get_field_hit_type_weight(field_range_map[field_id].type) > get_field_hit_type_weight(
                                tmp_type)) {
                            return;
                        }
    ...
    void IndexSelector::hit_field_or_like_range(ExprNode *expr, std::map<int32_t, FieldRange> &field_range_map,
        ...
        for (auto sub_expr: or_exprs) {
            ...
            if (new_fulltext_flag) {
                ...
                or_node.children.back()->info = std::make_pair(index_ids[index_ids_index++],
                                                               std::move(fulltext_or_range));
    ...
    void IndexSelector::hit_field_range(ExprNode *expr,
        ...
        auto try_add_into_fulltext = [field_id](FulltextInfoNode *index_node, range::FieldRange &&range,
                                                int64_t index_id) {
    ...
    int64_t IndexSelector::index_selector(const std::vector<pb::TupleDescriptor> &tuple_descs,
        ...
        // field_id => range映射
        std::shared_ptr<std::map<int32_t, FieldRange> > field_range_map = std::make_shared<std::map<int32_t,
            FieldRange> >();
        ...
        if (options.join_on_conditions != nullptr) {
            ...
            bool index_predicate_is_null = false;
            hit_field_range(options.join_on_conditions, *field_range_map, &index_predicate_is_null, table_id,
                            fulltext_index_tree.root.get());
        }
        ...
        for (auto index_id: index_ids) {
            ...
            if (index_info.type == pb::I_FULLTEXT && index_info.index_hint_status == pb::IHS_NORMAL && access_path->
                is_possible) {
                if (index_info.fields.size() == 1) {
                    // TODO: 两种类型倒排统一后类型不再需要index_info.storage_type
                    if (fulltext_fields_exact_like_map.count((index_info.fields[0].id << 5) + index_info.storage_type)
                        == 1) {
                        DB_WARNING("skip fulltext index %ld", index_id);
                        continue;
                    } else {
                        fulltext_fields_exact_like_map[(index_info.fields[0].id << 5) + index_info.storage_type] =
                                access_path->is_exact_like;
                    }
                } else {
        ...
        if (pushed_join_on_condition) {
            return scan_node->select_join_index_in_ksearch(sample_sql);
        }
        // 分区表解析分区信息
        ...
        return scan_node->select_index_in_ksearch(sample_sql);
    }
        ...
        if (table_info->partition_ptr != nullptr) {
            ...
            if (partition_type == pb::PT_HASH) {
                if (field_iter != field_range_map.end()
                    && (field_iter->second.type == EQ || field_iter->second.type == IN || field_iter->second.type ==
                        LIKE_EQ)
                ...
                if (field_iter != field_range_map.end()) {
                    if (!field_iter->second.eq_in_values.empty()) {
                        ...
                        for (auto &value: eq_in_values_set) {
                            ...
                            int64_t partition_index = partition_ptr->calc_partition(
                                user_info, value, is_read, &another_partition_id);
                            if (partition_index < 0) {
                                // 未找到partition，则跳过
                                continue;
                            }
                        ...
                        if (partition_ptr->calc_partitions(user_info,
                                                           left_value, left_open, right_value, right_open,
                                                           partition_ids, is_read) != 0) {
                ...
                if (conjunct != nullptr) {
                    ...
                    if (tuple_slot_ids.size() == 1) {
                        ...
                        if (tuple_slot_id.first == tuple_id &&
                            std::find(partition_slot_ids.begin(), partition_slot_ids.end(), tuple_slot_id.second) !=
                            partition_slot_ids.end()) {
    ...
    int64_t IndexSelector::index_merge_selector(const std::vector<pb::TupleDescriptor> &tuple_descs,
        ...
        for (auto or_sub_conjunct: or_sub_conjuncts) {
            ...
            IndexSelectorOptions options;
            index_selector(tuple_descs, scan_node, filter_node, sort_node, join_node, packet_node, agg_node,
                           having_filter_node, nullptr,
                           index_has_null, field_range_type, sample_sql,
                           options); // filter_node's pruned_conjuncts are modified
