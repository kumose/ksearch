...
namespace ksearch {
    ...
    ExecNode *JoinTypeAnalyzer::get_need_pushdown_scan_node(QueryContext *ctx,
        ...
        if (node->node_type() == pb::SCAN_NODE) {
            ...
            return get_need_pushdown_scan_node(ctx, static_cast<JoinNode *>(node)->get_outter_node(), condition,
                                               scannode_ctx, expr_can_pushdown);
            ...
            *scannode_ctx = iter->second.get();
            ...
            return get_need_pushdown_scan_node(dual_scan->sub_query_ctx(), dual_scan->sub_query_node(), condition,
                                               scannode_ctx, expr_can_pushdown);
    ...
    int JoinTypeAnalyzer::pushdown_in_condition_and_choose_index(QueryContext *ctx, ExecNode *node) {
        ...
        ExecNode *scan_node =
                get_need_pushdown_scan_node(ctx, inner_node, &in_condition, &scan_ctx, &expr_can_pushdown);
