...
//            B(wait A)      D (broadcast)
    ...
    int MppAnalyzer::seperate_store_fragment(QueryContext *ctx,
        ...
        // TODO 是否有更好的方法
        if (static_cast<SelectManagerNode *>(select_manager)->mpp_fetcher_global_index(fragment->last_runtime_state) !=
            0) {
    ...
    int MppAnalyzer::seperate_db_fragment(QueryContext *ctx, SmartFragment &fragment, ExecNode *node,
                                          NodePartitionProperty *parent_property) {
        ExchangeReceiverNode *receiver = nullptr;
        ExchangeSenderNode *sender = nullptr;
    ...
    int MppAnalyzer::check_need_add_exchange(NodePartitionProperty *parent_propety,
                                             NodePartitionProperty *node_property, bool *need_add_exchange) {
                ...
                for (auto &hash_columns: node_property->hash_partition_propertys) {
                    if (hash_columns.get() == parent_propety->hash_partition_propertys[0].get()
                        || (hash_columns->type == pb::HashPartitionType
                            && parent_propety->hash_partition_propertys[0]->
                            hash_partition_is_same(hash_columns.get()))) {
                        *need_add_exchange = false;
                        return 0;
                    }
                }
    ...
    int MppAnalyzer::optimize_fragment(QueryContext *ctx, SmartFragment &fragment) {
        ...
        if (parent_fragment != nullptr
            ...
            ExchangeReceiverNode *parent_fragment_receiver = static_cast<ExchangeReceiverNode *>(fragment_sender->
                get_exchange_receiver_node());
