...
namespace ksearch {
    ...
    public:
        ParquetArrowReader(const std::string &full_path, bool is_afs_file) : _full_path(full_path),
                                                                             _is_afs_file(is_afs_file) {
        }
    ...
    public:
        ParquetArrowReadableFile(const std::shared_ptr<ParquetArrowReader> &reader, int64_t size,
                                 ::arrow::MemoryPool *pool = GetMemoryPoolForRead()) : _file_reader(reader),
            _file_size(size), _pool(pool) {
        }
        ...
        static ::arrow::Result<std::shared_ptr<::arrow::io::RandomAccessFile> > Open(
            const std::string &path, int64_t size, bool is_afs_file) {
            ...
            auto file = std::shared_ptr<ParquetArrowReadableFile>(
                new(std::nothrow) ParquetArrowReadableFile(reader, size));
            if (KSEARCH_UNLIKELY(file == nullptr)) {
        ...
        ::arrow::Result<std::shared_ptr<::arrow::Buffer> > Read(int64_t nbytes) override {
            ...
            if (KSEARCH_UNLIKELY(buffer == nullptr)) {
