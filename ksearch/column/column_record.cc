...
namespace ksearch {
    ...
    bool ColumnRecord::TEST_record_batch_diff(const std::shared_ptr<arrow::RecordBatch> &rb1,
                                              const std::shared_ptr<arrow::RecordBatch> &rb2) {
        if (rb1 == nullptr || rb2 == nullptr) {
            return true;
        }
        if (!rb1->schema()->Equals(rb2->schema(), false)) {
            DB_WARNING("schema not equal, schema1: %s, schema2: %s", rb1->schema()->ToString().c_str(),
                       rb2->schema()->ToString().c_str());
            return true;
        }
        ...
        for (int col_idx = 0; col_idx < rb1->num_columns(); ++col_idx) {
            ...
            for (int row_idx = 0; row_idx < rb1->num_rows(); ++row_idx) {
                ...
                if (v1.compare(v2) != 0) {
                    DB_WARNING("column: %d, row: %d, value not equal, v1: %s, v2: %s", col_idx, row_idx,
                               v1.get_string().c_str(), v2.get_string().c_str());
                    return true;
                }
            }
        }
    ...
    std::string ColumnRecord::encode_row_key(std::shared_ptr<arrow::RecordBatch> record_batch, int row_index,
    ...
    std::shared_ptr<ColumnSchemaInfo> ColumnRecord::make_column_schema(int64_t tableid, SmartTable table_info,
                                                                       SmartIndex pri_info,
                                                                       const std::unordered_map<int32_t, FieldInfo *> &
                                                                       field_id2info_map) {
        ...
        for (const auto &field: schema_ptr->key_fields) {
            ...
            if (arrow_type < 0) {
                DB_COLUMN_FATAL("field: %s primitive type:%d to arrow type failed", field.lower_short_name.c_str(),
                                field.type);
                return nullptr;
            }
    ...
    int ColumnRecord::append_value(int field_pos, const ExprValue &value) {
        ...
        if (value.type == pb::NULL_TYPE) {
            TURBO_UNUSED(_builders[field_pos]->AppendNull());
            return 0;
        }
        // 影响性能后续去掉 COLUMNTODO
        ...
        switch (arrow_type) {
            case arrow::Type::type::BOOL:
                s = static_cast<arrow::BooleanBuilder *>(_builders[field_pos].get())->
                        Append(value.get_numberic<bool>());
                break;
            case arrow::Type::type::INT8:
