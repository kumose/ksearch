...
namespace ksearch {
    ...
    struct RecordBatch2 {
        RecordBatch2(const std::shared_ptr<arrow::RecordBatch> &batch, ColumnSchemaInfo *schema) : batch_ptr(batch),
            columns(batch->columns()), schema_info(schema) {
        }
    ...
    public:
        UnOrderSingleRowReader(const std::shared_ptr<arrow::RecordBatchReader> &reader, ColumnSchemaInfo *schema_info,
                               int64_t estimate_size) : _reader(reader), _schema_info(schema_info) {
    ...
    public:
        OrderSingleRowReader(const std::shared_ptr<arrow::RecordBatchReader> &reader,
                             ColumnSchemaInfo *schema_info) : _reader(reader), _schema_info(schema_info) {
        }
    ...
    public:
        explicit SortMerge(const SortMergeOptions &options,
                           const std::vector<std::shared_ptr<SingleRowReader> > &readers) : _options(options),
            _readers(readers), _cache_row_pool(options.schema_info.get(), 2 * readers.size()) {
    ...
    public:
        explicit AceroMerge(const AceroMergeOptions &options,
                            std::vector<std::shared_ptr<arrow::RecordBatchReader> > readers) : _options(options),
            _readers(readers) {
        }
