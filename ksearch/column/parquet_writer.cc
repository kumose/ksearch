...
#include <ksearch/proto/column.pb.h>
#include <ksearch/column/sort_merge.h>
#include <ksearch/column/column_statistics.h>
...
namespace ksearch {
    ...
    arrow::Status ParquetWriter::write_batch(const std::shared_ptr<arrow::RecordBatch> &record_batch,
                                             const RecordBatchInfo &rb_info) {
    ...
    arrow::Status ParquetWriter::execute_do(std::unique_ptr<arrow::RecordBatchReader> input, ColumnCompactionType type,
        ...
        while (true) {
            ...
            rb_info.start_key = ColumnRecord::get_frist_row_key(rb, _options.schema_info->key_fields,
                                                                rb_info.first_userid);
    ...
    arrow::Status SingleFileWriter::create_new_file() {
        ...
        _file_name = FLAGS_db_path + "_tmp/" + std::to_string(file_prefix.fetch_add(1)) + "_" + std::to_string(
                         butil::gettimeofday_us()) + ".parquet";
        ...
        auto parquet_file_writer = parquet::ParquetFileWriter::Open(outfile, schema_node, properties,
                                                                    _key_value_metadata);
    ...
    arrow::Status SingleFileWriter::write_batch(const std::shared_ptr<arrow::RecordBatch> &record_batch,
                                                const RecordBatchInfo &rb_info) {
    ...
    arrow::Status SingleFileWriter::write_rowgroup(
        const std::vector<std::shared_ptr<arrow::RecordBatch> > &record_batchs,
        const std::vector<RecordBatchInfo> &rb_infos) {
        ...
        if (!status.ok()) {
            DB_COLUMN_FATAL("file_name: %s, new buffered rowgroup failed error: %s", _file_name.c_str(),
        ...
        for (int i = 0; i < record_batchs.size(); ++i) {
            ...
            if (!status.ok()) {
                DB_COLUMN_FATAL("file_name: %s, write record batch failed error: %s", _file_name.c_str(),
                                status.message().c_str());
                return status;
            }
        }
    ...
    arrow::Status SingleFileWriter::close() {
        if (_file_writer != nullptr) {
            ...
            _key_value_metadata->Append("KSEARCH_META", value);
