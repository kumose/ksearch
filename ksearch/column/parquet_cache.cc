...
namespace ksearch {
    ...
    ::arrow::Result<int64_t> ParquetArrowReadableFile::ReadAt(int64_t position, int64_t nbytes, void *buffer) {
        ...
        MutTableKey key;
        key.append_i64(contents->region_id).append_i64(contents->start_version).append_i64(contents->end_version).
                append_i32(contents->file_idx).append_i64(position);
        ...
        parquet_cache->get_block_cache()->Insert(key_slice, block2, &BlockContents::kBasicHelper, block2->size,
                                                 nullptr);
    ...
    ParquetArrowReadableFile::read_from_device(int64_t position, int64_t nbytes, void *buffer) {
        if (nbytes == 0) {
            DB_WARNING("read from device, filename: %s, offset: %ld, len: 0", _file_reader->file_name().c_str(),
                       position);
            return 0;
        }
        int64_t r = -1;
        ...
        if (r < 0) {
            DB_COLUMN_FATAL("filename: %s, read offset: %lu, len: %ld failed", _file_reader->file_name().c_str(),
                            position, nbytes);
