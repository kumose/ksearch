...
namespace ksearch {
    ...
    public:
        ...
        int init(bool schema_with_order_info) {
            if (schema_with_order_info) {
                _column_record = std::make_shared<ColumnRecord>(_schema_info->schema_with_order_info,
                                                                _options.read_batch_size);
            } else {
        ...
        int row2col(rocksdb::Slice key, rocksdb::Slice value, ColumnKeyType key_type, int64_t raft_index,
                    int batch_pos);
    ...
    public:
        ...
        int get_raft_log(int64_t start_index, int64_t end_index, uint64_t txn_id,
