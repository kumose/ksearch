...
#include <ksearch/column/parquet_cache.h>
...
namespace ksearch {
    ...
    public:
        ...
        ColumnFileInfo(int64_t _table_id, int64_t _region_id, int64_t _start_version, int64_t _end_version,
                       int file_idx, const ColumnFileMeta &info) {
            file_name = ColumnFileInfo::make_file_name(_table_id, _region_id, _start_version, _end_version, file_idx,
                                                       info.file_size, info.row_count);
            afs_full_name.clear();
        ...
        std::string afs_full_name; // 兼容olap文件在afs上的完整路径。例：afs://yinglong.afs.baidu.com:9902/user/ks/xxxx
        ...
        static std::string make_file_name(int64_t table_id, int64_t region_id, int64_t start_version,
                                          int64_t end_version, int idx, int64_t size, int lines) {
        ...
        void parser_position(const std::vector<int> &row_group_indices,
            ...
            for (int rg_idx: row_group_indices) {
                ...
                for (int col_idx: column_indices) {
                    ...
                    ranges.push_back(ReadRange{
                        col_meta->file_offset() - col_meta->total_compressed_size(), col_meta->total_compressed_size()
                    });
                }
            }
        }
    ...
    public:
        ParquetFileReader(ParquetFileReaderOptions &options, std::shared_ptr<ParquetFile> file) : _options(options),
            _parquet_file(file) {
        }
