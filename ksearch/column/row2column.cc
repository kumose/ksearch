...
namespace ksearch {
    ...
    int Row2ColumnReader::row2col(rocksdb::Slice key, rocksdb::Slice value, ColumnKeyType key_type, int64_t raft_index,
                                  int batch_pos) {
    ...
    int RaftLogReader::init() {
        ...
        log_data_key.append_i64(_options.region_id).append_u8(MyRaftLogStorage::LOG_DATA_IDENTIFY).append_i64(
            _options.start_index);
        MutTableKey prefix;
        MutTableKey end;
        prefix.append_i64(_options.region_id).append_u8(MyRaftLogStorage::LOG_DATA_IDENTIFY);
        end.append_i64(_options.region_id).append_u8(MyRaftLogStorage::LOG_DATA_IDENTIFY).append_i64(
            _options.end_index + 1);
        rocksdb::ReadOptions options;
        ...
        if (_first_index != _options.start_index && _total_row_nums > 0) {
            // 可能丢数据，报警
            DB_COLUMN_FATAL(
                "column read raft log is not start index, region_id: %ld, start_index: %ld, first_index: %ld",
    ...
    int RaftLogReader::get_raft_log(int64_t start_index, int64_t end_index, uint64_t txn_id,
        ...
        for (; iter->Valid(); iter->Next()) {
            ...
            if (request.txn_infos_size() > 0 && txn_id == request.txn_infos(0).txn_id() && request.op_type() ==
                pb::OP_KV_BATCH) {
