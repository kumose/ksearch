...
#include <ksearch/column/file_manager.h>
#include <ksearch/store/meta_writer.h>
...
namespace ksearch {
    ...
    DEFINE_int64(column_fast_base_compaction_interval_s, 1800,
                 "column_fast_base_compaction_interval_s, default(1800s)");
    DEFINE_int64(column_base_compaction_threshold_mb, 256, "column_base_compaction_threshold_mb, default(256MB)");
    DEFINE_int64(column_base_compaction_cumulative_threshold_mb, 128,
                 "column_base_compaction_cumulative_threshold_mb, default(128MB)");
    DEFINE_int64(column_cumulative_file_max_size_mb, 16, "column_cumulative_file_max_size_mb, default(16MB)");
    DEFINE_int64(column_base_compaction_threshold_percent, 10,
                 "column_major_compaction_threshold_percent, default(10%)");
    ...
    void print_parquet_metadata(const std::shared_ptr<::parquet::FileMetaData> &file_metadata,
        ...
        for (int rg_idx = 0; rg_idx < file_metadata->num_row_groups(); ++rg_idx) {
            ...
            for (int32_t col_idx = 0; col_idx < rg_meta->num_columns(); ++col_idx) {
                ...
                os << "rg" << rg_idx << "_col" << col_idx << ": [" << col_meta->file_offset() - col_meta->
    ...
    int ParquetFile::init_kv_metadata(const std::vector<std::string> &keys, const std::vector<std::string> &values) {
        if (keys.size() != values.size()) {
            DB_COLUMN_FATAL("path: %s, kv meta key size diff with value size [%ld, %ld]",
                            _file_info->full_path().c_str(), keys.size(), values.size());
            return -1;
        }
        ...
        for (size_t i = 0; i < keys.size(); ++i) {
            ...
            for (const auto &index_item: column_meta.sparse_index()) {
                if (_sparse_index_map.find(index_item.key()) != _sparse_index_map.end()) {
                    // COLUMNTODO 冲突怎么处理
                    DB_COLUMN_FATAL("path: %s, index key already exist [%d, %ld]", _file_info->full_path().c_str(),
                                    index_item.rowgroup_idx(), index_item.pos_in_rowgroup());
                }
                // DB_WARNING("path: %s, index key size: %ld, rg idx: %d, row idx: %ld", _file_info->full_path().c_str(), index_key.size(), rg_idx, row_idx);
                _sparse_index_map[index_item.key()] = std::make_pair(index_item.rowgroup_idx(),
                                                                     index_item.pos_in_rowgroup());
            }
            return 0;
        }
        DB_COLUMN_FATAL("path: %s, no KSEARCH_META found", _file_info->full_path().c_str());
        return -1;
    }

    bool ParquetFile::check_interval_overlapped(const pb::PossibleIndex::Range &index_range, bool is_eq,
                                                bool is_left_open, bool is_right_open,
    ...
    int ParquetFile::get_qualified_rowgroup_and_rowranges(
        ...
        for (const auto &key_range: possible_index.ranges()) {
            ...
            if (left_rowgroup_idx == right_rowgroup_idx) {
                // 只有一个RowGroup
                std::pair<int64_t, int64_t> rowrange = std::make_pair(left_rowgroup_rowidx.second,
                                                                      right_rowgroup_rowidx.second);
    ...
    int ParquetFileReader::init() {
        ...
        if (_options.pos_index != nullptr && _options.pos_index->ranges_size() > 0) {
            ...
            if (_parquet_file->get_qualified_rowgroup_and_rowranges(*_options.pos_index, row_group_indices, row_ranges)
                != 0) {
    ...
    ::arrow::Status ParquetFileReader::ReadNext(std::shared_ptr<::arrow::RecordBatch> *batch) {
        ...
        if (!s.ok()) {
            DB_COLUMN_FATAL("%s Fail to read next batch: %s", _parquet_file->get_file_path().c_str(),
                            s.message().c_str());
            return s;
        }
        ...
        for (int i = 0; i < schema->num_fields(); ++i) {
            ...
            if (f->name() == RAFT_INDEX_NAME) {
                ...
                if (it == fields.end()) {
                    DB_COLUMN_FATAL("%s Fail to find field: %s", _parquet_file->get_file_path().c_str(),
                                    f->name().c_str());
    ...
    void ColumnFileManager::fileinfo2proto(const std::shared_ptr<ColumnFileInfo> &mem_info,
                                           pb::ColumnFileInfo *pb_info) {
    ...
    void ColumnFileManager::make_snapshot(std::shared_ptr<ColumnFileSet> *column_file_set,
                                          pb::RegionColumnFiles &pb_file_info) {
    ...
    void ColumnFileManager::remove_column_data(pb::ColumnStatus column_status, int64_t max_version) {
        ...
        DB_NOTICE("region_id: %ld, remove column data, status: %d, max_version: %ld", _region_id, column_status,
                  max_version);
    }
    ...
    int ColumnFileManager::fileinfo2proto_unlock() {
        ...
        DB_DEBUG("region_id: %ld, write column file info success, info: %s", _region_id,
                 pb_file_info.ShortDebugString().c_str());
        return 0;
    }
    ...
    void ColumnFileManager::proto2fileinfo_unlock(const pb::RegionColumnFiles &pb_file_info) {
        ...
        DB_NOTICE("region_id: %ld, read column file info success, info: %s", _region_id,
                  pb_file_info.ShortDebugString().c_str());
    }
    ...
    int ColumnFileManager::pick_minor_compact_file(int64_t applied_index, int64_t &start_version) {
        ...
        if (butil::gettimeofday_us() - _last_minor_compact_time < FLAGS_column_minor_compaction_interval_s * 1000 *
            1000LL
    ...
    int ColumnFileManager::pick_major_compact_file(std::vector<std::shared_ptr<ColumnFileInfo> > &file_infos) {
        ...
        if (total_file_cnt < FLAGS_column_major_compaction_minor_interval
            && butil::gettimeofday_us() - _last_major_compact_time < FLAGS_column_major_compaction_minor_interval *
            FLAGS_column_minor_compaction_interval_s * 1000 * 1000LL) {
            return -1;
        }
    ...
    int ColumnFileManager::pick_base_compact_file(std::vector<std::shared_ptr<ColumnFileInfo> > &file_infos,
        ...
        if (butil::gettimeofday_us() - _last_base_compact_time > FLAGS_column_base_compaction_interval_s * 1000 *
            1000LL) {
            need_check_cumulatives_size = false;
        }

        // 非merge on read，及时做base compaction
        if (only_read_base && (butil::gettimeofday_us() - _last_base_compact_time >
                               FLAGS_column_fast_base_compaction_interval_s * 1000 * 1000LL)) {
            need_check_cumulatives_size = false;
        }
    ...
    int ColumnFileManager::finish_minor_compact(const std::shared_ptr<ColumnFileInfo> &new_file,
                                                int64_t last_max_version) {
        std::unique_lock<std::mutex> l(_mutex);
        if (_max_version != last_max_version) {
            DB_WARNING("region_id: %ld, max version %ld not match [%ld vs %ld]", _region_id, new_file->end_version,
                       _max_version, last_max_version);
            return -1;
        }
    ...
    int ColumnFileManager::update_version_only(int64_t max_version, int64_t last_max_version) {
        std::unique_lock<std::mutex> l(_mutex);
        if (_max_version != last_max_version) {
            DB_WARNING("region_id: %ld, max version %ld not match [%ld vs %ld]", _region_id, max_version, _max_version,
                       last_max_version);
            return -1;
        }
        _max_version = max_version;
    ...
    int ColumnFileManager::finish_major_compact(const std::vector<std::shared_ptr<ColumnFileInfo> > &old_files,
                                                const std::vector<std::shared_ptr<ColumnFileInfo> > &new_files,
                                                bool is_base) {
        std::unique_lock<std::mutex> l(_mutex);
        // 检查文件列表是否发生变动
    ...
    void ColumnFileManager::finish_row2column(const std::vector<std::shared_ptr<ColumnFileInfo> > &new_files,
                                              int64_t max_version) {
        std::unique_lock<std::mutex> l(_mutex);
    ...
    void ColumnFileManager::finish_flush_to_cold(const std::vector<std::shared_ptr<ColumnFileInfo> > &new_files,
                                                 int64_t max_version) {
        std::unique_lock<std::mutex> l(_mutex);
        _delete_files.insert(_active_files.begin(), _active_files.end());
        _active_files.clear();
