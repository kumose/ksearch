...
#include <ksearch/column/sort_merge.h>
#include <ksearch/column/region_column.h>
#include <ksearch/column/parquet_writer.h>
#include <ksearch/column/row2column.h>
...
namespace ksearch {
    ...
    DEFINE_int64(column_major_compaction_use_acero_max_rows, 2000000,
                 "column_major_compaction_use_acero_max_rows, default(200w)");
...
#define IF_DONE_SET_RESPONSE(done, errcode, err_message) \
    ...
    int Region::column_on_snapshot_load(const std::string &dir) {
        ...
        if (!pb_file_info.ParseFromString(meta_string)) {
            DB_FATAL("parse from pb fail when read column file info, value:%s",
        ...
        if (_column_mgr.column_status() == pb::CS_COLD || _column_mgr.max_version() >= _applied_index) {
            ...
            for (const auto &file: files) {
                if (writer->add_file(file) != 0) {
                    ...
                    DB_WARNING("Error while adding extra_fs to writer, region_id: %ld, file: %s", _region_id,
                               file.c_str());
                }
            }
            return;
    ...
    void Region::column_snapshot_save() {
        std::unique_ptr<ColumnSnapshotClosure> closure;
        {
            ...
            if (_column_mgr.max_version() < _snapshot_closure->get_raft_index() && _snapshot_closure->get_time() <
                FLAGS_column_snapshot_timeout_s * 1000 * 1000LL) {
        ...
        if (_column_mgr.max_version() < closure->get_raft_index()) {
            DB_WARNING("snapshot time out region_id: %ld, max_version: %ld < snapshot_raft_index: %ld", _region_id,
                       _column_mgr.max_version(), closure->get_raft_index());
            return;
        }
        ...
        for (const auto &file: files) {
            if (writer->add_file(file) != 0) {
                DB_COLUMN_FATAL("Error while adding extra_fs to writer, region_id: %ld, file: %s", _region_id,
                                file.c_str());
                return;
            }
        }
    ...
    int Region::get_column_files(const pb::PossibleIndex &pos_index,
                                 std::vector<std::shared_ptr<ParquetFile> > &files) {
        ...
        for (const auto &[_, info]: column_file_set->column_files) {
            if (pos_index.ranges_size() > 0) {
                ...
                for (const auto &range: pos_index.ranges()) {
                    if (ParquetFile::check_interval_overlapped(range, pos_index.is_eq(), pos_index.left_open(),
                                                               pos_index.right_open(),
    ...
    bool Region::use_userid_statis(
        bool is_eq, const google::protobuf::RepeatedPtrField<pb::PossibleIndex::Range> &key_ranges) {
    ...
    std::vector<std::shared_ptr<ColumnFileInfo> > Region::column_link_files(
        const std::vector<ColumnFileMeta> &file_infos, int64_t min_version, int64_t max_version) {
        std::vector<std::shared_ptr<ColumnFileInfo> > new_files;
        new_files.reserve(file_infos.size());
        int file_idx = 0;
        bool failed = false;
        for (const auto &info: file_infos) {
            auto new_file = std::make_shared<ColumnFileInfo>(get_table_id(), _region_id, min_version, max_version,
                                                             file_idx++, info);
            ...
            if (boost::filesystem::exists(new_path) || !boost::filesystem::exists(old_path)) {
                DB_COLUMN_FATAL("old file: %s or new file: %s exist", info.file_name.c_str(),
                                new_file->full_path().c_str());
                break;
            }
            if (!ParquetFileManager::get_instance()->link_file(info.file_name, new_file->full_path())) {
                DB_COLUMN_FATAL("link file: %s to path: %s failed", info.file_name.c_str(),
                                new_file->full_path().c_str());
                failed = true;
                break;
            } else if (boost::filesystem::file_size(boost::filesystem::path(new_file->full_path())) != info.file_size) {
                DB_COLUMN_FATAL("file: %s size is not equal to file: %s", info.file_name.c_str(),
                                new_file->full_path().c_str());
                failed = true;
                break;
            } else {
    ...
    int Region::column_minor_compact() {
        ...
        std::shared_ptr<ColumnSchemaInfo> schema_info = ColumnRecord::make_column_schema(
            get_table_id(), nullptr, nullptr, field_id2info_map);
        ...
        if (raftlog_reader->put_count() <= 0 && raftlog_reader->delete_count() <= 0 &&
            FLAGS_column_minor_compaction_use_acero) {
            ...
            auto unorder_reader = std::make_shared<UnOrderSingleRowReader>(
                raftlog_reader, schema_info.get(), raftlog_reader->row_count());
        ...
        parquet_options.max_row_group_length = (FLAGS_parquet_rowgroup_max_length / write_batch_length) *
                                               write_batch_length; // 保证row group长度是write_batch_length的整数倍
        ...
        if (file_infos.size() != 1) {
            DB_COLUMN_FATAL("region_id: %ld, compaction generate more than one file size: %ld", _region_id,
                            file_infos.size());
            return -1;
        }
        ...
        DB_NOTICE(
            "region_id: %ld, minor compact success, read lines[%ld, %ld, %ld], write lines: %ld, use_acero_flag: %d, cost: %ld, new_files: %s",
            _region_id, raftlog_reader->row_count(), raftlog_reader->put_count(), raftlog_reader->delete_count(),
            writer->row_count(),
    ...
    void Region::column_major_compact(bool is_base) {
        ...
        DB_NOTICE(
            "region_id: %ld, major compact success, is_base: %d, write lines: %ld, cost: %ld, old files: %s, new files: %s",
    ...
    int Region::sync_column_info(const std::vector<std::shared_ptr<ColumnFileInfo> > &new_column_files) {
        ...
        DB_NOTICE("region_id: %ld sync column info: %s cost: %ld", _region_id, req.ShortDebugString().c_str(),
                  tc.get_time());
        return 0;
    }
    ...
    void Region::column_flush_to_cold() {
        ...
        std::string path = make_make_relative_path_without_filename("ks_column", get_table_id(), get_partition_id());
        if (path.empty()) {
            return;
        }
        for (const auto &[_, old_file]: column_file_set->column_files) {
            ...
            int ret = copy_file(old_file->full_path(), path + "/" + new_file->file_name, new_file->afs_full_name,
                                old_file->file_size);
            if (ret != 0) {
                DB_FATAL("copy file to afs failed, region_id: %ld, old_file:%s", _region_id,
                         old_file->full_path().c_str());
                failed = true;
                break;
            }
