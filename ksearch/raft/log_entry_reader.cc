...
namespace ksearch {
    ...
    int LogEntryReader::read_log_entry(int64_t region_id, int64_t start_log_index, int64_t end_log_index,
        ...
        if (start_log_index > end_log_index) {
            DB_FATAL("region_id:%ld, start_log_index:%ld, end_log_index:%ld", region_id, start_log_index,
                     end_log_index);
            return -1;
        }
        TimeCost cost;
        ...
        for (; iter->Valid(); iter->Next()) {
            ...
            if (store_req.txn_infos_size() > 0) {
                ...
                if (txn_ids.count(txn_id) == 1) {
                    ...
                    DB_WARNING("read txn log entry region_id:%ld, log_index:%ld, txn_id:%ld", region_id, log_index,
                               txn_id);
                }
            }
        }
