...
namespace ksearch {
    ...
    RocksdbReaderAdaptor::RocksdbReaderAdaptor(int64_t region_id,
                                               ...
                                               bool is_meta_reader) : _region_id(region_id),
                                                                      _path(path),
                                                                      _rs(rs),
                                                                      _context(context),
    ...
    ssize_t RocksdbReaderAdaptor::read(butil::IOPortal *portal, off_t offset, size_t size) {
        ...
        if (offset < _context->offset) {
            ...
            // 重置 _context
            if (offset == 0 && _context->offset_update_time.get_time() > FLAGS_snapshot_timeout_min * 60 * 1000 *
                1000ULL) {
                _last_offset = 0;
                _num_lines = 0;
                context_reset();
        ...
        while (count < size) {
            ...
            if (_context->is_meta_sst && _context->iter->key().starts_with(log_index_prefix)) {
                ...
                auto ret = LogEntryReader::get_instance()->read_log_entry(
                    _region_id, log_index, _context->applied_index, txn_id, txn_infos);
                if (ret < 0) {
                    _context->done = true;
                ...
                for (auto &txn_info: txn_infos) {
                    ++key_num;
                    read_size += serialize_to_iobuf(
                        portal, MetaWriter::get_instance()->transcation_pb_key(_region_id, txn_id, txn_info.first));
...
*/
    ...
    braft::FileAdaptor *RocksdbFileSystemAdaptor::open_reader_adaptor(const std::string &path, int oflag,
        ...
        reader->open();
        DB_WARNING(
            "region_id: %ld open reader: path: %s snapshot_index: %ld applied_index: %ld data_index: %ld , time_cost: %ld",
    ...
    bool RocksdbFileSystemAdaptor::open_snapshot(const std::string &path) {
        ...
        DB_WARNING("region_id: %ld, data_index:%ld, binlog_check_point:%ld, open snapshot path: %s",
                   _region_id, _snapshots[path].ptr->data_index, _snapshots[path].ptr->binlog_check_point,
                   path.c_str());
        return true;
    }
