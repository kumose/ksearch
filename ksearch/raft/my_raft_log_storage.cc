...
#include <ksearch/raft/can_add_peer_setter.h>
...
namespace ksearch {
    ...
    MyRaftLogStorage::MyRaftLogStorage(
        ...
        rocksdb::ColumnFamilyHandle *binlog_handle) : _first_log_index(0),
                                                      _last_log_index(0),
    ...
    braft::LogStorage *MyRaftLogStorage::new_instance(const std::string &uri) const {
        ...
        braft::LogStorage *instance = new(std::nothrow) MyRaftLogStorage(
            region_id, rocksdb, raftlog_handle, binlog_handle);
        if (instance == NULL) {
    ...
    int MyRaftLogStorage::_build_key_value(
        ...
        if (_is_binlog_region &&binlog_value
