...
namespace ksearch {
    ...
    public:
        ...
        virtual std::string to_sql(const std::unordered_map<int32_t, std::string> &slotid_fieldname_map,
                                   ks::client::MysqlShortConnection *conn) override {
            std::string res;
            res += "(";
            for (int i = 0; i < _children.size(); ++i) {
    ...
    public:
        virtual ExprValue get_value(MemRow *row) {
            ...
            for (int i = 0; i < children_size(); i++) {
                ExprValue val = _children[i]->get_value(row);
                if (!val.is_null() && val.get_numberic<bool>() == true) {
                    // short-circuit
                    return ExprValue::True();
                }
                if (val.is_null()) {
    ...
    public:
        ...
        virtual int transfer_to_arrow_expression() {
            ...
            _arrow_expr = arrow::compute::not_(arrow_cast(_children[0]->arrow_expr(), _children[0]->col_type(),
                                                          pb::BOOL));
            return 0;
        }
