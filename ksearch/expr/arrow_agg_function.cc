...
#include <ksearch/expr/agg_fn_call.h>
...
namespace ksearch {
    ...
    arrow::compute::HashAggregateKernel MakeKernel(arrow::compute::InputType argument_type,
                                                   arrow::compute::KernelInit init,
                                                   const bool ordered = false) {
        return MakeKernel(
            arrow::compute::KernelSignature::Make(
                {std::move(argument_type), arrow::compute::InputType(arrow::Type::UINT32)},
    ...
    struct AvgIntermediateImpl final : public GroupedAggregator {
        ...
        arrow::Status Merge(GroupedAggregator &&raw_other, const arrow::ArrayData &group_id_mapping) override {
            ...
            for (uint32_t other_g = 0; static_cast<int64_t>(other_g) < group_id_mapping.length; ++other_g, ++g) {
                if (other->avgs_[other_g].count == 0) {
                    // null
                    continue;
                }
    ...
    template<typename T>
    arrow::Result<std::unique_ptr<arrow::compute::KernelState> > GroupedAvgIntermidateInit(
    ...
    template<typename T>
    arrow::Result<std::unique_ptr<arrow::compute::KernelState> > GroupedAvgFinalizeInit(
        arrow::compute::KernelContext *ctx,
        const arrow::compute::KernelInitArgs &args) {
    ...
    static auto kCountOptionsType =
            arrow::compute::internal::GetFunctionOptionsType<GroupConcatOptions>(
                arrow::internal::DataMember("separator_", &GroupConcatOptions::separator_));
    ...
    arrow::Status ArrowFunctionManager::RegisterAllHashAggFunction() {
        auto registry = arrow::compute::GetFunctionRegistry();
        {
            ...
            auto func = std::make_shared<arrow::compute::HashAggregateFunction>(
                "hash_avg_intermediate", arrow::compute::Arity::Binary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());

            ARROW_RETURN_NOT_OK(AddHashAggKernels({arrow::boolean()}, GroupedAvgIntermediateFactory::Make, func.get()));
            ARROW_RETURN_NOT_OK(
                AddHashAggKernels(arrow::SignedIntTypes(), GroupedAvgIntermediateFactory::Make, func.get()));
            ARROW_RETURN_NOT_OK(
                AddHashAggKernels(arrow::UnsignedIntTypes(), GroupedAvgIntermediateFactory::Make, func.get()));
            ARROW_RETURN_NOT_OK(
                AddHashAggKernels({arrow::float32(), arrow::float64()}, GroupedAvgIntermediateFactory::Make, func.get()
                ));
            ARROW_RETURN_NOT_OK(
                AddHashAggKernels({arrow::large_binary()}, GroupedAvgIntermediateFactory::Make, func.get()));
            ARROW_RETURN_NOT_OK(registry->AddFunction(std::move(func)));
        }
        {
            // avg中间结果聚合成double
            auto func = std::make_shared<arrow::compute::HashAggregateFunction>(
                "hash_avg_finalize", arrow::compute::Arity::Binary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            ARROW_RETURN_NOT_OK(
                AddHashAggKernels(arrow::BaseBinaryTypes(), GroupedAvgFinalizeFactory::Make, func.get()));
            // 一阶段agg产出中间结果
            ARROW_RETURN_NOT_OK(registry->AddFunction(std::move(func)));
        }
        {
            ...
            auto func = std::make_shared<arrow::compute::HashAggregateFunction>(
                "group_concat_intermediate", arrow::compute::Arity::Binary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty(), &default_group_concat_options);
            ARROW_RETURN_NOT_OK(
                func->AddKernel(MakeKernel(arrow::compute::InputType::Any(), HashAggregateInit<
                    GroupeConcatIntermediateImpl>)));
            ARROW_RETURN_NOT_OK(registry->AddFunction(std::move(func)));
        }
        return arrow::Status::OK();
    }
