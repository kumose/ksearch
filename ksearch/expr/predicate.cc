...
namespace ksearch {
        ...
        if (_is_row_expr) {
            ...
            for (size_t j = 0; j < _col_size; j++) {
                if (children(0)->children(j)->col_type() == pb::UINT64
                    ...
                    arrow::compute::Expression cast_int64_first = arrow::compute::call(
                        "cast", {fields->children(j)->arrow_expr()},
    ...
    int LikePredicate::open_by_re2() {
        ...
        std::unordered_set<int32_t> slot_ids;
        children(1)->get_all_slot_ids(slot_ids);
        _option.set_encoding(re2::RE2::Options::EncodingLatin1);;
    ...
    int LikePredicate::transfer_to_arrow_expression() {
        ...
        if (is_string(_children[0]->col_type())) {
            ...
            args.emplace_back(arrow::compute::call("cast", {_children[0]->arrow_expr()},
        ...
        if (_fn.fn_op() == parser::FT_EXACT_LIKE) {
            ...
            arrow::compute::MatchSubstringOptions opt(children(1)->get_value(nullptr).get_string(), /*ignore_case*/
                                                      false);
