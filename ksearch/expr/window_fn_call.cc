...
#include <ksearch/expr/window_fn_call.h>
        ...
        switch (_window_type) {
            ...
            case LAG: {
                ...
                if (_children.size() > 1) {
                    ...
                    window_intermediate->offset = _children[1]->get_value(nullptr).cast_to(pb::INT64).get_numberic<
                        int64_t>();
        ...
        switch (_window_type) {
            ...
            case DENSE_RANK: {
                ...
                if (_mem_row_compare->need_not_compare()) {
                    ...
                    if (window_intermediate->cur_idx == 1) {
                        ...
                        MemRow *last_row = window_intermediate->cur_batch->get_row(window_intermediate->cur_idx - 2).
                                get();
                        MemRow *cur_row = window_intermediate->cur_batch->get_row(window_intermediate->cur_idx - 1).
                                get();
            ...
            case PERCENT_RANK: {
                ...
                if (_mem_row_compare->need_not_compare()) {
                    ...
                    if (num_rows == 1) {
                        ...
                        expr_value._u.double_val =
                                static_cast<double>(window_intermediate->last_rank - 1) / (num_rows - 1);
                    }
                }
                break;
            }
            case CUME_DIST: {
                ...
                if (_mem_row_compare->need_not_compare()) {
                    ...
                    while (true) {
                        ...
                        MemRow *last_row = window_intermediate->cur_batch->get_row(window_intermediate->last_rank).
                                get();
            ...
            case LAG: {
                ...
                if (idx >= 0 && idx < window_intermediate->cur_batch->size()) {
                    expr_value =
                            _children[0]->get_value(window_intermediate->cur_batch->get_row(idx).get()).cast_to(
                                _col_type);
                } else {
                    if (window_intermediate->default_expr != nullptr) {
                        ...
                        if (cur_idx < 0 || cur_idx >= window_intermediate->cur_batch->size()) {
                            DB_WARNING("Invalid cur_idx: %ld, size: %lu", cur_idx,
                                       window_intermediate->cur_batch->size());
                            return -1;
                        }
