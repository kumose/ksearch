...
namespace ksearch {
    ...
    int arrow_concat_ws(std::vector<ExprNode *> &children, pb::Function *fn,
        ...
        /// A null in any input results in a null in the output. binary_join_element_wise要求args类型都一样, large_binary
        auto option = std::make_shared<arrow::compute::JoinOptions>(
            arrow::compute::JoinOptions::NullHandlingBehavior::SKIP);
        out = arrow::compute::call("binary_join_element_wise", args, option);
        return 0;
    }
    ...
    int arrow_substr(std::vector<ExprNode *> &children, pb::Function *fn,
        ...
        out = arrow::compute::call("binary_slice", {children[0]->arrow_expr()},
    ...
    int arrow_upper(std::vector<ExprNode *> &children, pb::Function *fn,
        ...
        out = arrow::compute::call("ks_upper", {children[0]->arrow_expr()});
        return 0;
    }
    ...
    int arrow_lower(std::vector<ExprNode *> &children, pb::Function *fn,
        ...
        out = arrow::compute::call("ks_lower", {children[0]->arrow_expr()});
        return 0;
    }
    ...
    int arrow_substring_index(std::vector<ExprNode *> &children, pb::Function *fn,
        ...
        RETURN_NULL_IF_COLUMN_SATISFY_COND(
            children[1]->col_type() == pb::NULL_TYPE || children[2]->col_type() == pb::NULL_TYPE);
        ...
        out = arrow::compute::call("ks_substring_index", {children[0]->arrow_expr()}, std::move(option));
        return 0;
    }
    ...
    arrow::Status ArrowFunctionManager::RegisterAllStringFunction() {
        ...
        {
            auto upper = std::make_shared<arrow::compute::ScalarFunction>("ks_upper", arrow::compute::Arity::Unary(),
            ...
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::BaseBinaryTypes()) {
                ARROW_RETURN_NOT_OK(
                    upper->AddKernel({in_ty}, arrow::large_binary(),
                                     arrow::compute::internal::GenerateVarBinaryToVarBinary<UpperTransfer,
                                         arrow::LargeBinaryType>(*in_ty)));
            }
            ARROW_RETURN_NOT_OK(registry->AddFunction(upper));
        }
        {
            auto lower = std::make_shared<arrow::compute::ScalarFunction>("ks_lower", arrow::compute::Arity::Unary(),
                                                                          /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::BaseBinaryTypes()) {
                ARROW_RETURN_NOT_OK(
                    lower->AddKernel({in_ty}, arrow::large_binary(),
                                     arrow::compute::internal::GenerateVarBinaryToVarBinary<LowerTransfer,
                                         arrow::LargeBinaryType>(*in_ty)));
            }
            ARROW_RETURN_NOT_OK(registry->AddFunction(lower));
        }
        {
            auto substring_index = std::make_shared<arrow::compute::ScalarFunction>(
                "ks_substring_index", arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::BaseBinaryTypes()) {
                ARROW_RETURN_NOT_OK(
                    substring_index->AddKernel({in_ty}, arrow::large_binary(),
                                               arrow::compute::internal::GenerateVarBinaryToVarBinary<SubstringIndex,
                                                   arrow::LargeBinaryType>(*in_ty),
                                               InitCommonState));
            }
