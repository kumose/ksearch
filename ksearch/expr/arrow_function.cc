...
namespace ksearch {
    ...
    public:
        ...
        Copy(const arrow::compute::FunctionOptions &options) const override {
    ...
    int build_arrow_expr_with_cast(ExprNode *node, const pb::PrimitiveType &col_type, bool force_same) {
        ...
        arrow::Expression cast_expr = arrow::compute::call("cast", {node->arrow_expr()},
                                                           arrow::compute::CastOptions::Unsafe(arrow_type));
        node->set_arrow_expr(cast_expr);
        return 0;
    }
    ...
    void build_row_expr_range_compare_expression(std::vector<ExprNode *> &children, const std::string &op,
        ...
        for (size_t i = 0; i < children[0]->children_size(); i++) {
            ...
            if (i == 0) {
                ...
                sub_exprs.emplace_back(
                    arrow::compute::and_(equal_expr, arrow::compute::call(last_param_op, {left, right})));
                break;
            } else {
    ...
    int arrow_if_null(std::vector<ExprNode *> &children, pb::Function *fn, const pb::PrimitiveType &return_type,
        ...
        out = arrow::compute::call("if_else", {
                                       arrow::compute::call("is_null", {children[0]->arrow_expr()}),
                                       children[1]->arrow_expr(),
                                       children[0]->arrow_expr()
                                   });
        return 0;
    ...
    int arrow_case_expr_when(std::vector<ExprNode *> &children, pb::Function *fn, const pb::PrimitiveType &return_type,
        ...
        for (size_t i = 0; i < (children.size() - 1) / 2; ++i) {
            ...
            predicates.emplace_back(arrow::compute::call("equal", {
                                                             children[0]->arrow_expr(),
                                                             children[predicate_value_idx]->arrow_expr()
                                                         }));
        }
        case_when_arguments.emplace_back(arrow::compute::call("make_struct", predicates));
        // values, cast to return type
    ...
    int arrow_cast_to_string(std::vector<ExprNode *> &children, pb::Function *fn, const pb::PrimitiveType &return_type,
        ...
        if (is_datetime_specic(children[0]->col_type())
            ...
            out = arrow::compute::call("cast", {children[0]->arrow_expr()},
    ...
    struct ExecExprValueToDate {
        ...
        static arrow::Status ExecStringInput(arrow::compute::KernelContext *ctx, const arrow::compute::ExecSpan &batch,
                                             arrow::compute::ExecResult *out) {
            ExprValueCastState *state = static_cast<ExprValueCastState *>(ctx->state());
            pb::PrimitiveType type = state->type;
            const arrow::ArraySpan &input = batch[0].array;
    ...
    arrow::Status ExecMurmurHash(arrow::compute::KernelContext *ctx, const arrow::compute::ExecSpan &batch,
                                 arrow::compute::ExecResult *out) {
        const arrow::ArraySpan &input = batch[0].array;
        arrow::ArraySpan *out_data = out->array_span_mutable();
    ...
    arrow::Status ArrowFunctionManager::RegisterAllDefinedFunction() {
         ...
         */
        {
            auto expr_value_to_string_func = std::make_shared<arrow::compute::ScalarFunction>(
                "expr_value_to_string", arrow::compute::Arity::Unary(),
            ...
            arrow::compute::ScalarKernel k_float_to_string({arrow::float32()}, arrow::large_binary(),
                                                           ExecExprValueToString<arrow::LargeBinaryType,
                                                               arrow::FloatType>::Exec, InitExprValueCast);
            // double -> string, 精度统一
            arrow::compute::ScalarKernel k_double_to_string({arrow::float64()}, arrow::large_binary(),
                                                            ExecExprValueToString<arrow::LargeBinaryType,
                                                                arrow::DoubleType>::Exec, InitExprValueCast);
         ...
         */
        {
            auto expr_value_to_date_func = std::make_shared<arrow::compute::ScalarFunction>(
                "expr_value_to_date", arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::IntTypes()) {
         ...
         */
        {
            auto expr_value_to_datetime_func = std::make_shared<arrow::compute::ScalarFunction>(
                "expr_value_to_datetime", arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::IntTypes()) {
            ...
            arrow::compute::ScalarKernel k_binary_to_datetime({arrow::binary()}, arrow::uint64(),
                                                              ExecExprValueToDateTime<arrow::UInt64Type,
                                                                  arrow::BinaryType>::ExecStringInput,
                                                              InitExprValueCast);
         ...
         */
        {
            auto expr_value_to_time_func = std::make_shared<arrow::compute::ScalarFunction>(
                "expr_value_to_time", arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::IntTypes()) {
            ...
            arrow::compute::ScalarKernel k_binary_to_time({arrow::binary()}, arrow::int32(),
                                                          ExecExprValueToTime<arrow::Int32Type,
                                                              arrow::BinaryType>::ExecStringInput, InitExprValueCast);
         ...
         */
        {
            auto expr_value_to_timestamp_func = std::make_shared<arrow::compute::ScalarFunction>(
                "expr_value_to_timestamp", arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const std::shared_ptr<arrow::DataType> &in_ty: arrow::IntTypes()) {
            ...
            arrow::compute::ScalarKernel k_binary_to_timestamp({arrow::binary()}, arrow::uint32(),
                                                               ExecExprValueToTimeStamp<arrow::UInt32Type,
                                                                   arrow::BinaryType>::ExecStringInput,
                                                               InitExprValueCast);
            ...
            arrow::compute::ScalarKernel k_utf8_to_timestamp({arrow::utf8()}, arrow::uint32(),
                                                             ExecExprValueToTimeStamp<arrow::UInt32Type,
                                                                 arrow::StringType>::ExecStringInput,
                                                             InitExprValueCast);
         ...
         */
        {
            ...
            for (const std::shared_ptr<arrow::DataType> &out_ty: arrow::NumericTypes()) {
                ...
                auto string_cast_numberic_func = std::make_shared<arrow::compute::ScalarFunction>(
                    func_name, arrow::compute::Arity::Unary(),
                    /*doc=*/arrow::compute::FunctionDoc::Empty());
            ...
            auto string_cast_bool_func = std::make_shared<arrow::compute::ScalarFunction>(
                func_name, arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
            for (const auto &in_ty: arrow::BaseBinaryTypes()) {
                arrow::compute::ArrayKernelExec exec = arrow::compute::internal::GenerateVarBinaryBase<
                    arrow::compute::internal::applicator::ScalarUnaryNotNull,
         ...
         */
        {
            auto arrow_murmur_hash = std::make_shared<arrow::compute::ScalarFunction>(
                "murmur_hash", arrow::compute::Arity::Unary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
        ...
        */
        {
            auto arrow_round = std::make_shared<arrow::compute::ScalarFunction>(
                "arrow_round", arrow::compute::Arity::Binary(),
                /*doc=*/arrow::compute::FunctionDoc::Empty());
