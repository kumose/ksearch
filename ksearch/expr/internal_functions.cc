...
namespace ksearch {
    ...
    DEFINE_string(db_version, "5.7.16-Ksearch-v"KSEARCH_REVISION, "db version");
#else
    DEFINE_string(db_version, "5.7.16-Ksearch", "db version");
#endif
    ...
    */
    ...
    ExprValue json_object(const std::vector<ExprValue> &input) {
        ...
        for (size_t i = 0; i < input.size(); i += 2) {
            obj.AddMember(rapidjson::StringRef(input[i].get_string().c_str()),
                          rapidjson::StringRef(input[i + 1].get_string().c_str()), obj.GetAllocator());
        }
        rapidjson::StringBuffer buffer;
        rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
        obj.Accept(writer);
...
*/
    ...
    ExprValue rb_contains_range(const std::vector<ExprValue> &input) {
        ...
        if (input.size() != 3 || !input[0].is_bitmap()) {
            ...
            tmp._u.bool_val = input[0]._u.bitmap->containsRange(input[1].get_numberic<uint32_t>(),
                                                                input[2].get_numberic<uint32_t>());
        }
        return tmp;
    }
    ...
    ExprValue soundex(const std::vector<ExprValue> &input) {
        ...
        while (i < len && size < 3) {
            ...
            if (code.empty()) {
                if (bytes == 1 && std::tolower(c) >= 'a' and std::tolower(c)
                <=
                'z'
                )
                {
