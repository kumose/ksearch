...
#include <ksearch/common/hll_common.h>
...
namespace ksearch {
                ...
                if (!is_merger) {
                    ...
                    if (_agg_type == MULTI_COUNT_DISTINCT) {
                        ...
                        std::sort(value_list.begin(), value_list.end(),
                                  [is_asc](const std::string &s1, const std::string &s2) {
                                      if (s1.size() == s2.size()) {
                                          return s1 < s2;
                                      } else {
    ...
    template<typename T>
    int AggFnCall::multi_distinct_unserialize_numberic(char *type_reader, char *end, ExprValueUniqSet &set,
    ...
    int AggFnCall::multi_distinct_unserialize_string(char *type_reader, char *end, ExprValueUniqSet &set,
                                                     const pb::PrimitiveType &col_type) {
        while (type_reader < end) {
        ...
        if (_children[i]->is_slot_ref()
            ...
            if (need_cast_string_to_double
                && (_children[i]->is_slot_ref()
                    ...
                    || (_children[i]->arrow_expr().type() != nullptr && arrow::is_base_binary_like(
                            _children[i]->arrow_expr().type()->id())))) {
                generate_projection_exprs.emplace_back(
                    arrow_cast(_children[i]->arrow_expr(), _children[i]->col_type(), pb::DOUBLE));
            } else {
    ...
    int AggFnCall::transfer_to_arrow_avg(std::vector<arrow::compute::Aggregate> &aggs,
        ...
        if (is_merge && !_is_distinct) {
            ...
            for (int i = 0; i < _children.size(); ++i) {
                if (0 != build_agg_argument(i, true, args, generate_projection_exprs,
    ...
    int AggFnCall::transfer_to_arrow_group_concat(std::vector<arrow::compute::Aggregate> &aggs,
        ...
        if (is_merge && !_is_distinct) {
            ...
            if (first_row_expr_children.size() == 1
                ...
                arrow::compute::Expression concat_expr = arrow::compute::call(
                    "binary_join_element_wise", concat_args, option);
    ...
    int AggFnCall::transfer_to_arrow_agg_function(std::vector<arrow::compute::Aggregate> &aggs,
        ...
        if (_agg_type == GROUP_CONCAT) {
            return transfer_to_arrow_group_concat(aggs, is_merge, generate_projection_exprs,
                                                  generate_projection_exprs_names);
        }
        ...
        if (is_merge && !_is_distinct) {
            ...
            for (int i = 0; i < _children.size(); ++i) {
                if (0 != build_agg_argument(i, need_check_type, args, generate_projection_exprs,
                                            generate_projection_exprs_names)) {
                    DB_FATAL("build agg argument fail, id: %d", i);
                    return -1;
                }
