...
namespace ksearch {
     ...
     * @return 操作是否成功
     */
        static int process_response(const std::vector<TsoRequestTask> &requests, const pb::TsoResponse &response,
                                    int ret);
    ...
    private:
        TsoProxy() : _current_timer_id(0),
                     _tso_request_count_per_batch_avg("tso_request_count_per_batch_avg", &_tso_request_count_per_batch,
                                                      1),
