...
#include <ksearch/common/ks_heartbeat.h>
#include <ksearch/common/proto_process.hpp>
...
namespace ksearch {
    ...
    DEFINE_string(ks_resource_tag, "", "resource tag");
    DECLARE_int32 (ks_port);
    DEFINE_int32(ks_port, 28282, "Server port");

    void KsHeartBeat::construct_heart_beat_request(pb::KsHeartBeatRequest &request, bool is_backup, bool is_binlog) {
        ...
        auto schema_read_recallback = [&request, factory](const SchemaMapping &schema) {
            for (auto &info_pair: schema.table_info_mapping) {
                const int64_t meta_id = ::ksearch::get_meta_id(info_pair.second->id);
                if (meta_id != 0) {
                    // 跳过非主Meta数据
                    continue;
                }
        ...
        TaskFactory<pb::RegionDdlWork>::get_instance()->construct_heartbeat(request,
                                                                            &pb::KsHeartBeatRequest::add_region_ddl_works);
        TaskFactory<pb::DdlWorkInfo>::get_instance()->construct_heartbeat(request,
                                                                          &pb::KsHeartBeatRequest::add_ddl_works);
        request.set_can_do_ddlwork(is_backup ? false : FLAGS_can_do_ddlwork);
        //定时将request当中消息体更新 等待process_ks_heartbeat处理更新至TableManager中的virtual_Index_sql_map
        ...
        for (auto &it1: index_id_name_map) {
            const int64_t meta_id = ::ksearch::get_meta_id(it1.first);
            if (meta_id != 0) {
                // 跳过非主Meta数据
                continue;
            }
        ...
        if (!is_backup && !is_binlog) {
            pb::KsStatus *status = request.mutable_ks_status();
            std::string address = SchemaFactory::get_instance()->get_address();
            status->set_address(address);
            status->set_resource_tag(FLAGS_ks_resource_tag);
            // 检查是否迁移
    ...
    // 处理非主Meta的心跳
    void KsHeartBeat::construct_heart_beat_request(
        ...
        for (auto &dblink_info: dblink_infos) {
            ...
            pb::KsHeartBeatRequest &request = meta_request_map[meta_id];
            pb::KsHeartBeatTable *heartbeat_table = request.add_heartbeat_tables();
        ...
        for (auto &kv: meta_request_map) {
            ...
            pb::KsHeartBeatRequest &request = kv.second;
            ...
            // 非主Meta虚拟索引信息
            for (auto &it1: index_id_name_map) {
                const int64_t meta_id_tmp = ::ksearch::get_meta_id(it1.first);
    ...
    void KsHeartBeat::process_heart_beat_response(
        const pb::KsHeartBeatResponse &response, bool is_backup, const int64_t meta_id) {
        SchemaFactory *factory = nullptr;
        if (is_backup) {
            factory = SchemaFactory::get_backup_instance();
        } else {
            ...
            TaskFactory<pb::RegionDdlWork>::get_instance()->process_heartbeat(response,
                                                                              static_cast<
                                                                                  ...
                                                                                  google::protobuf::RepeatedPtrField<
                                                                                      pb::RegionDdlWork>& (
                                                                                      pb::KsHeartBeatResponse::*)()
                                                                                  const
                                                                              >(&
                                                                                  pb::KsHeartBeatResponse::region_ddl_works));
            TaskFactory<pb::DdlWorkInfo>::get_instance()->process_heartbeat(response,
                                                                            static_cast<
                                                                                const google::protobuf::RepeatedPtrField
                                                                                <pb::DdlWorkInfo>& (
                                                                                    pb::KsHeartBeatResponse::*)() const
                                                                            >(&pb::KsHeartBeatResponse::ddl_works));

            if (response.ks_status_size() != 0) {
                std::vector<pb::KsStatus> address_status_vec;
                for (int i = 0; i < response.ks_status_size(); ++i) {
                    pb::KsStatus db_status = response.ks_status(i);
    ...
    void KsHeartBeat::process_heart_beat_response_sync(
        const pb::KsHeartBeatResponse &response, const int64_t meta_id) {
        TimeCost cost;
        SchemaFactory *factory = SchemaFactory::get_instance();
        if (response.has_last_updated_index() &&
            response.last_updated_index() > factory->last_updated_index(meta_id)) {
    ...
    int BaseKsHeartBeat::init() {
        ...
        _is_inited = true;
        DB_NOTICE("Succ to init BaseKsHeartBeat");

        return 0;
    }

    int BaseKsHeartBeat::heartbeat(bool is_sync) {
        TimeCost cost;
        ...
        for (const auto &full_table: _table_names) {
            ...
            if (heartbeat_table == nullptr) {
                DB_WARNING("ks_heartbeat_table is nullptr");
                return -1;
            }
    ...
    void BaseKsHeartBeat::report_heartbeat() {
        while (!_shutdown) {
            heartbeat(false);
    ...
    bool BinlogNetworkServer::init() {
        ...
        pb::KsHeartBeatResponse response;
        //1、构造心跳请求
        KsHeartBeat::construct_heart_beat_request(request, false, true);
        request.set_can_do_ddlwork(false);
        request.set_need_heartbeat_table(true);
    ...
    void BinlogNetworkServer::process_heart_beat_response(const pb::KsHeartBeatResponse &response) {
    ...
    int BinlogNetworkServer::update_table_infos() {
        ...
        for (SmartTable table: table_ptrs) {
            if (table->binlog_id != _binlog_id && table->binlog_ids.count(_binlog_id) == 0) {
                DB_WARNING("table[%s.%s] has different binlog id %ld", table->namespace_.c_str(), table->name.c_str(),
                           table->binlog_id);
                continue;
            } else {
    ...
    bool BinlogNetworkServer::process_heart_beat_response_sync(const pb::KsHeartBeatResponse &response) {
        DB_DEBUG("response %s", response.ShortDebugString().c_str());
        TimeCost cost;
        SchemaFactory *factory = SchemaFactory::get_instance();
        SchemaVec sv;
