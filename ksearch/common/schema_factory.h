...
#include <ksearch/common/statistics.h>
...
namespace ksearch {
    ...
    inline size_t double_buffer_table_region_erase(std::unordered_map<int64_t, TableRegionPtr> &table_region_map,
                                                   int64_t table_id) {
    ...
    public:
        ...
        int64_t calc_partitions(
            ...
            auto range_match_fn = [right_open](const ExprValue &left_value_tmp, const ExprValue &right_value_tmp,
                                               const ExprValue &range_left_value,
                                               const ExprValue &range_right_value) -> bool {
                // 分区右端点为开区间
    ...
    // DBLink表使用，用于db侧存储 ksMeta和meta_id的映射关系
    struct MetaMap {
    ...
    // addresses里都是在 ks_faulty_interval_times * ks_heartbeat_interval_us
    // 状态为NORMAL的 ks 实例
    struct KsAddresses {
    ...
    using DoublBufferedDBAddresses = butil::DoublyBufferedData<KsAddresses>;
    ...
    public:
        ...
        void get_all_table_by_db(const std::string &namespace_, const std::string &db_name,
        ...
        int get_disable_indexs(std::vector<std::string> &index_names) {
            ...
            for (auto &index_info: index_info_mapping) {
                const int64_t meta_id = ::ksearch::get_meta_id(index_info.first);
                if (meta_id != 0) {
                    continue;
                }
        ...
        int get_binlog_regions(int64_t binlog_id, int64_t partition_index,
                               std::map<int64_t, pb::RegionInfo> &region_infos);
        ...
        void update_virtual_index_info(const int64_t virtual_index_id, const std::string &virtual_index_name,
        ...
        int64_t get_ksearch_alive_time_us() {
            return _ksearch_restart_time.get_time();
        }
        ...
        // DBLINK表使用，获取主Meta上的所有DBLink Ksearch表，用于心跳交互
            ...
            auto func = [&dblink_infos](const SchemaMapping &schema) {
                for (auto &info_pair: schema.table_info_mapping) {
                    ...
                    if (info_pair.second->engine != pb::DBLINK ||
                        info_pair.second->dblink_info.type() != pb::LT_KSEARCH) {
                        continue;
                    }
        ...
        void update_valiable_addresses(const std::vector<pb::KsStatus> &ks_status_vec);
    ...
    private:
        ...
        int update_valiable_addresses_internal(KsAddresses &db_addresses,
        ...
        //记录ksearch模块的起始时间用于计算启动时长
        TimeCost _ksearch_restart_time;
