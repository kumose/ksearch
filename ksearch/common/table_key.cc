...
namespace ksearch {
    TableKey::TableKey(const MutTableKey &key) : _full(key.get_full()),
                                                 _data(key.data()) {
    }
    ...
    std::string TableKey::decode_start_key_string(const std::vector<pb::PrimitiveType> &types,
    ...
    int TableKey::decode_field(Message *message,
        ...
        switch (field_info.type) {
            ...
            case pb::INT32: {
                if (pos + sizeof(int32_t) > size()) {
                    DB_WARNING("int32_t pos out of bound: %d %d %ld %zu", field->number(), pos, pos + sizeof(int32_t),
                               size());
                    return -2;
                }
