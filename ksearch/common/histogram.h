...
#include <ksearch/runtime/sorter.h>
...
namespace ksearch {
    ...
    typedef std::map<ExprValue, std::shared_ptr<BucketInfo>, std::function<bool(const ExprValue &, const ExprValue &)> >
    HistogramMap;

    // 列直方图，处理单列中直方图桶操作
    class Histogram {
    public:
        Histogram(pb::PrimitiveType type, int32_t field_id, int32_t distinct_cnt, int32_t null_value_cnt) : _type(type),
            _field_id(field_id), _distinct_cnt(distinct_cnt), _null_value_cnt(null_value_cnt),
        ...
        int32_t calc_scalar(const ExprValue &lower, const ExprValue &upper,
            ...
            if (lower.compare(bucket_ptr->start) <= 0 && upper.compare(bucket_ptr->end) >= 0) {
                ...
                ratio = calc_fraction(bucket_ptr->start, bucket_ptr->end, upper) - calc_fraction(
                            bucket_ptr->start, bucket_ptr->end, lower);
                ratio = ratio > 1.0 ? 1.0 : ratio;
                ratio = ratio < distinct_1 ? distinct_1 : ratio;
                count = ceil(bucket_ptr->bucket_size * ratio);
            }
    ...
    public:
        PacketSample(std::vector<std::shared_ptr<RowBatch> > &batch_vector,
                     ...
                     pb::TupleDescriptor *tuple_desc) : _order_exprs(slot_order_exprs), _tuple_desc(tuple_desc) {
