...
#include <ksearch/sqlparser/utils.h>
#include <ksearch/common/tdigest.h>
#include <ksearch/common/datetime.h>
#include <ksearch/common/type_utils.h>
...
namespace ksearch {
    struct ExprValue {
        ...
        explicit ExprValue(pb::PrimitiveType type_ = pb::NULL_TYPE) : type(type_) {
            ...
            if (type_ == pb::BITMAP) {
                _u.bitmap = new(std::nothrow) roaring::Roaring();
            ...
            switch (type) {
                ...
                case pb::BITMAP: {
                    ...
                    if (value.string_val().size() > 0) {
                        try {
                            *_u.bitmap = roaring::Roaring::readSafe(value.string_val().data(),
        ...
        ExprValue &cast_to(pb::PrimitiveType type_) {
            ...
            switch (type_) {
                ...
                case pb::BITMAP: {
                    ...
                    if (str_val.size() > 0) {
                        try {
                            *_u.bitmap = roaring::Roaring::readSafe(str_val.c_str(), str_val.size());
                        } catch (...) {
                            DB_WARNING("bitmap read from string failed");
                        }
                    }
     ...
     */
        int64_t compare(const ExprValue &other) const {
            ...
            switch (type) {
                ...
                case pb::INT64:
                    return _u.int64_val > other._u.int64_val ? 1 : (_u.int64_val < other._u.int64_val ? -1 : 0);
                case pb::UINT8:
                ...
                case pb::UINT64:
                case pb::DATETIME:
                    return _u.uint64_val > other._u.uint64_val ? 1 : (_u.uint64_val < other._u.uint64_val ? -1 : 0);
                case pb::FLOAT:
                    return _u.float_val > other._u.float_val ? 1 : (_u.float_val < other._u.float_val ? -1 : 0);
                case pb::DOUBLE:
                    return _u.double_val > other._u.double_val ? 1 : (_u.double_val < other._u.double_val ? -1 : 0);
                case pb::STRING:
                case pb::HEX:
                case pb::JSON:
     ...
     */
        ...
        int convert_charset(const pb::Charset &from_charset, const pb::Charset &to_charset) {
            ...
            if (ksearch::convert_charset(from_charset, str_val, to_charset, new_str_val) != 0) {
                return -1;
            }
        ...
        bool is_string() const {
            return ::ksearch::is_string(type);
        }

        bool is_double() const {
            return ::ksearch::is_double(type);
        }

        bool is_int() const {
            return ::ksearch::is_int(type);
        }

        bool is_uint() const {
            return ::ksearch::is_uint(type);
        }
        ...
        void dt_cast_len() {
            if (float_precision_len >= 0 and float_precision_len
            <=
            6
            )
            {
