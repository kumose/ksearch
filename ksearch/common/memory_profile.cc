...
namespace ksearch {
    ...
    MemTracker::MemTracker(uint64_t log_id, int64_t bytes_limit, MemTracker *parent) : _log_id(log_id),
        ...
        while (!_shutdown) {
            ...
            _mem_tracker_pool.traverse_with_key_value(
                [&need_erase](const uint64_t &log_id, SmartMemTracker &mem_tracker) {
                    if (butil::gettimeofday_us() - mem_tracker->last_active_time() > FLAGS_mem_tracker_gc_interval_s *
                        1000 * 1000LL) {
                        need_erase[log_id] = mem_tracker;
                    }
                });
            for (auto &iter: need_erase) {
                if (butil::gettimeofday_us() - iter.second->last_active_time() > FLAGS_mem_tracker_gc_interval_s * 1000
                    * 1000LL) {
