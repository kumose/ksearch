...
namespace ksearch {
    ...
    public:
        ...
        int query(const std::string &sql, ks::client::ResultSet *result_set, bool store = true);

        // @brief 获取当前连接错误码
        int get_error_code();

        // @brief 获取当前连接，如果连接不存在，则会新创建连接
        ks::client::MysqlShortConnection *get_connection();

        // @brief 对字符串进行转义
        static std::string mysql_escape_string(ks::client::MysqlShortConnection *conn, const std::string &value);

    private:
        std::unique_ptr<ks::client::MysqlShortConnection> fetch_connection();

    private:
        pb::MysqlInfo _mysql_info;
        std::unique_ptr<ks::client::MysqlShortConnection> _mysql_conn;
    };
} // namespace ksearch