...
namespace ksearch {
    ...
    int del_meta_info(pb::KsSchemaHeartBeat &ks_schema_heartbeat) {
        if (ks_schema_heartbeat.has_table_id()) {
            ks_schema_heartbeat.set_table_id(get_del_meta_id(ks_schema_heartbeat.table_id()));
        }
        return 0;
    }

    int del_meta_info(pb::KsHeartBeatTable &ks_heartbeat_table) {
        if (ks_heartbeat_table.has_namespace_name()) {
            ks_heartbeat_table.set_namespace_name(get_del_meta_name(ks_heartbeat_table.namespace_name()));
        }
        if (ks_heartbeat_table.has_table_id()) {
            ks_heartbeat_table.set_table_id(get_del_meta_id(ks_heartbeat_table.table_id()));
        }
        return 0;
    }
