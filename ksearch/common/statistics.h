...
#include <ksearch/common/hll_statistics.h>
    ...
    private:
        void init_histogram(const pb::Histogram &histogram) {
            ...
            for (auto &column: histogram.column_infos()) {
                auto ptr = std::make_shared<Histogram>(column.col_type(), column.field_id(), column.distinct_cnt(),
                                                       column.null_value_cnt());
