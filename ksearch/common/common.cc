...
namespace ksearch {
    ...
    DEFINE_int32(ks_heartbeat_concurrency, 10, "ks heartbeat concurrency, default:10");
    ...
    DEFINE_int32(ks_heartbeat_interval_us, 10 * 1000 * 1000, "ks_heartbeat_interval(us)");
    ...
    void update_schema_conf_common(const std::string &table_name, const pb::SchemaConf &schema_conf,
    ...
    std::string store_or_db_bns_to_meta_bns(const std::string &bns) {
        ...
        if (bns_group.find("ksStore") != bns_group.npos || bns_group.find("ksBinlog") != bns_group.npos) {
            is_store_bns = true;
        }
        ...
        for (const std::string &instance: instances) {
            ...
            re2::RE2 reg(".*(group.*ksMeta.*all).*", option);
            meta_bns.clear();
