...
namespace ksearch {
    ...
    public:
        ...
        MutTableKey(rocksdb::Slice key, bool full = true) : _full(full),
                                                            _data(key.data_, key.size_) {
        }

        MutTableKey(const std::string &key, bool full) : _full(full),
                                                         _data(key.data(), key.size()) {
        }

        MutTableKey(MutTableKey &&key) noexcept : _full(key.get_full()),
                                                  _data(std::move(key._data)) {
        }

        MutTableKey(const MutTableKey &key) : _full(key.get_full()),
                                              _data(key._data) {
        }
        ...
        MutTableKey &replace_i32(int32_t val, size_t pos) {
            // cstore
            uint32_t encode = KeyEncoder::to_endian_u32(KeyEncoder::encode_i32(val));
    ...
    public:
        TableKeyPair(const std::string &left_key, bool left_full,
                     const std::string &right_key, bool right_full) : _left_key(left_key, left_full),
                                                                      _right_key(right_key, right_full) {
        }
