...
#define KSEARCH_LIKELY(x)   __builtin_expect(!!(x), 1)
#define KSEARCH_UNLIKELY(x) __builtin_expect(!!(x), 0)
    ...
    public:
        explicit ConcurrencyBthread(int concurrency) : _concurrency(concurrency) {
        }

        ConcurrencyBthread(int concurrency, const bthread_attr_t *attr) : _concurrency(concurrency),
                                                                          _attr(attr) {
        }
    ...
    class ScopeGuard {
    public:
        explicit ScopeGuard(std::function<void()> exit_func) : _exit_func(exit_func) {
        }
...
#define SAFE_DELETE(p) { if(p){delete(p);  (p)=NULL;} }
    ...
    public:
        ...
        bool check_and_update_incremental(std::function<void(const T &)> update_incremental,
                                          int64_t &last_updated_index, const int64_t applied_index) {
    ...
    public:
        ...
        SlowQueryInfo(uint64_t log_id_,
                      ...
                      bool status_) : log_id(log_id_), sign(sign_), start_time(start_time_), end_time(end_time_),
                                      exec_time(exec_time_), filtered_rows(filtered_rows_),
                                      affected_rows(affected_rows_), scan_rows(scan_rows_), return_rows(return_rows_),
                                      ip(ip_),
                                      resource_tag(resource_tag_), user_name(user_name_), family(family_),
                                      table_name(table_name_), sql(sql_), status(status_) {
        }
    ...
    public:
        ...
        BvarSlowQueryMap &operator+=(const BvarSlowQueryMap &other) {
            ...
            if (other.internal_slow_query_map.size() == 1 && internal_slow_query_map[other_slow_query_map.begin()->
                    first].size() > FLAGS_limit_slow_sql_size) {
                return *this;
            }
    ...
    public:
        ...
        BvarMap(const std::string &key, int64_t index_id, int64_t table_id, int64_t cost, int64_t err_cost,
            ...
            internal_map[key][index_id] = SumCount(table_id, cost, err_cost, 1, err_count, affected_rows,
                                                   scan_rows, read_disk_size, filter_rows, region_count,
                                                   field_range_type_, parent_sign, subquery_signs, tag);
        }
    ...
    inline std::ostream &operator<<(std::ostream &os, const BvarMap &bm) {
        for (auto &pair: bm.internal_map) {
            for (auto &pair2: pair.second) {
                os << pair.first << " : " << pair2.first << " : " << pair2.second.sum << "," << pair2.second.count <<
                        std::endl;
            }
        }
        return os;
    ...
    public:
        ...
        VirtualIndexMap(const int64_t virtual_index_id, const std::string &virtual_index_name,
    ...
    public:
        ...
        explicit LatencyOnly(time_t window_size) : _latency_window(&_latency, window_size) {
        }
    ...
    extern void update_schema_conf_common(const std::string &table_name, const pb::SchemaConf &schema_conf,
                                          pb::SchemaConf *p_conf);
    ...
    extern void parse_sample_sql(const std::string &sample_sql, std::string &database, std::string &table,
                                 std::string &sql);
    ...
    struct IconvEncoding {
        inline static constexpr const char *get_encoding_string() {
            switch (To) {
                case pb::GBK:
                    return ErrMode == IconvOnError::IGNORE
                               ? "gb18030//IGNORE"
                               : ErrMode == IconvOnError::TRANSLIT
                                     ? "gb18030//TRANSLIT"
                                     : "gb18030";
                case pb::UTF8:
                    return ErrMode == IconvOnError::IGNORE
