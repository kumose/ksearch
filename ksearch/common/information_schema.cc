...
#include <ksearch/expr/scalar_fn_call.h>
...
namespace ksearch {
    ...
    void InformationSchema::init_binlog_region_infos() {
        ...
        _calls[table_id] = [table_id, this](RuntimeState *state, std::vector<ExprNode *> &conditions) ->
            std::vector<SmartRecord> {
                    ...
                    std::unordered_map<int64_t, std::unordered_map<int64_t, std::vector<pb::RegionInfo> > >
                            table_id_partition_binlogs;
                    std::unordered_map<int64_t, std::unordered_map<int64_t, std::vector<pb::StoreRes> > >
                            table_id_to_query_info;
                    std::vector<std::vector<std::string> > result_rows;
                    SchemaFactory::get_instance()->get_partition_binlog_regions(
                        binlog_table_name, input_partition_id, table_id_partition_binlogs);
    ...
    void InformationSchema::query_regions_concurrency(
        std::unordered_map<int64_t, std::unordered_map<int64_t, std::vector<pb::StoreRes> > > &table_id_to_binlog_info,
        std::unordered_map<int64_t, std::unordered_map<int64_t, std::vector<pb::RegionInfo> > > &
        partition_binlog_region_infos) {
 ...
 */
    void InformationSchema::process_binlogs_region_info(std::vector<std::vector<std::string> > &result_rows,
                                                        std::unordered_map<int64_t,
                                                            std::unordered_map<int64_t, std::vector<pb::StoreRes> > > &
                                                        table_id_to_query_info) {
        ...
        _calls[table_id] = [table_id](RuntimeState *state, std::vector<ExprNode *> &conditions) ->
            std::vector<SmartRecord> {
                    ...
                    if (!database_name.empty() && !table_name.empty()) {
                        int64_t condition_table_id = 0;
                        if (factory->get_table_id(namespace_ + "." + database_name + "." + table_name,
                                                  condition_table_id) != 0) {
                            return records;
                        }
                        ...
                        auto func = [&condition_table_ids, &condition_table_id_db_map, &condition_table_id_tbl_map, &
                    ...
                    for (int64_t condition_table_id: condition_table_ids) {
                        ...
                        for (const auto &info: table->partition_info.range_partition_infos()) {
                            table_partition_name_cold[table->id][info.partition_id()] = {
                                info.partition_name(), info.is_cold()
                            };
                        }
                    }
                    ...
                    for (const auto &pair: instance_region_ids_map) {
                        ...
                        auto func = [store_addr, region_ids, table_id, &condition_table_id_db_map, &
                                    condition_table_id_tbl_map,
                                    &table_partition_name_cold, &region_id_table_id_map, &region_id_partition_id_map, &
                                    records, &lock]() {
                            pb::RegionIds req;
                            pb::StoreRes res;
                            ...
                            DB_WARNING("store_addr: %s, req_size: %d, res_size: %d", store_addr.c_str(),
                                       req.region_ids_size(), res.extra_res().infos_size());
                            ...
                            for (const auto &info: res.extra_res().infos()) {
                                ...
                                record->set_string(record->get_field_by_name("table_name"),
                                                   condition_table_id_tbl_map[region_id_table_id_map[info.
                                                       region_id()]]);
                                ...
                                if (iter != table_partition_name_cold.end()) {
                                    ...
                                    if (iter2 != iter->second.end()) {
                                        record->set_string(record->get_field_by_name("partition_name"),
                                                           iter2->second.first);
                                        record->set_int64(record->get_field_by_name("partition_is_cold"),
                                                          iter2->second.second);
                                    }
                                }
                                ...
                                record->set_string(record->get_field_by_name("olap_state"),
                                                   pb::OlapRegionStat_Name(info.olap_state()));
                                std::string files;
                        ...
                        auto func = [store_addr, region_ids, table_id, &condition_table_id_db_map, &
                            ...
                            for (const auto &info: res.extra_res().infos()) {
                                ...
                                for (auto &olap_index_info: info.olap_index_info_list()) {
                                    auto record = SchemaFactory::get_instance()->new_record(table_id);
                                    record->set_string(record->get_field_by_name("database_name"),
                                                       condition_table_id_db_map[region_id_table_id_map[info.
                                                           region_id()]]);
                                    record->set_string(record->get_field_by_name("table_name"),
                                                       condition_table_id_tbl_map[region_id_table_id_map[info.
                                                           region_id()]]);
                                    ...
                                    const std::string &rollup_name = SchemaFactory::get_instance()->get_index_name(
                                        olap_index_info.index_id());
                                    record->set_int64(record->get_field_by_name("rollup_id"),
                                                      olap_index_info.index_id());
                                    record->set_string(record->get_field_by_name("rollup_name"), rollup_name);
                                    record->set_string(record->get_field_by_name("rollup_state"),
                                                       pb::OlapRegionStat_Name(olap_index_info.state()));
                                    std::string files;
                    ...
                    for (const auto &table_info: tb_vec) {
                        ...
                        if (table_info->partition_info.has_dynamic_partition_attr()) {
                            ...
                            if (dynamic_partition_attr.has_time_unit()) {
                                record->set_string(record->get_field_by_name("time_unit"),
                            ...
                            if (dynamic_partition_attr.has_start()) {
                                record->set_string(record->get_field_by_name("start"),
                                                   std::to_string(dynamic_partition_attr.start()));
                            } else {
                                record->set_string(record->get_field_by_name("start"), "NULL");
                            }
                            if (dynamic_partition_attr.has_cold()) {
                                record->set_string(record->get_field_by_name("cold"),
                                                   std::to_string(dynamic_partition_attr.cold()));
                            } else {
                                record->set_string(record->get_field_by_name("cold"), "NULL");
                            }
                            if (dynamic_partition_attr.has_end()) {
                                record->set_string(record->get_field_by_name("end"),
                                                   std::to_string(dynamic_partition_attr.end()));
                            } else {
                                record->set_string(record->get_field_by_name("end"), "NULL");
                            }
                            if (dynamic_partition_attr.has_isolation()) {
                                record->set_string(record->get_field_by_name("isolation"),
                            ...
                            if (table_info->partition_info.has_primary_range_partition_type()) {
                                record->set_string(record->get_field_by_name("primary_range_partition_type"),
                                                   pb::RangePartitionType_Name(
                                                       table_info->partition_info.primary_range_partition_type()));
                            } else {
                            ...
                            if (type_size > 0) {
                                ...
                                for (size_t i = 0; i < type_size; ++i) {
                                    types += pb::RangePartitionType_Name(
                                        table_info->partition_info.gen_range_partition_types(i));
                                    types += ",";
                                }
                                types.pop_back();
                            ...
                            if (table_info->schema_conf.has_olap_pre_split_cnt() && table_info->schema_conf.
                                olap_pre_split_cnt() > 0) {
                                record->set_int64(record->get_field_by_name("pre_split_cnt"),
                                                  table_info->schema_conf.olap_pre_split_cnt());
                            } else {
                                record->set_int64(record->get_field_by_name("pre_split_cnt"), 0);
                            }
                            if (table_info->schema_conf.has_use_column_storage() && table_info->schema_conf.
                                use_column_storage()) {
                            ...
                            if (table_info->schema_conf.has_enable_column_engine() && table_info->schema_conf.
                            ...
                            if (table_info->schema_conf.has_cold_use_column_only() && table_info->schema_conf.
                            ...
                            if (table_info->schema_conf.has_force_column_storage() && table_info->schema_conf.
                            ...
                            if (table_info->is_range_partition) {
                                RangePartition *partition_ptr = static_cast<RangePartition *>(table_info->partition_ptr.
                                    get());
                            ...
                            if (table_info->is_range_partition) {
                                ...
                                if (ranges.empty()) {
                                    ...
                                    record->set_string(record->get_field_by_name("start_date"),
                                                       ranges[0].left_value.get_string());
                                    ...
                                    if (watt_start_date.empty()) {
                                        ...
                                        record->set_string(record->get_field_by_name("watt_start_date"),
                                                           watt_start_date);
                                    }
                                }
                                ...
                                if (has_month_partition && dynamic_partition_attr.has_time_unit() &&
                                    dynamic_partition_attr.time_unit() == "DAY") {
                    ...
                    for (const auto &table_id_name_partitions_pair: table_id_name_partitions) {
                        for (const auto &name_partitions_pair: table_id_name_partitions_pair.second) {
                            ...
                            for (const auto &partition: name_partitions_pair.second) {
                                auto record = SchemaFactory::get_instance()->new_record(table_id);
                                record->set_string(record->get_field_by_name("table_name"), table_name);
                                record->set_int64(record->get_field_by_name("table_id"),
                                                  table_id_name_partitions_pair.first);
                        ...
                        for (auto &field: table_info->fields) {
                            ...
                            if (field.default_expr_value.type != pb::NULL_TYPE) {
                                if (field.default_value == "(current_timestamp())") {
                                    record->set_string(record->get_field_by_name("COLUMN_DEFAULT"),
                                                       "CURRENT_TIMESTAMP");
                                } else {
                                    record->set_string(record->get_field_by_name("COLUMN_DEFAULT"),
                                                       field.default_value);
                            ...
                            record->set_string(record->get_field_by_name("DATA_TYPE"),
                                               to_mysql_type_string(field.type));
                            switch (field.type) {
                                case pb::STRING:
                            ...
                            record->set_string(record->get_field_by_name("COLUMN_TYPE"),
                                               to_mysql_type_full_string(
                                                   field.type, field.float_total_len, field.float_precision_len));
                            ...
                            if (field_index.count(field.id) == 0) {
                                ...
                                record->set_string(record->get_field_by_name("COLUMN_KEY"),
                                                   boost::algorithm::join(index_types, "|"));
                            }
                            ...
                            record->set_string(record->get_field_by_name("EXTRA"),
                                               boost::algorithm::join(extra_vec, "|"));
                            record->set_string(record->get_field_by_name("PRIVILEGES"),
                        ...
                        for (auto &index_id: table_info->indices) {
                            ...
                            for (auto &field: index_info.fields) {
                                ...
                                record->set_string(record->get_field_by_name("CONSTRAINT_NAME"),
                                                   index_type == pb::I_PRIMARY ? "PRIMARY" : "name_key");
                                record->set_string(record->get_field_by_name("TABLE_CATALOG"), "def");
                                record->set_string(record->get_field_by_name("TABLE_SCHEMA"), db);
                                record->set_string(record->get_field_by_name("TABLE_NAME"), table_info->short_name);
                                record->set_string(record->get_field_by_name("COLUMN_NAME"), field.short_name);
        ...
        _calls[table_id] = [table_id](RuntimeState *state, std::vector<ExprNode *> &conditions) ->
            std::vector<SmartRecord> {
                    ...
                    for (auto &table_info: tb_vec) {
                        ...
                        record->set_string(record->get_field_by_name("TABLE_SCHEMA"), db);
                        record->set_string(record->get_field_by_name("TABLE_NAME"), table_info->short_name);
                        record->set_string(record->get_field_by_name("TABLE_TYPE"),
                                           table_info->is_view ? "VIEW" : "BASE TABLE");
                        ...
                        record->set_int64(record->get_field_by_name("AVG_ROW_LENGTH"),
                                          table_info->byte_size_per_record);
    ...
    void InformationSchema::init_virtual_index_influence_info() {
        ...
        _calls[table_id] = [table_id](RuntimeState *state, std::vector<ExprNode *> &conditions) ->
            std::vector<SmartRecord> {
                    ...
                    //3.取出response中的影响面信息
                    auto &virtual_index_info_sqls = response.virtual_index_influence_info();
                    //virtual_index_info   and   affected_sqls
                    if (state->client_conn() == nullptr) {
                        return records;
                    }
                    std::string namespace_ = state->client_conn()->user_info->namespace_;
                    ...
                    auto func = [&records, &blacklist_table, &special_signs](const SmartTable &table) -> bool {
                        for (auto sign: table->sign_blacklist) {
                            ...
                            if (special_signs.count(sign) > 0 && special_signs[sign] != "") {
                                uint64_t out[2];
                                butil::MurmurHash3_x64_128(special_signs[sign].c_str(), special_signs[sign].size(),
                                                           0x1234, out);
                                record->set_string(record->get_field_by_name("sign_without_resource_tag"),
                                                   std::to_string(out[0]));
            ...
            std::vector<SmartRecord> {
                    ...
                    auto func = [&records, &exec_type_table, &special_signs](const SmartTable &table) -> bool {
                        for (auto sign_index: table->sign_exec_type) {
                            ...
                            record->set_string(record->get_field_by_name("sign"),
                                               std::to_string(sign) + ": " + explain_type_to_str(exec_type));
                            if (special_signs.count(sign) > 0 && special_signs[sign] != "") {
                                uint64_t out[2];
                                butil::MurmurHash3_x64_128(special_signs[sign].c_str(), special_signs[sign].size(),
                                                           0x1234, out);
            ...
            std::vector<SmartRecord> {
                    ...
                    for (const auto &db_id: db_id_list) {
                        ...
                        if (db_info.has_charset) {
                            record->set_string(record->get_field_by_name("DEFAULT_CHARSET_SET_NAME"),
                                               pb::Charset_Name(db_info.charset));
                        }
                        if (db_info.byte_size_per_record > 0) {
                            record->set_int64(record->get_field_by_name("AVG_ROW_LENGTH"),
                                              db_info.byte_size_per_record);
                        }
                        ...
                        if (db_info.region_split_lines > 0) {
                            record->set_int64(record->get_field_by_name("REGION_SPLIT_LINES"),
                                              db_info.region_split_lines);
                        }
                        ...
                        if (!db_info.main_logical_room.empty()) {
                            record->set_string(record->get_field_by_name("MAIN_LOGICAL_ROOM"),
                                               db_info.main_logical_room);
                        }
                        if (!db_info.partition_info_str.empty()) {
                            record->set_string(record->get_field_by_name("PARTITION_INFO_STR"),
                                               db_info.partition_info_str);
                        }
                        ...
                        if (!learner_resource_tag_str.empty()) {
                            record->set_string(record->get_field_by_name("LEARNER_RESOURCE_TAGS"),
                                               learner_resource_tag_str);
                        }
                        ...
                        if (db_info.binlog_infos.size() > 0) {
                            ...
                            for (size_t i = 0; i < db_info.binlog_infos.size(); ++i) {
                                ...
                                if (db_info.binlog_infos[i].has_partition_is_same_hint()) {
                                    ...
                                    binlog_infos_inner_str += db_info.binlog_infos[i].partition_is_same_hint()
                                                                  ? "true"
                                                                  : "false";
                                    binlog_infos_inner_str += ",";
                                }
