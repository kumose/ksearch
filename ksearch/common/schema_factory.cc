...
namespace ksearch {
    ...
    void SchemaFactory::update_valiable_addresses(const std::vector<pb::KsStatus> &ks_status_vec) {
        std::function < int(KsAddresses &, const std::vector<pb::KsStatus> &) > update_func =
                std::bind(&SchemaFactory::update_valiable_addresses_internal, this, std::placeholders::_1,
                          std::placeholders::_2);
        _doubly_valiable_addresses.Modify(update_func, ks_status_vec);
    }
    ...
    void SchemaFactory::set_db_unavailable(const std::string &db_address) {
        ...
        std::vector<pb::KsStatus> ks_status_vec;
        pb::KsStatus faulty_db;
        faulty_db.set_address(db_address);
        faulty_db.set_status(pb::FAULTY);
        ks_status_vec.emplace_back(faulty_db);
        std::function < int(KsAddresses &, const std::vector<pb::KsStatus> &) > update_func =
                std::bind(&SchemaFactory::update_valiable_addresses_internal, this, std::placeholders::_1,
                          std::placeholders::_2);
        _doubly_valiable_addresses.Modify(update_func, ks_status_vec);
    ...
    int SchemaFactory::update_valiable_addresses_internal(KsAddresses &db_addresses,
                                                          const std::vector<pb::KsStatus> &ks_status_vec) {
        ...
        // meta的leader宕机后新leader短时间可能没有完整 ks instance状态信息
        for (const auto &it: ks_status_vec) {
            // 删除返回状态为FAULTY的 ks 实例，其余更新last_updated_timestamp
    ...
    int SchemaFactory::update_table_internal(SchemaMapping &background, const pb::SchemaInfo &table) {
        ...
        for (int idx = 0; idx < field_cnt; ++idx) {
            ...
            if (proto_type == -1) {
                DB_FATAL("field_name: %s, mysql_type %d not supported.", field.field_name().c_str(),
                         field.mysql_type());
                return -1;
            }
        ...
        if (table.has_ttl_duration()) {
            ...
            if (table.has_ttl_field() && !table.ttl_field().field_name().empty()
                ...
                auto iter = std::find_if(tbl_info.fields.begin(), tbl_info.fields.end(),
                                         [ttl_field_name](const FieldInfo &field_info) {
                                             return field_info.short_name == ttl_field_name;
                                         });
    ...
    void SchemaFactory::update_show_db(const DataBaseVec &db_infos) {
        ...
        for (auto db_info: db_infos) {
            ...
            if (db_info.namespace_name() == "INTERNAL" && db_info.database() == "ksearch") {
                //忽略INTERNAL.ksearch
                continue;
            }
            DatabaseInfo info;
    ...
    void SchemaFactory::update_statistics(const StatisticsVec &statistics) {
        ...
        for (auto &st: statistics) {
            ...
            DB_WARNING("update statistics, table_id:%ld, version:%ld,size:%ld", ptr->table_id(), ptr->version(),
                       st.ByteSizeLong());
        }
    ...
    double SchemaFactory::get_histogram_ratio(int64_t table_id, int field_id, const ExprValue &lower,
                                              const ExprValue &upper) {
        DoubleBufferedTable::ScopedPtr table_ptr;
        if (_double_buffer_table.Read(&table_ptr) != 0) {
            DB_WARNING("read double_buffer_table error.");
            return 1.0;
    ...
    void SchemaFactory::table_with_statistics_info(std::vector<std::string> &database_table) {
        ...
        for (auto &st: table_statistics_mapping) {
            const int64_t meta_id = ::ksearch::get_meta_id(st.first);
            if (meta_id != 0) {
                continue;
            }
    ...
    std::vector<SmartTable> SchemaFactory::get_table_list(std::string namespace_, UserInfo *user) {
        ...
        for (auto &table_pair: table_ptr->table_info_mapping) {
            const int64_t meta_id = ::ksearch::get_meta_id(table_pair.first);
            if (meta_id != 0) {
                continue;
            }
            auto &table_info = table_pair.second;
    ...
    void SchemaFactory::get_cost_switch_open(std::vector<std::string> &database_table) {
        ...
        for (auto &table: table_info_mapping) {
            const int64_t meta_id = ::ksearch::get_meta_id(table.first);
            if (meta_id != 0) {
                continue;
            }
    ...
    void SchemaFactory::get_schema_conf_open(const std::string &conf_name, std::vector<std::string> &database_table) {
        ...
        for (auto &table: table_info_mapping) {
            ...
            if (conf_name == "pk_prefix_balance" || conf_name == "tail_split_num" || conf_name == "tail_split_step" ||
                ...
                database_table.emplace_back(
                    table.second->namespace_ + "." + table.second->name + "." + pb::BackupTable_Name(
                        static_cast<pb::BackupTable>(value)));
    ...
    bool SchemaFactory::is_olap_table(int64_t table_id, int64_t partition_id, bool *is_cold) {
        ...
        *is_cold = false;
        if (table->is_range_partition) {
            for (auto it = table->partition_info.range_partition_infos().rbegin();
                 it != table->partition_info.range_partition_infos().rend(); ++it) {
    ...
    )
 {
    ...
    for (auto& table : table_info_mapping) {
        ...
        if (select_table(table.second)) {
            ...
            if (table.second->engine == pb::DBLINK) {
                if (table.second->dblink_info.type() == pb::LT_KSEARCH) {
    ...
    // used for transaction recovery after ksearch crash
    ...
    int RangePartition::init(const pb::PartitionInfo &partition_info, SmartTable &table_ptr, int64_t partition_num) {
        ...
        for (const auto &range_partition_info: partition_info.range_partition_infos()) {
            ...
            if (partition_info.has_dynamic_partition_attr() && partition_info.dynamic_partition_attr().
                has_isolation()) {
    ...
    int SchemaFactory::get_binlog_region_by_partition_id(int64_t binlog_id, int64_t partition_id,
                                                         pb::RegionInfo &region_info,
    ...
    int SchemaFactory::get_partition_binlog_regions(const std::string &db_table_name, int64_t partition_input_value,
                                                    std::unordered_map<int64_t, std::unordered_map<int64_t, std::vector<
                                                        pb::RegionInfo> > > &table_id_partition_binlogs) {
        DoubleBufferedTableRegionInfo::ScopedPtr table_region_mapping_ptr;
        if (_table_region_mapping.Read(&table_region_mapping_ptr) != 0) {
            DB_WARNING("DoubleBufferedTableRegion read scoped ptr error.");
            return -1;
    ...
    bool SchemaFactory::table_suitable_for_broadcast_join(int64_t table_id) {
        ...
        if (total_rows <= FLAGS_broadcast_table_max_rows || total_bytes <= FLAGS_broadcast_table_max_mb * 1024 *
            1024ULL) {
            return true;
        }
        return false;
