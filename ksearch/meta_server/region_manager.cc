...
#include <ksearch/meta_server/common_state_machine.h>
...
namespace ksearch {
    ...
    void RegionManager::split_region(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                     braft::Closure *done) {
        ...
        DB_WARNING("apply_index: %ld, generate %d region_id: [%ld, %ld]", apply_index, new_region_num,
                   new_region_start_id, new_region_end_id);
        //更新内存
        ...
        if (done && ((MetaServerClosure *) done)->response) {
            ...
            if (new_region_num == 1) {
                ((MetaServerClosure *) done)->response->mutable_split_response()->
                        set_new_region_id(new_region_start_id);
            } else {
                for (int64_t i = new_region_start_id; i <= new_region_end_id; ++i) {
                    int pos = i - new_region_start_id;
                    if (pos >= ((MetaServerClosure *) done)->response->mutable_split_response()->
                        multi_new_regions_size()) {
                        break;
                    }
                    ((MetaServerClosure *) done)->response->mutable_split_response()->mutable_multi_new_regions(pos)->
                            set_new_region_id(i);
                }
            }
    ...
    void RegionManager::pre_process_add_peer_for_store(const std::string &instance,
                                                       const IdcInfo &instance_idc, pb::Status status,
                                                       std::unordered_map<std::string, std::vector<pb::AddPeer> > &
                                                       add_peer_requests) {
        std::vector<int64_t> region_ids;
        get_region_ids(instance, region_ids);
        for (auto &region_id: region_ids) {
 ...
 */
    void RegionManager::leader_load_balance_on_pk_prefix(const std::string &instance,
                                                         ...
                                                         std::unordered_map<int64_t, std::unordered_map<std::string,
                                                             std::vector<int64_t> > > &pk_prefix_leader_region_map,
                                                         const std::set<int64_t> &trans_leader_region_ids,
                                                         std::unordered_map<int64_t, int64_t> &
                                                         table_transfer_leader_count,
 ...
 */
    void RegionManager::pk_prefix_load_balance(
        const std::unordered_map<std::string, int64_t> &pk_prefix_add_peer_counts,
        ...
        for (auto &add_peer_count: pk_prefix_add_peer_counts) {
            ...
            for (size_t i = 0; i < total_region_count; ++i, ++index) {
                ...
                add_peer.add_new_peers(new_instance);
                add_peer_requests.
                        push_back(std::pair<std::string, pb::AddPeer>(master_region_info->leader(), add_peer));
    ...
    void RegionManager::update_binlog_status(const pb::StoreHeartBeatRequest *request) {
        ...
        for (auto &binlog_info: request->binlog_ts_infos()) {
            ...
            _binlog_region_state_map.init_if_not_exist_else_update(region_id, true,
                                                                   [table_id, region_id, instance, oldest_ts](
                                                               BinlogRegionState &binlog_state) {
                                                                       binlog_state.
                                                                               peer_oldest_timestamp_to_now_interval[
                                                                                   instance] = oldest_ts;
                                                                       binlog_state.table_id = table_id;
                                                                       DB_DEBUG(
                                                                           "table_id: %ld, region_id: %ld, instance: %s, set oldest ts: %ld",
                                                                           table_id, region_id, instance.c_str(),
                                                                           oldest_ts);
                                                                   });
        }
    ...
    void RegionManager::update_leader_status(const pb::StoreHeartBeatRequest *request, int64_t timestamp) {
        for (auto &leader_region: request->leader_regions()) {
            ...
            if (leader_region.peers_status_size() > 0) {
                _region_peer_state_map.init_if_not_exist_else_update(region_id, true,
                                                                     [&leader_region, timestamp](
                                                                 RegionPeerState &peer_state) {
                                                                         peer_state.legal_peers_state.clear();
                                                                         for (auto &peer_status: leader_region.
                                                                              peers_status()) {
                                                                             peer_state.legal_peers_state.emplace_back(
                                                                                 peer_status);
                                                                             peer_state.legal_peers_state.back().
                                                                                     set_timestamp(timestamp);
                                                                             peer_state.legal_peers_state.back().
                                                                                     set_table_id(
                                                                                         leader_region.region().
                                                                                         table_id());
                                                                             for (auto illegal_peer_iter = peer_state.
                                                                                         ilegal_peers_state.begin();
                                                                                 illegal_peer_iter != peer_state.
                                                                                 ilegal_peers_state.end();
                                                                                 ++illegal_peer_iter) {
                                                                                 if (illegal_peer_iter->peer_id() ==
                                                                                     peer_status.peer_id()) {
                                                                                     peer_state.ilegal_peers_state.
                                                                                             erase(illegal_peer_iter);
                                                                                     break;
                                                                                 }
                                                                             }
                                                                         }
            ...
            _region_learner_peer_state_map.init_if_not_exist_else_update(
                region_id, true, [request, &master_region_info](RegionLearnerState &ls) {
        ...
        for (auto &learner_region: request->learner_regions()) {
            ...
            _region_learner_peer_state_map.init_if_not_exist_else_update(
                region_id, true, [request](RegionLearnerState &ls) {
    ...
    void RegionManager::put_incremental_regioninfo(const int64_t apply_index,
                                                   std::vector<pb::RegionInfo> &region_infos) {
    ...
    bool RegionManager::check_and_update_incremental(
        ...
        int cnt = 0;
        auto update_func = [response, need_heartbeat_table, &heartbeat_table_partition_map, &cnt](
            const std::vector<pb::RegionInfo> &region_infos) {
        ...
        TimeCost cost;
        bool need_upd = _incremental_region_info.check_and_update_incremental(
            update_func, last_updated_index, applied_index);
    ...
    bool RegionManager::add_region_is_exist(int64_t table_id, const std::string &start_key,
        ...
        if (start_key.empty()) {
            int64_t cur_regionid = TableManager::get_instance()->get_startkey_regionid(
    ...
    void RegionManager::leader_heartbeat_for_region(const pb::StoreHeartBeatRequest *request,
        ...
        for (auto &leader_region: request->leader_regions()) {
            ...
            if (master_region_info->learners_size() > 0) {
                int ret = ClusterManager::get_instance()->get_instances_idc_info(
                    master_region_info->learners(), learner_local_idc);
                if (ret <= -2) {
                    continue;
                }
                if (ret < 0) {
                    ...
                    if (current_resource_learners.size() < learner_replica_num) {
                        ...
                        for (auto current_resource_learner: current_resource_learners) {
                            ...
                            if (!is_health) {
                                ...
                                // 实例故障，等待实例迁移。
                                if (instance_status == pb::FAULTY || instance_status == pb::DEAD || instance_status ==
                                    pb::MIGRATE) {
                                    continue;
                                } else {
                                    if (master_region_info->peers_size() > 1) {
    ...
    void RegionManager::remove_learner_peer(int64_t region_id,
                                            std::vector<std::pair<std::string, pb::RemoveRegion> > &
                                            remove_learner_requests,
    ...
    void RegionManager::check_peer_count(int64_t region_id,
                                         ...
                                         table_replica_dists_maps,
                                         std::vector<std::pair<std::string, pb::RaftControlRequest> > &
                                         remove_peer_requests,
                                         std::set<int64_t> &binlog_table_ids,
        ...
        const pb::RegionInfo &leader_region_info =
                leader_region.simple() ? *master_region_info : leader_region.region();
        ...
        if (leader_region_info.peers_size() > replica_num) {
            ...
            for (auto &candicate_remove_peer: candidate_remove_peers) {
                ...
                //先判断这些peer中是否有peer所在的实例状态不是NORMAL
                if (status == pb::MIGRATE && is_binlog_region && !binlog_peer_can_delete(
                        candicate_remove_peer, region_id, table_id)) {
            ...
            if (remove_peer.empty() && !peer_not_in_replica_dist.empty()) {
                ...
                if (is_binlog_region && !binlog_peer_can_delete(remove_peer, region_id, table_id)) {
                    DB_WARNING(
                        "table_id: %ld, region_id: %ld, want remove peer: %s for not in replicaDist. but region without enough binlog data, skip",
            ...
            remove_peer_requests.push_back(
                std::pair<std::string, pb::RaftControlRequest>(remove_peer, remove_peer_request));
        }
    }
        ...
        int64_t region_id, RegionLearnerState &learner_map) {
                for (auto &learner_state: learner_map.learner_state_map) {
                    if (butil::gettimeofday_us() - learner_state.second.timestamp() >
                        ...
                        DB_DEBUG("region %ld learner not heartbeat instance %s", region_id,
                                 learner_state.first.c_str());
    ...
    void RegionManager::recovery_single_region_by_init_region(const std::set<int64_t> region_ids,
        ...
        for (int64_t region_id: region_ids) {
            ...
            if (iter != exist_regions.end()) {
                ...
                auto send_init_region_func = [this, region_id, region_ptr, health_instances, &choose_index, &
                            recover_region_way]() {
    ...
    void RegionManager::recovery_all_region(const pb::MetaManagerRequest &request,
        ...
        auto get_peers_func = [&region_peers_map, recover_opt
                ](const int64_t &region_id, RegionPeerState &region_state) {
