...
namespace ksearch {
    ...
    public:
        ...
        void process_common_task_hearbeat(const std::string &address, const pb::KsHeartBeatRequest *request,
                                          pb::KsHeartBeatResponse *response);

        void process_broadcast_task_hearbeat(const std::string &address, const pb::KsHeartBeatRequest *request,
                                             pb::KsHeartBeatResponse *response);
        ...
        void update_ksearch_info(const std::string &address, const std::string &room) {
        ...
        std::vector<std::string> get_faulty_ksearch();
    ...
    private:
        ...
        void update_table_ddl_mem(const ksearch::pb::DdlWorkInfo &info) {
            BAIDU_SCOPED_LOCK(_table_mutex);
        ...
        bool get_ddl_mem(int64_t table_id, ksearch::pb::DdlWorkInfo &work_info) {
            BAIDU_SCOPED_LOCK(_table_mutex);
            auto it = _table_ddl_mem.find(table_id);
            if (it != _table_ddl_mem.end()) {
