...
#include <ksearch/meta_server/tso_state_machine.h>
...
#include <ksearch/meta_server/query_namespace_manager.h>
...
namespace ksearch {
    ...
    int MetaServer::send_rpc_to_trans_leader(int type, std::string old_leader,
    ...
    void MetaServer::ks_heartbeat(google::protobuf::RpcController *controller,
        ...
        RETURN_IF_NOT_INIT(_init_success, response, log_id);
        if (_meta_state_machine != nullptr) {
            _meta_state_machine->ks_heartbeat(controller, request, response, done_guard.release());
        }
    }

    void MetaServer::ks_other_heartbeat(google::protobuf::RpcController *controller,
        ...
        RETURN_IF_NOT_INIT(_init_success, response, log_id);
        if (_meta_state_machine != nullptr) {
            _meta_state_machine->ks_other_heartbeat(controller, request, response, done_guard.release());
        }
    }
