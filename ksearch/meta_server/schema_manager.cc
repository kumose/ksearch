 ...
 */
            ...
            case pb::OP_SPECIFY_SPLIT_KEYS: {
                ...
                if (request->op_type() == pb::OP_ADD_INDEX || request->op_type() == pb::OP_DROP_INDEX
                    ...
                    if (request->op_type() == pb::OP_ADD_INDEX) {
                        auto ret = TableManager::get_instance()->pre_process_for_add_index(
                            request, response, log_id, done);
                        if (ret < 0) {
                            return;
                        }
                    }
                ...
                if ((request->op_type() == pb::OP_ADD_PARTITION || request->op_type() == pb::OP_DROP_PARTITION ||
                     request->op_type() == pb::OP_MODIFY_PARTITION || request->op_type() ==
                     pb::OP_UPDATE_DYNAMIC_PARTITION_ATTR) &&
                ...
                if (request->op_type() == pb::OP_ADD_PARTITION) {
                    auto ret = TableManager::get_instance()->pre_process_for_add_partition(
                        request, response, log_id, done);
                    if (ret < 0) {
                        return;
                    }
    ...
    void SchemaManager::process_ks_heartbeat(const pb::KsHeartBeatRequest *request,
        ...
        std::unordered_map<int64_t, std::unordered_set<int64_t> > new_heartbeat_table_partition_map;
        // 按需获取场景，新增表需要获取心跳基准
        ...
        bool need_global_index_heartbeat = (request->has_need_global_index_heartbeat() && request->
                                            need_global_index_heartbeat());
            ...
            for (const auto &table_info: request->heartbeat_tables()) {
                if (need_binlog_heartbeat && table_info.table_name() == "*") {
                    ...
                    const std::string &full_table_name =
                            table_info.namespace_name() + "\001" + table_info.database() + "\001" + table_info.
                            table_name();
        ...
        DB_NOTICE("process schema info for ks heartbeat, prepare_time: %ld,"
    ...
    int SchemaManager::load_snapshot() {
        ...
        for (; iter->Valid(); iter->Next()) {
            ...
            if (iter->key().starts_with(region_prefix)) {
                ret = RegionManager::get_instance()->load_region_snapshot(
...
#undef SET_REQUEST_TABLE_INFO
        ...
        if (table_info.engine() != pb::ROCKSDB && table_info.engine() != pb::ROCKSDB_CSTORE) {
            if (table_info.has_ttl_duration()) {
                ERROR_SET_RESPONSE(response, pb::INPUT_PARAM_ERROR,
                                   "engine != rocksdb/rocksdb_cstore can not create ttl table", request->op_type(),
                                   log_id);
                return -1;
            }
        }
        ...
        for (auto idx = 0;
             idx < table_info.partition_num() && TableManager::get_instance()->is_create_table_support_engine(
                 table_info.engine());
             ++idx) {
            ...
            for (auto i = 0; i < table_info.split_keys_size(); ++i) {
                ...
                for (auto j = 0; j <= split_key->split_keys_size(); ++j) {
                    ...
                    if (ret < 0) {
                        ...
                        ERROR_SET_RESPONSE(response, pb::INTERNAL_ERROR, "select instance fail", request->op_type(),
                                           log_id);
                        return -1;
                    }
        ...
        //没有指定split_key的索引
        for (auto i = 0;
             i < table_info.partition_num() && TableManager::get_instance()->is_create_table_support_engine(
                 table_info.engine());
             ++i) {
            ...
            for (auto &index: need_pick_instance_index) {
                std::string instance;
                if (namespace_name == "INTERNAL" && table_info.database() == "ksearch" && table_info.table_name() ==
                    "__ksearch_instance") {
    ...
    int SchemaManager::whether_dists_legal(pb::MetaManagerRequest *request,
        ...
        for (auto &dist: request->table_info().dists()) {
            ...
            if (ClusterManager::get_instance()->logical_and_physical_room_valid(
                dist.logical_room(), dist.physical_room())) {
                total_count += dist.count();
                continue;
            }
            ERROR_SET_RESPONSE(response, pb::INPUT_PARAM_ERROR,
    ...
    int SchemaManager::pre_process_for_partition(const pb::MetaManagerRequest *request,
        ...
        for (auto &kv: range_partition_info_map) {
            ...
            std::sort(range_partition_info_vec.begin(), range_partition_info_vec.end(),
                      partition_utils::RangeComparator());
            ...
            for (size_t i = 0; i < range_partition_info_vec.size(); ++i) {
                ...
                if (i == 0) {
                    ...
                    if (partition_utils::create_partition_expr(partition_col_type, partition_str_val, *p_left_value) !=
                        0) {
        ...
        for (auto &kv: range_partition_info_map) {
            ...
            for (auto &range_partition_info: range_partition_info_vec) {
                if (partition_utils::check_partition_overlapped(p_partition_info->range_partition_infos(),
                    ...
                    DB_WARNING("Partition overlapped, range_partition_info: %s",
