...
namespace ksearch {
    ...
    struct InstanceStateInfo {
        int64_t timestamp{0}; //上次收到该实例心跳的时间戳
        pb::Status state{pb::NORMAL}; //实例状态
    ...
    using DoubleBufferedSchedulingInfo = butil::DoublyBufferedData<std::unordered_map<std::string,
    ...
    struct Instance {
        ...
        Instance(const pb::InstanceInfo &instance_info) : address(instance_info.address()),
    ...
    public:
        ...
        // 只在leader内存添加、更新
        void add_ks_instance(const pb::KsStatus &ks_status);

        void update_ks_instance(const pb::KsStatus &ks_status);
        ...
        void process_ks_heartbeat(const pb::KsHeartBeatRequest *request,
        ...
        void process_instance_param_heartbeat_for_ks(const pb::KsOtherHeartBeatRequest *request,
        ...
        void ks_healthy_check_function();

        void construct_ks_heartbeat_response(const pb::KsHeartBeatRequest *request, pb::KsHeartBeatResponse *response);
    ...
    public:
        ...
        int update_ks_instance_info(const pb::KsHeartBeatRequest &request);
    ...
    private:
        ClusterManager() {
            ...
            bthread_mutex_init(&_ks_instance_mutex, NULL);
            {
                BAIDU_SCOPED_LOCK(_physical_mutex);
    ...
    private:
        ...
        bthread_mutex_t _ks_instance_mutex;
        // ks实例信息，只记录状态 address->InstanceStateInfo
        std::unordered_map<std::string, InstanceStateInfo> _ks_instance_info;
        std::unordered_map<std::string, std::string> _ks_instance_resource_tag_map;
        std::unordered_map<std::string, std::set<std::string> > _resource_tag_ks_instances_map;
