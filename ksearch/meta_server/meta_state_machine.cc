...
#include <ksearch/meta_server/meta_state_machine.h>
...
namespace ksearch {
    ...
    void MetaStateMachine::ks_heartbeat(google::protobuf::RpcController *controller,
        ...
        DB_DEBUG("ksearch request[%s]", request->ShortDebugString().c_str());
        TimeCost step_time_cost;
        ON_SCOPE_EXIT([]() {
            Concurrency::get_instance()->ks_heartbeat_concurrency.decrease_signal();
        });
        int ret = Concurrency::get_instance()->ks_heartbeat_concurrency.increase_timed_wait(10 * 1000 * 1000LL);
        if (ret != 0) {
            DB_FATAL("ksearch:%s time_cost: %ld, log_id: %lu",
        ...
        step_time_cost.reset();
        DatabaseManager::get_instance()->process_ks_heartbeat(request, response);
        ClusterManager::get_instance()->process_ks_heartbeat(request, response);
        int64_t cluster_time = step_time_cost.get_time();
        step_time_cost.reset();
        PrivilegeManager::get_instance()->process_ks_heartbeat(request, response);
        int64_t privilege_time = step_time_cost.get_time();
        step_time_cost.reset();
        SchemaManager::get_instance()->process_ks_heartbeat(request, response, log_id);
        int64_t schema_time = step_time_cost.get_time();
        step_time_cost.reset();
        DBManager::get_instance()->process_ks_heartbeat(request, response, cntl);
        int64_t ddl_time = step_time_cost.get_time();
        step_time_cost.reset();
        NamespaceManager::get_instance()->process_ks_heartbeat(request, response);
        int64_t ns_cost = step_time_cost.get_time();
        _ks_heart_beat << time_cost.get_time();
        DB_NOTICE("ksearch:%s heart beat, wait_time:%ld, time_cost: %ld, cluster_time: %ld, "
    ...
    void MetaStateMachine::ks_other_heartbeat(google::protobuf::RpcController *controller,
        ...
        TimeCost step_time_cost;
        Concurrency::get_instance()->ks_other_heartbeat_concurrency.increase_wait();
        ON_SCOPE_EXIT([]() {
            Concurrency::get_instance()->ks_other_heartbeat_concurrency.decrease_signal();
        });
        int64_t wait_time = step_time_cost.get_time();
        step_time_cost.reset();
        response->set_errcode(pb::SUCCESS);
        ...
        ClusterManager::get_instance()->process_instance_param_heartbeat_for_ks(request, response);
        int64_t schema_time = step_time_cost.get_time();


        DB_NOTICE("ksearch:%s heart beat, wait time: %ld, update_cost: %ld, log_id: %lu",
                  butil::endpoint2str(cntl->remote_side()).c_str(),
    ...
    void MetaStateMachine::console_heartbeat(google::protobuf::RpcController *controller,
        ...
        DB_NOTICE("ksearch:%s heart beat, time_cost: %ld, "
    ...
    void MetaStateMachine::healthy_check_function() {
        ...
        */
        ...
        while (_node.is_leader()) {
            ...
            // ks 相关信息也存储在Cluster中
            ClusterManager::get_instance()->ks_healthy_check_function();
            //region多久没上报心跳了
