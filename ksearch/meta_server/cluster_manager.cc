...
namespace ksearch {
    ...
    DECLARE_int32 (ks_heartbeat_interval_us);
    DECLARE_int32 (ks_faulty_interval_times);
    DECLARE_int32 (ks_clear_faulty_interval_times);
    ...
    bvar::Window<bvar::Adder<int> > g_ksmeta_min_fallback_count("cluster_manager", "min_fallback_count",
                                                                &min_fallback_count, 3600);
    bvar::Window<bvar::Adder<int> > g_ksmeta_min_count("cluster_manager", "min_count", &min_count, 3600);
    bvar::Window<bvar::Adder<int> > g_ksmeta_rolling_fallback_count("cluster_manager", "rolling_fallback_count",
                                                                    &rolling_fallback_count, 3600);
    bvar::Window<bvar::Adder<int> > g_ksmeta_rolling_count("cluster_manager", "rolling_count", &rolling_count, 3600);
        ...
        switch (request->op_type()) {
            ...
            case pb::OP_UPDATE_INSTANCE_PARAM: {
                if (request->instance_params().size() < 1) {
                    ERROR_SET_RESPONSE(response, pb::INPUT_PARAM_ERROR, "no instance params", request->op_type(),
                                       log_id);
                    return;
                }
                _meta_state_machine->process(controller, request, response, done_guard.release());
            ...
            case pb::OP_MOVE_PHYSICAL: {
                if (!request->has_move_physical_request()) {
                    ERROR_SET_RESPONSE(response, pb::INPUT_PARAM_ERROR, "no move physical request", request->op_type(),
                                       log_id);
                    return;
                }
                _meta_state_machine->process(controller, request, response, done_guard.release());
    ...
    void ClusterManager::add_instance(const pb::MetaManagerRequest &request, braft::Closure *done) {
        ...
        auto call_func = [](std::unordered_map<std::string, InstanceSchedulingInfo> &scheduling_info,
            ...
            scheduling_info[instance_mem.address].idc = {
                instance_mem.resource_tag, instance_mem.logical_room, instance_mem.physical_room
            };
    ...
    void ClusterManager::add_ks_instance(const pb::KsStatus &ks_status) {
        std::string address = ks_status.address();
        std::string resource_tag = ks_status.resource_tag();

        if (address.empty() || resource_tag.empty()) {
            // should not reach here
            DB_FATAL("ks address or resource tag empty when try to add ks instance. address :%s, resource tag:%s",
                     address.c_str(),
                     resource_tag.c_str());
            return;
        }
        ...
        // 判断该实例是否已经存在
        if (0 != _ks_instance_info.count(address)) {
            DB_WARNING("ks instance:%s has already exist", address.c_str());
            return;
        }

        // 更新内存值
        {
            BAIDU_SCOPED_LOCK(_ks_instance_mutex);
            InstanceStateInfo state_info;
            state_info.state = ks_status.status();
            state_info.timestamp = butil::gettimeofday_us();
            _ks_instance_info[address] = state_info;
            _ks_instance_resource_tag_map[address] = resource_tag;

            if (0 == _resource_tag_ks_instances_map.count(resource_tag)) {
                _resource_tag_ks_instances_map[resource_tag] = std::set<std::string>();
            }
            std::set<std::string> &instances = _resource_tag_ks_instances_map[resource_tag];
            instances.insert(address);
        }
        DB_NOTICE("add ks instance success, ks address:%s , new resource_tag:%s",
                  address.c_str(),
                  resource_tag.c_str());
    }

    void ClusterManager::update_ks_instance(const pb::KsStatus &ks_status) {
        std::string address = ks_status.address();
        std::string resource_tag = ks_status.resource_tag();

        if (address.empty() || resource_tag.empty()) {
            // should not reach here
            DB_FATAL("ks address or resource tag empty when try to update ks instance. address :%s, resource tag:%s",
                     address.c_str(),
                     resource_tag.c_str());
            return;
        }
        ...
        // 判断该实例是否已经存在
        if (0 == _ks_instance_info.count(address)) {
            DB_WARNING("ks instance:%s doesn't exist", address.c_str());
            return;
        }
        // 更新内存值
        {
            ...
            std::string old_resource_tag = _ks_instance_resource_tag_map[address];
            // 如果需要更新resource_tag
            if (old_resource_tag != resource_tag) {
                if (0 != _resource_tag_ks_instances_map.count(old_resource_tag)) {
                    std::set<std::string> &instances = _resource_tag_ks_instances_map[old_resource_tag];
        ...
        DB_NOTICE("modify ks resourece tag success, ks address:%s , new resource_tag:%s",
                  address.c_str(),
                  resource_tag.c_str());
    }

    // 如果已存在则修改，不存在则新添加
    inline void agg_instance_param(const pb::InstanceParam &old_param, const pb::InstanceParam &new_param,
    ...
    void ClusterManager::process_ks_heartbeat(const pb::KsHeartBeatRequest *request,
        ...
        int ret = update_ks_instance_info(*request);
        if (ret == 0) {
            construct_ks_heartbeat_response(request, response);
            return;
        }

        if (ret == -1) {
            add_ks_instance(request->ks_status());
        } else if (ret == -2) {
            update_ks_instance(request->ks_status());
        }
        construct_ks_heartbeat_response(request, response);
    }

    void ClusterManager::construct_ks_heartbeat_response(const pb::KsHeartBeatRequest *request,
                                                         pb::KsHeartBeatResponse *response) {
        if (!request->has_ks_status()) {
            return;
        }
        std::string resource_tag = request->ks_status().resource_tag();
        if (resource_tag.empty() || 0 == _resource_tag_ks_instances_map.count(resource_tag)) {
            return;
        }

        for (auto &address: _resource_tag_ks_instances_map[resource_tag]) {
            pb::KsStatus *ks_status = response->add_ks_status();
            ks_status->set_address(address);
            ks_status->set_status(_ks_instance_info[address].state);
            ks_status->set_resource_tag(resource_tag);
            ks_status->set_last_heartbeat_timestamp(_ks_instance_info[address].timestamp);
        }
    }
    ...
    // 获取实例参数
    void ClusterManager::process_instance_param_heartbeat_for_ks(const pb::KsOtherHeartBeatRequest *request,
                                                                 pb::KsOtherHeartBeatResponse *response) {
        if (request->has_ksearch_resource_tag()) {
            BAIDU_SCOPED_LOCK(_instance_param_mutex);

            auto iter = _instance_param_map.find("__ksearch");
            if (iter != _instance_param_map.end()) {
                *(response->add_instance_param()) = iter->second;
            }

            iter = _instance_param_map.find(request->ksearch_resource_tag());
            if (iter != _instance_param_map.end()) {
                *(response->add_instance_param()) = iter->second;
            }
        }
    ...
    void ClusterManager::process_pk_prefix_load_balance(
    ...
    void ClusterManager::ks_healthy_check_function() {
        // 放在外面防止其他线程insert导致迭代器失效
        BAIDU_SCOPED_LOCK(_ks_instance_mutex);
        for (auto instance_pair = _ks_instance_info.begin(); instance_pair != _ks_instance_info.end();) {
            ...
            if ((butil::gettimeofday_us() - last_timestamp) >
                FLAGS_ks_heartbeat_interval_us * FLAGS_ks_clear_faulty_interval_times) {
                // 超过清理时长获取不了心跳清除记录
                instance_pair = _ks_instance_info.erase(instance_pair);
                DB_WARNING("ks instance %s: No heartbeat received for more than %d seconds, cleared",
                           instance_name.c_str(),
                           FLAGS_ks_heartbeat_interval_us * FLAGS_ks_clear_faulty_interval_times / 1000000);

                if (0 == _ks_instance_resource_tag_map.count(instance_name)) {
                    DB_WARNING("instance: %s resource tag does not exist", instance_name.c_str());
                    continue;
                }
                std::string resource_tag = _ks_instance_resource_tag_map[instance_name];
                _ks_instance_resource_tag_map.erase(instance_name);
                ...
                if (0 == instances.size()) {
                    _resource_tag_ks_instances_map.erase(resource_tag);
                }
            } else if ((butil::gettimeofday_us() - last_timestamp) >
                       FLAGS_ks_heartbeat_interval_us * FLAGS_ks_faulty_interval_times) {
                // 超时获取不了心跳设置为FAULTY
                state_info.state = pb::FAULTY;
                ++instance_pair;
                DB_WARNING("ks instance %s: No heartbeat received for more than %d seconds, set as 'FAULTY'",
                           instance_name.c_str(),
                           FLAGS_ks_heartbeat_interval_us * FLAGS_ks_faulty_interval_times / 1000000);
            } else {
                ++instance_pair;
            }
        }
        ...
        for (auto &store_pair: migrate_stores) {
            ...
            for (auto &store: store_pair.second) {
                if (_meta_state_machine->get_migrate(store_pair.first)
                    ...
                    && RegionManager::get_instance()->check_binlog_regions_can_migrate(store.address)
                    // 检查store上所有的binlog有其他数据超过7天的peer
    ...
    int ClusterManager::load_instance_snapshot(const std::string &instance_prefix,
        ...
        auto call_func = [](std::unordered_map<std::string, InstanceSchedulingInfo> &scheduling_info,
            ...
            scheduling_info[instance_pb.address()].idc = {
                instance_pb.resource_tag(), instance_pb.logical_room(), instance_pb.physical_room()
            };
    ...
    int ClusterManager::update_ks_instance_info(const pb::KsHeartBeatRequest &request) {
        if (!request.has_ks_status()) {
            return 0;
        }
        std::string address = request.ks_status().address();
        std::string resource_tag = request.ks_status().resource_tag();
        if (address.empty() || resource_tag.empty()) {
            return 0;
        }
        ...
        if (0 == _ks_instance_info.count(address)) {
            // 不存在 需要添加
            return -1;
        } else if (_ks_instance_resource_tag_map[address] != resource_tag) {
            // 存在 但需要更新
            return -2;
        }

        if (_ks_instance_info[address].state != request.ks_status().status()) {
            // 状态更新，需要同步到follower
            return -2;
        }
        // 正常需要更新心跳, 不会更新到follower
        _ks_instance_info[address].timestamp = butil::gettimeofday_us();
        return 0;
    }
    ...
    int ClusterManager::update_instance_info(const pb::InstanceInfo &instance_info) {
        ...
        if (status == pb::NORMAL) {
            ...
            if (FLAGS_store_rocks_hang_check
                &&store_rocks_check_cost 
            >=
            FLAGS_store_rocks_hang_check_timeout_s * 1000 * 1000LL
            )
            {
                return 0;
            }
