...
namespace ksearch {
    ...
    public:
        ...
        void update_table_internal(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                   braft::Closure *done,
        ...
        void drop_table_tombstone(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                  braft::Closure *done);
        ...
        void set_main_logical_room(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                   braft::Closure *done);
        ...
        void update_ttl_duration(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                 braft::Closure *done);

        void update_ttl_info(const pb::MetaManagerRequest &request, const int64_t apply_index, braft::Closure *done);

        void update_resource_tag(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                 braft::Closure *done);

        void update_table_comment(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                  braft::Closure *done);

        void update_dynamic_partition_attr(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                           braft::Closure *done);
        ...
        void set_index_hint_status(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                   braft::Closure *done);
        ...
        void update_index_status(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                 braft::Closure *done);
        ...
        void check_update_statistics(const pb::KsOtherHeartBeatRequest *request,
                                     pb::KsOtherHeartBeatResponse *response);

        int get_statistics(const int64_t table_id, pb::Statistics &stat_pb);

        void check_update_or_drop_table(const pb::KsHeartBeatRequest *request,
    ...
    public:
        ...
        void get_table_by_resource_tag(const std::string &resource_tag,
            ...
            for (auto &pair: info->table_info_map) {
                if (resource_tag == "") {
                    ...
                    for (auto &t: pair.second.schema_pb.learner_resource_tags()) {
                        if (t == resource_tag) {
                            std::string name = pair.second.schema_pb.database() + "." + pair.second.schema_pb.
                                               table_name();
                            table_id_name_map.insert(std::make_pair(pair.first, name));
                            break;
                        }
        ...
        void get_partition_regions(int64_t table_id,
                                   std::unordered_map<int64_t, std::set<int64_t> > &partition_regions) {
            DoubleBufferedTableMemMapping::ScopedPtr info;
            if (_table_mem_infos.Read(&info) != 0) {
                DB_WARNING("read double_buffer_table error.");
        ...
        bool check_daily_partition(int64_t table_id) {
            ...
            if (table_iter->second.schema_pb.has_partition_info()
                ...
                && boost::algorithm::iequals(
                    table_iter->second.schema_pb.partition_info().dynamic_partition_attr().time_unit(), "DAY")) {
                return true;
            }
            return false;
        }
        ...
        void remove_global_index_data(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                      braft::Closure *done);
        ...
        void get_delay_delete_index(std::vector<pb::SchemaInfo> &index_to_delete,
                                    std::vector<pb::SchemaInfo> &index_to_clear) {
        ...
        // 获取指定表分区的region_ids
        void get_region_ids(const int64_t table_id, const std::unordered_set<int64_t> &partition_ids,
    ...
    private:
        TableManager() : _max_table_id(0) {
            ...
            _dblink_mysql_table_timer.init(FLAGS_ks_heartbeat_interval_us / 1000);
        }
        ...
        void load_virtual_indextosqls_to_memory(const pb::KsHeartBeatRequest *request);
