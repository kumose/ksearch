...
#include <ksearch/common/mut_table_key.h>
...
namespace ksearch {
    ...
    DEFINE_int32(remove_dropped_partition_region_s, 60 * 1000 * 1000LL,
                 "remove dropped partition region delay seconds, default: 60s");
            ...
            auto dynamic_partition_func = [this, dynamic_partition_requests]() {
                ...
                if (dynamic_partition_requests.size() > 0) {
                    ...
                    if (dynamic_partition_requests.size() > FLAGS_dynamic_partition_change_cnt) {
                        sleep_s = static_cast<double>(FLAGS_dynamic_partition_change_time_s) /
                                  dynamic_partition_requests.size();
                    }
    ...
    void DBLinkMysqlTableTimer::run() {
        ...
        for (auto &schema: dblink_mysql_schemas) {
            ...
            if (TableManager::get_instance()->get_fields_from_mysql(schema.dblink_info().mysql_info(), mysql_fields) !=
                0) {
    ...
    void TableManager::update_table_internal(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                             braft::Closure *done,
                                             std::function<void(const pb::MetaManagerRequest & request,
    ...
    void TableManager::create_view(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                   braft::Closure *done) {
    ...
    void TableManager::create_table(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                    braft::Closure *done) {
    ...
    int TableManager::do_create_table_sync_req(pb::SchemaInfo &schema_pb,
        ...
        if (schema_pb.engine() == pb::ROCKSDB
            ...
            if (ret == 0) {
                ...
                if (namespace_name == "INTERNAL" && database == "ksearch" && table_name == "__ksearch_instance") {
                    // for ksearch instance id, do nothing
                } else {
    ...
    void TableManager::drop_table(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                  braft::Closure *done) {
        ...
        if (check_table_is_linked(drop_table_id)) {
            ...
            IF_DONE_SET_RESPONSE(done, pb::INPUT_PARAM_ERROR,
                                 "table is linked binlog table, watt data stream may need to be restarted after operation");
            return;
        }
        ...
        DB_WARNING("drop_table, request: %s, schema_info: %s", request.ShortDebugString().c_str(),
    ...
    void TableManager::drop_table_tombstone(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                            braft::Closure *done) {
        std::vector<std::string> delete_rocksdb_keys;
    ...
    void TableManager::restore_table(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                     braft::Closure *done) {
        int64_t table_id = 0;
        if (check_table_exist(request.table_info(), table_id) == 0) {
    ...
    bool TableManager::check_and_update_incremental(
        const pb::KsHeartBeatRequest *request, pb::KsHeartBeatResponse *response, int64_t applied_index,
            ...
            for (const auto &info: schema_infos) {
                if (need_heartbeat_table) {
                    ...
                    if (need_binlog_heartbeat) {
                        if (need_skip_table) {
                            if (heartbeat_binlog_main_table_ids.find(table_id) != heartbeat_binlog_main_table_ids.
                                end()) {
        ...
        bool need_upd = _incremental_schemainfo.check_and_update_incremental(
            update_schema_func, last_updated_index, applied_index);
    ...
    void TableManager::put_incremental_schemainfo(const int64_t apply_index,
                                                  std::vector<pb::SchemaInfo> &schema_infos) {
    ...
    // 状态机外选store实例
    int TableManager::pre_process_for_add_partition(const pb::MetaManagerRequest *request,
    ...
    void TableManager::add_partition(const pb::MetaManagerRequest &request,
        ...
        if (request.table_info().partition_info().range_partition_infos_size()
            ...
            DB_FATAL("request range_partition_infos_size: %d withpartition_index_instance_size: %d not match",
                     request.table_info().partition_info().range_partition_infos_size(),
                     request.partition_index_instance_size());
        ...
        for (auto &range_partition_info: request.table_info().partition_info().range_partition_infos()) {
            ...
            pb::RangePartitionInfo &mut_range_partition_info = const_cast<pb::RangePartitionInfo &>(
                range_partition_info);
            ...
            if (p_read_add_range_partition_info == nullptr) {
                DB_WARNING("p_read_add_range_partition_info is nullptr, request: %s",
                           request.ShortDebugString().c_str());
        ...
        if (!mem_schema_pb.SerializeToString(&mem_schema_pb_str)) {
            DB_WARNING("request SerializeToString fail when add_partition, request: %s",
                       request.ShortDebugString().c_str());
            ...
            for (size_t i = 0; i < mem_schema_pb.partition_info().range_partition_infos_size(); ++i) {
                ...
                if (req_range_partition_info.has_is_cold()) {
                    if (req_range_partition_info.is_cold()) {
                        if (p_range_partition_info->type() == mem_schema_pb.partition_info().
                            primary_range_partition_type()) {
                            // 热转冷，只有主分区才能触发
                            std::pair<std::string, std::string> range_key;
    ...
    void TableManager::convert_partition(const pb::MetaManagerRequest &request,
        ...
        update_table_internal(request, apply_index, done,
                              [](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                 braft::Closure *done) {
                                  if (mem_schema_pb.has_partition_info() && mem_schema_pb.partition_info().type() ==
                                      pb::PT_RANGE) {
                                      ...
                                      if (request.has_table_info() && request.table_info().has_partition_info()) {
                                          const pb::PartitionInfo &req_partition_info = request.table_info().
                                                  partition_info();
                                          const pb::RangePartitionType primary_range_partition_type = req_partition_info
                                                  .primary_range_partition_type();
                                          bool found = false;
                                          ...
                                          if (!found) {
                                              if (done != nullptr && ((MetaServerClosure *) done)->response !=
                                                  nullptr) {
                                                  ((MetaServerClosure *) done)->response->set_errcode(
                                                      pb::INPUT_PARAM_ERROR);
                                              }
                                              DB_WARNING(
                                                  "primary_range_partition_type is not in gen_range_partition_types");
                                              return;
                                          }
                                          ...
                                          if (p_partition_info->primary_range_partition_type() == pb::RPT_DEFAULT &&
                                              ...
                                              if (p_partition_info->has_dynamic_partition_attr() &&
                                                  ...
                                                  const std::string &time_unit = p_partition_info->
                                                          dynamic_partition_attr().time_unit();
                                                  const int32_t cold = p_partition_info->dynamic_partition_attr().
                                                          cold();
                                                  const std::string &isolation = p_partition_info->
                                                          dynamic_partition_attr().isolation();
                                                  const pb::PrimitiveType &partition_col_type = p_partition_info->
                                                          field_info().mysql_type();
                                                  // 计算cold日期
                                                  TimeUnit unit;
                                                  ...
                                                  if (partition_col_type == pb::DATE) {
                                                      timestamp_to_format_str(
                                                          cold_ts, partition_utils::DATE_FORMAT, cold_str);
                                                  } else {
                                                      timestamp_to_format_str(
                                                          cold_ts, partition_utils::DATETIME_FORMAT, cold_str);
                                                  }
                                          ...
                                          p_partition_info->set_primary_range_partition_type(
                                              primary_range_partition_type);
                                          if (req_partition_info.gen_range_partition_types_size() > 0) {
                                              p_partition_info->clear_gen_range_partition_types();
                                              for (const auto &type: req_partition_info.gen_range_partition_types()) {
                                                  p_partition_info->add_gen_range_partition_types(
                                                      static_cast<pb::RangePartitionType>(type));
                                              }
                                          }
                                      }
                                      mem_schema_pb.set_version(mem_schema_pb.version() + 1);
    ...
    void TableManager::update_dynamic_partition_attr(const pb::MetaManagerRequest &request,
        ...
        if (done) {
            ...
            get_change_partition_schema(mem_schema_pb, add_partition_schema, del_partition_schema,
                                        cold_partition_schema);
            if (add_partition_schema.partition_info().range_partition_infos_size() != 0) {
                ...
                SchemaManager::get_instance()->process_schema_info(NULL, &alter_partition_request, NULL,
                                                                   brpc::DoNothing());
            }
            ...
            if (cold_partition_schema.partition_info().range_partition_infos_size() != 0) {
                ...
                DB_NOTICE("DDL_LOG modify dynamic partition req[%s]",
                          alter_partition_request.ShortDebugString().c_str());
                SchemaManager::get_instance()->process_schema_info(NULL, &alter_partition_request, NULL,
                                                                   brpc::DoNothing());
            }
        }
        ...
        update_table_internal(request, apply_index, done,
                              [](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                 braft::Closure *done) {
                                  ...
                                  if (schema_conf.has_snapshot_blacklist() || schema_conf.
                                      has_del_snapshot_blacklist()) {
    ...
    int TableManager::check_gflag(const IdcInfo &idc, const std::string &gflag_name,
        ...
        for (int retry_times = 0; retry_times < max_retry_times; retry_times++) {
            ...
            if (start_pos == std::string::npos) {
                ...
                DB_WARNING(
                    "query gflag failed, response unexpected format, instance_addr: %s, gflag_name: %s, response: %s",
            ...
            for (const auto &field: table_schema.fields()) {
                if (field.field_name() == ttl_field_name) {
                    ...
                    if (field.mysql_type() != pb::PrimitiveType::DATETIME
                        ...
                        DB_WARNING("ttl_field is not supported type, type[%s]",
                                   pb::PrimitiveType_Name(field.mysql_type()).c_str());
        ...
        update_table_internal(request, apply_index, done,
                              [](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                 braft::Closure *done) {
                                  ...
                                  } else if (mem_schema_pb.ttl_duration() <= 0 && request.table_info().ttl_duration() >
                                             0) {
                                      // online ttl
                                      int64_t online_ttl_expire_time_us =
                                              butil::gettimeofday_us() + request.table_info().ttl_duration() *
                                              1000000LL;
                                      mem_schema_pb.set_ttl_duration(request.table_info().ttl_duration());
    ...
    void TableManager::check_update_statistics(const pb::KsOtherHeartBeatRequest *request,
        ...
        for (const auto &iter: table_version_map) {
            ...
            if (ret < 0) {
                if (ret == -2) {
                    ...
                    stat_pb.set_version(version);
                    DB_WARNING("statistics not found, treated as deleted, table_id:%ld, version:%ld", iter.first,
                               version);
                } else {
                    continue;
                }
            ...
            for (auto &partition_region: table_info_pair.second.partition_regions) {
                if (need_heartbeat_table) {
                    if (heartbeat_table_partition_map.find(table_id) != heartbeat_table_partition_map.end()) {
                        ...
                        if (!partition_ids.empty() && partition_ids.find(partition_region.first) == partition_ids.
                            end()) {
                            continue;
                        }
                    }
    ...
    void TableManager::check_add_region(
        ...
        const std::unordered_map<int64_t, std::unordered_set<int64_t> > &heartbeat_table_partition_map) {
        bool need_heartbeat_table = request->has_need_heartbeat_table() && request->need_heartbeat_table();
        std::unordered_map<int64_t, std::unordered_set<int64_t> > table_partition_for_add_region;
        // 需要add_region的table_id及partition_ids
    ...
    int TableManager::write_schema_for_not_level(const pb::MetaManagerRequest &request,
        ...
        if (table_mem.schema_pb.has_partition_info() && table_mem.schema_pb.partition_info().type() == pb::PT_RANGE) {
            if (table_mem.schema_pb.partition_num() != table_mem.schema_pb.partition_info().
                range_partition_infos_size()) {
    ...
    void TableManager::send_link_binlog_request(const pb::SchemaInfo &schema_info,
                                                const google::protobuf::RepeatedPtrField<pb::BinlogInfo> &
                                                binlog_infos) {
    ...
    void TableManager::check_table_exist_for_peer(const pb::StoreHeartBeatRequest *request,
        ...
        for (auto &peer_info: request->peer_infos()) {
            ...
            int64_t main_table_id = peer_info.main_table_id() == 0 ? peer_info.table_id() : peer_info.main_table_id();
    ...
    int TableManager::alloc_index_id(pb::SchemaInfo &table_info, TableMem &table_mem,
            ...
            if (table_info.indexs(i).index_type() == pb::I_ROLLUP) {
                ...
                for (auto j = 0; j < table_info.indexs(i).field_names_size(); ++j) {
                    ...
                    if (primary_key_field_id_set.count(field_id) == 0) {
                        DB_WARNING("table:%s rollup key: %s not in primary key", table_name.c_str(),
                                   field_name.c_str());
                        return -1;
                    }
        ...
        if (has_rollup_key) {
            for (auto j = 0; j < table_info.fields_size(); ++j) {
                ...
                if (primary_key_field_id_set.count(field_info.field_id()) != 0 || field_info.field_name() ==
                    "__version__") {
                    continue;
                }
                // rollup索引不支持指标列为非INT
                if (field_info.mysql_type() != pb::INT64
                    ...
                    DB_WARNING("table:%s field:%s rollup value only support INT, not support %s",
                               table_name.c_str(), field_info.field_name().c_str(),
                               pb::PrimitiveType_Name(field_info.mysql_type()).c_str());
                    return -1;
                }
            }
        }
        ...
        if (fulltext_idxes.size() != 0) {
            ...
            for (int idx: fulltext_idxes) {
                ...
                if (0 != _need_check_wordweight_types.count(table_info.indexs(idx).segment_type()) && !
                    support_wordweight) {
    ...
    bool TableManager::check_region_when_update(int64_t table_id,
        ...
        for (const auto &start_pid_key: min_start_key) {
            ...
            auto &partition_startkey_regiondesc_map = _table_tmp_regions[table_id].startkey_regiondesc_map[
                partition_id];
    ...
    void TableManager::partition_update_startkey_regionid_map(int64_t table_id, std::string min_start_key,
                                                              ...
                                                              std::map<std::string, RegionDesc> &
                                                              startkey_regiondesc_map) {
        if (startkey_regiondesc_map.size() == 0) {
            //首个region加入
        ...
        BAIDU_SCOPED_LOCK(_table_mutex);
        {
            for (auto &table_info: _table_tmp_regions) {
                ...
                for (auto &iter: startkey_regiondesc_map) {
                    ...
                    if (startkey_regiondesc_map[partition_id].size() == 0 && key_newregion_map[partition_id].size() !=
                        0) {
    ...
    void TableManager::drop_index(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                  braft::Closure *done) {
        //检查参数有效性
                                         ...
                                         [&index_req](const pb::IndexInfo &info) {
                                             ...
                                             return boost::algorithm::iequals(info.index_name(), index_req.index_name())
                                                    ...
                                                    (info.index_type() == pb::I_UNIQ || info.index_type() == pb::I_KEY
                                                     ||
                                                     info.index_type() == pb::I_FULLTEXT || info.index_type() ==
                                                     pb::I_ROLLUP || info.index_type() == pb::I_VECTOR);
                                         });
        ...
        if (index_info.has_index_type() && index_info.index_type() == pb::I_FULLTEXT && index_info.has_segment_type()) {
            if (0 != _need_check_wordrank_types.count(index_info.segment_type())) {
                ...
                if (cmp_value < 0) {
                    ...
                    DB_WARNING("DDL_LOG[add_index] store cluster does not support wordrank, resource_tag: %s",
                               resource_tag.c_str());
                    response->set_errcode(pb::INPUT_PARAM_ERROR);
                ...
                if (cmp_value < 0) {
                    ...
                    DB_WARNING("DDL_LOG[add_index] store cluster does not support wordweight, resource_tag: %s",
                               resource_tag.c_str());
                    response->set_errcode(pb::INPUT_PARAM_ERROR);
        ...
        if (request.table_info().indexs(0).is_global()) {
            ...
            if (!ttl_field_name.empty()) {
                ...
                if (!any_fields_equal && !if_pk_contains_ttl_field(mem_schema_pb)) {
                    DB_WARNING("DDL_LOG[add_index] check fields info fail, request:%s",
                               request.ShortDebugString().c_str());
    ...
    int TableManager::init_global_index_region(const pb::MetaManagerRequest &request, const pb::SchemaInfo &schema_info,
                                               int64_t table_id, braft::Closure *done, pb::IndexInfo &index_info,
                                               const int64_t apply_index) {
        std::vector<std::string> rocksdb_keys;
        std::vector<std::string> rocksdb_values;
        ...
        if (is_create_table_support_engine(schema_info.engine())) {
            if (schema_info.partition_num() != request.partition_index_instance_size()) {
                DB_FATAL("has partition num: %ld, but only %d partition_index_instance", schema_info.partition_num(),
                         request.partition_index_instance_size());
                return -1;
            }
            for (auto i = 0; i < schema_info.partition_num(); ++i) {
                ...
                if (partition_index_instances.partition_id() != partition_id) {
                    DB_FATAL("partition_id not match, partition_id: %ld, partition_index_instances.partition_id(): %ld",
                             partition_id, partition_index_instances.partition_id());
                    return -1;
                }
    ...
    int TableManager::do_add_index(const pb::MetaManagerRequest &request,
        ...
        if (index_info.is_global() && init_global_index_region(request, mem_schema_pb, table_id, done, index_info,
                                                               apply_index) != 0) {
    ...
    void TableManager::update_index_status(const pb::MetaManagerRequest &request,
                                           ...
                                           braft::Closure *done) {
        update_table_internal(request, apply_index, done,
                              [this](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                     braft::Closure *done) {
                                  ...
                                  for (; index_iter != mem_schema_pb.mutable_indexs()->end(); index_iter++) {
                                      if (request_index_info.index_id() == index_iter->index_id()) {
                                          if (request_index_info.job_state() != pb::IS_DELETE_LOCAL &&
                                              ...
                                              if (request_index_info.op_type() == pb::OP_DROP_INDEX && index_iter->
                                                  hint_status() == pb::IHS_NORMAL) {
                                              ...
                                              if (request_index_info.job_state() == pb::IS_DELETE_LOCAL
                                                  ...
                                                  int64_t due_time =
                                                          butil::gettimeofday_us() + FLAGS_table_tombstone_gc_time_s *
                                                          1000 * 1000LL;
    ...
    void TableManager::link_binlog(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                   braft::Closure *done) {
        ...
        pb::FieldInfo link_field;
        {
            ...
            if (binlog_table_mem.is_partition) {
                if (mem_schema_pb.has_partition_info() && mem_schema_pb.partition_info().type() == pb::PT_RANGE) {
                    DB_WARNING("Range partition table not support partition binlog, request:%s",
                               request.DebugString().c_str());
                    IF_DONE_SET_RESPONSE(done, pb::INPUT_PARAM_ERROR,
                                         "Range partition table not support partition binlog");
                    return;
                }
                if (request.table_info().has_link_field()) {
                    for (const auto &field_info: mem_schema_pb.fields()) {
                        if (!field_info.deleted() && field_info.field_name() == request.table_info().link_field().
                            field_name()) {
    ...
    void TableManager::unlink_binlog(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                     braft::Closure *done) {
        DB_DEBUG("link binlog, request:%s", request.ShortDebugString().c_str());
        int64_t table_id;
        if (check_table_exist(request.table_info(), table_id) != 0) {
        ...
        }
        {
            ...
            if (!binlog_table_mem.is_binlog || binlog_table_mem.binlog_target_ids.count(table_id) == 0) {
                DB_WARNING("table is not binlog or not correct binlog table, request:%s",
                           request.DebugString().c_str());
                IF_DONE_SET_RESPONSE(done, pb::INPUT_PARAM_ERROR, "table is not binlog");
                return;
            }
    ...
    void TableManager::set_index_hint_status(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                             braft::Closure *done) {
        update_table_internal(request, apply_index, done,
                              [](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                 braft::Closure *done) {
                                      ...
                                      for (const auto &index_info: request.table_info().indexs()) {
                                          ...
                                          for (; index_iter != mem_schema_pb.mutable_indexs()->end(); index_iter++) {
                                              ...
                                              if (boost::algorithm::iequals(
                                                  ...
                                                  if (index_info.hint_status() == pb::IHS_NORMAL && index_iter->state()
                                                      != pb::IS_PUBLIC) {
                                                      continue;
                                                  }
    ...
    void TableManager::drop_virtual_index(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                          braft::Closure *done) {
        update_table_internal(request, apply_index, done,
                              [](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                 braft::Closure *done) {
                                  if (request.has_table_info()) {
                                      ...
                                      if (mem_schema_pb.mutable_indexs() != nullptr) {
                                          auto index_iter = mem_schema_pb.mutable_indexs()->begin();
                                          for (; index_iter != mem_schema_pb.mutable_indexs()->end(); index_iter++) {
                                              if (index_iter != mem_schema_pb.mutable_indexs()->end() && index_iter->
                                                  index_name() == index_info.index_name() &&
                                                  index_iter->index_type() != pb::I_PRIMARY) {
                                                  mem_schema_pb.mutable_indexs()->erase(index_iter);
                                                  DB_NOTICE("set index hint status schema %s",
    ...
    void TableManager::add_learner(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                   braft::Closure *done) {
        int ret = 0;
    ...
    void TableManager::drop_learner(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                    braft::Closure *done) {
        int ret = 0;
        DB_DEBUG("request:%s", request.ShortDebugString().c_str());
        int64_t table_id;
        ...
        for (auto learner_iter = learner_resource_ptr->begin(); learner_iter != learner_resource_ptr->end();
             learner_iter++) {
    ...
    void TableManager::remove_global_index_data(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                                braft::Closure *done) {
        int ret = 0;
    ...
    void TableManager::load_virtual_indextosqls_to_memory(const pb::KsHeartBeatRequest *request) {
        BAIDU_SCOPED_LOCK(_load_virtual_to_memory_mutex);
        ...
        for (auto &it1: affect_info) {
            ...
            if (_just_add_virtual_index_info.count(virtual_index_id) > 0) {
                ...
                std::string sql;
                uint64_t out[2];
                ksearch::parse_sample_sql(influenced_sql, database_name, table_name, sql);
        ...
        if (has_cold) {
            ...
            for (auto &cold_range_partition_info: cold_partition_info) {
                if (cold_range_partition_info.type() == schema.partition_info().primary_range_partition_type()) {
                    // 主分区控制转冷
                    pb::RangePartitionInfo *p_cold_range_partition_info = p_cold_partition_info->
                            add_range_partition_infos();
    ...
    int TableManager::get_partition_split_key(
        ...
        for (const auto &index_info: table_info.indexs()) {
                ...
                if (!p_region_info->start_key().empty()) {
                    ...
                    switch (partition_col_type) {
                        case pb::DATE: {
                            ...
                            partition_utils::create_partition_expr(partition_col_type, new_value_str,
                                                                   new_partition_expr);
                        }
                        break;
                        case pb::DATETIME: {
                    ...
                    if (!(partition_utils::compare(new_partition_expr, range_partition_info.range().left_value()) >= 0
                          &&
                          partition_utils::compare(new_partition_expr, range_partition_info.range().right_value()) <
                          0)) {
            ...
            if (p_split_key->split_keys_size() > 0) {
                // 对split_key进行排序
                std::sort(
                    p_split_key->mutable_split_keys()->pointer_begin(),
                    p_split_key->mutable_split_keys()->pointer_end(),
                ...
                // 对split_key进行去重
                int remove_num = std::distance(
                    std::unique(p_split_key->mutable_split_keys()->begin(), p_split_key->mutable_split_keys()->end()),
    ...
    void TableManager::check_partition_exist_for_peer(const pb::StoreHeartBeatRequest *request,
        ...
        for (auto &peer_info: request->peer_infos()) {
            ...
            if (partition_regions.find(partition_id) == partition_regions.end()) {
                ...
                if (butil::gettimeofday_us() - drop_partition_ts_map.at(partition_id) <
                    FLAGS_remove_dropped_partition_region_s) {
                    continue;
                }
    ...
    void TableManager::specify_split_keys(const pb::MetaManagerRequest &request, const int64_t apply_index,
                                          braft::Closure *done) {
        update_table_internal(request, apply_index, done,
                              [](const pb::MetaManagerRequest &request, pb::SchemaInfo &mem_schema_pb,
                                 braft::Closure *done) {
    ...
    void TableManager::drop_partition_internal(pb::SchemaInfo &mem_schema_pb,
        ...
        for (const auto &range_partition_info: mem_schema_pb.partition_info().range_partition_infos()) {
            if (partition_utils::check_partition_overlapped(
                ...
                // 报警
                DB_FATAL("Partition overlapped range_partition_info: %s",
                         range_partition_info.ShortDebugString().c_str());
