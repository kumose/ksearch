...
namespace ksearch {
    DEFINE_int32(ksearch_max_concurrent, 5, "ddl work ksearch concurrent");
    DEFINE_int32(single_table_ddl_max_concurrent, 50, "ddl work ksearch concurrent");
    ...
    void DBManager::process_common_task_hearbeat(const std::string &address, const pb::KsHeartBeatRequest *request,
        ...
        for (const auto &region_ddl_info: request->region_ddl_works()) {
            ...
            _common_task_map.update(region_ddl_info.address(),
                                    [this, task_id, &region_ddl_info](CommonTaskMap &db_task_map) {
                                        if (region_ddl_info.status() == pb::DdlWorkDoing) {
                                            // 正在运行，跟新时间戳。
                                            ...
                                            if (doing_iter != db_task_map.doing_task_map.end()) {
                                                DB_NOTICE("task_%s work done %s", task_id.c_str(),
                                                          region_ddl_info.ShortDebugString().c_str());
                                                DDLManager::get_instance()->update_region_ddlwork(region_ddl_info);
    ...
    void DBManager::process_broadcast_task_hearbeat(const std::string &address, const pb::KsHeartBeatRequest *request,
                                                    pb::KsHeartBeatResponse *response) {
        {
    ...
    void DBManager::process_ks_heartbeat(const pb::KsHeartBeatRequest *request,
                                         pb::KsHeartBeatResponse *response, brpc::Controller *cntl) {
        // 更新ksearch 信息
        ...
        update_ksearch_info(address, room);
        ...
        DB_NOTICE("process ddl ks heartbeat update biakaldb info %ld, common task time %ld, broadcast task time %ld",
    ...
    bool DBManager::round_robin_select(std::string *selected_address, bool is_column_update_ddl) {
        ...
        for (size_t index = 0; index < instance_count; ++index) {
            ...
            auto find_task_map = _common_task_map.init_if_not_exist_else_update(
                iter->first, false, [&current_task_number](CommonTaskMap &db_task_map) {
                    current_task_number = db_task_map.doing_task_map.size() + db_task_map.to_do_task_map.size();
                });
            int32_t max_concurrent = is_column_update_ddl
                                         ? FLAGS_ksearch_max_concurrent * 5
                                         : FLAGS_ksearch_max_concurrent;
    ...
    int DBManager::execute_task(MemRegionDdlWork &work) {
        ...
        int32_t max_concurrent = is_column_update
                                     ? FLAGS_single_table_ddl_max_concurrent * 10
                                     : FLAGS_single_table_ddl_max_concurrent;
        ...
        if (select_instance(&address, is_column_update)) {
            auto task_id = std::to_string(region_ddl_info.table_id()) + "_" + std::to_string(
                               region_ddl_info.region_id());
            ...
            if (region_info != nullptr && !is_column_update) {
                ...
                if (store_ddlwork_cnt_map[region_info->leader()] >= single_store_max_ddlwork_num) {
                    ...
                    DB_NOTICE("store_check address_%s start task: %s ddl_work_num: %d",
                              region_info->leader().c_str(), task_id.c_str(),
                              store_ddlwork_cnt_map[region_info->leader()]);
                }
            }
    ...
    std::vector<std::string> DBManager::get_faulty_ksearch() {
    ...
    void DBManager::init() {
        _bth.run([this]() {
            ...
            while (!_shutdown) {
                        ...
                        work_map.traverse_with_key_value(
                            [&cast_task_ptr, &timeout_instance_vec](const std::string &instance, MemDdlWork &work) {
                                if (butil::gettimeofday_us() - work.update_timestamp >
                                    FLAGS_ks_heartbeat_interval_us * 30) {
                ...
                auto faulty_dbs = get_faulty_ksearch();
    ...
    int DBManager::restore_task(const pb::RegionDdlWork &region_ddl_info) {
        ...
        _common_task_map.init_if_not_exist_else_update(region_ddl_info.address(), false,
                                                       [this, region_ddl_info, &work, &task_id](
                                                   CommonTaskMap &db_task_map) {
                                                           db_task_map.doing_task_map[task_id] = work;
                                                           {
                                                               SmartRegionInfo region_info =
                                                                       RegionManager::get_instance()->get_region_info(
                                                                           region_ddl_info.region_id());
                                                               if (region_info != nullptr) {
                                                                   ...
                                                                   task_store_map[task_id] = region_info->leader();
                                                                   DB_NOTICE(
                                                                       "store_check address_%s is doing task: %s ",
                                                                       region_info->leader().c_str(), task_id.c_str());
                                                               }
                                                           }
                                                       }, map);
        ...
        while (!_shutdown) {
            ...
            std::unordered_map<int64_t, MemDdlInfo> temp_ddl_mem;
            {
                ...
                for (auto iter = _table_ddl_mem.begin(); iter != _table_ddl_mem.end(); iter++) {
                    if (iter->second.work_info.errcode() == pb::SUCCESS || iter->second.work_info.errcode() ==
                        pb::EXEC_FAIL) {
                        ...
                        if (iter->second.work_info.errcode() == pb::EXEC_FAIL && iter->second.work_info.op_type() ==
                            pb::OP_ADD_INDEX) {
    ...
    int DDLManager::add_index_ddlwork(pb::DdlWorkInfo &ddl_work) {
        ...
        DB_NOTICE("table_id:%ld index_id:%ld region size %zu, is_rollup: %d", table_id, ddl_work.index_id(),
                  region_size, is_rollup);
        pb::IndexState current_state;
        if (TableManager::get_instance()->get_index_state(ddl_work.table_id(), ddl_work.index_id(), current_state) !=
            0) {
            DB_WARNING("ddl index not ready. table_id[%ld] index_id[%ld]",
                ...
                if (is_rollup) {
                    ...
                    MemRegionDdlWork &region_work) -> bool {
                            ...
                            if (region_work.region_info.status() == pb::DdlWorkIdle) {
                                ...
                                TableManager::get_instance()->get_region_ids(
                                    table_id, {region_work.region_info.partition()}, region_ids);
                                if (region_ids.count(region_work.region_info.region_id()) == 0) {
                                    region_work.region_info.set_status(pb::DdlWorkDone);
                                    DB_NOTICE(
                                        "table_id: %ld, region_id: %ld, partition %ld not exist, region ddl work done",
                                        table_id, region_work.region_info.region_id(),
                                        region_work.region_info.partition());
                                    return true;
                                }
                                ...
                                if (FLAGS_cold_data_rollup_done) {
                                    ...
                                    if (table_schema_pb.has_partition_info()) {
                                        ...
                                        for (auto it = partition_info_pb.range_partition_infos().rbegin();
                                             it != partition_info_pb.range_partition_infos().rend(); ++it) {
                                            if (it->partition_id() == region_work.region_info.partition()) {
                                                if (it->is_cold()) {
                                                    region_work.region_info.set_status(pb::DdlWorkDone);
                                                    DB_NOTICE(
                                                        "table_id: %ld, region_id: %ld, partition %ld is cold, region ddl work done",
                                                        table_id, region_work.region_info.region_id(),
                                                        region_work.region_info.partition());
                                                    return true;
                                                }
    ...
    bool DDLManager::execute_or_retry_task(bool &done, bool &rollback, int64_t table_id, size_t region_size,
                                           size_t &current_task_number, size_t max_task_number,
        ...
        if (region_work.region_info.status() == pb::DdlWorkIdle) {
            ...
            if (ret == 0) {
                ...
                DB_NOTICE("table_%ld not enough ksearch to execute.", table_id);
                return false;
            }
        }
    ...
    int DDLManager::is_region_work_need_executor(MemRegionDdlWork &region_work, int64_t table_id, bool &done) {
        ...
        TableManager::get_instance()->get_partition_range_info(table_id, region_work.region_info.partition(),
                                                               partition_range);
        if (partition_range.left_value().nodes_size() == 0 || partition_range.right_value().nodes_size() == 0) {
            DB_FATAL("table_id: %ld, region_id: %ld, partition %ld has no range info", table_id,
                     region_work.region_info.region_id(), region_work.region_info.partition());
            return -2;
        }
