...
namespace ksearch {
    ...
    bool SelectPlanner::is_pk_consistency(const std::vector<FieldInfo> &pk_fields_in_factory,
                                          const std::vector<int32_t> &select_pk_fields) {
    ...
    void SelectPlanner::add_snapshot_blacklist_to_where_filters() {
        ...
        for (auto table_name: tables) {
            ...
            pb::SlotDescriptor slot = get_scan_ref_slot(table_name, table_ptr->id, snapshot_field.id,
                                                        snapshot_field.type);
        ...
        if (multi_distinct_cnt > 1) {
            if (multi_col_single_child) {
                _ctx->stat_info.error_msg <<
                        "The query contains multi count/sum/group_concat distinct, each can't have multi columns.";
                return -1;
            }
            if (name_set.size() > 1) {
