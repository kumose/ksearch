...
#include <ksearch/logical_plan/ddl_work_planner.h>
...
namespace ksearch {
    ...
    int DDLWorkPlanner::exec_rollup_node(RuntimeState &state, NetworkSocket *client_conn, pb::OpType op_type) {
        ...
        std::map<int64_t, pb::RegionInfo> region_info = static_cast<RocksdbScanNode *>(rollup_node.get())->
                region_infos();
    ...
    int DDLWorkPlanner::execute() {
        ...
        ON_SCOPE_EXIT(([this, &state, &client_conn, &rollup_finish_succ]() {
            ...
            while (exec_cnt < MAX_RETRY_TIMES) {
                if (_is_rollup_index && !rollup_finish_succ && exec_rollup_node(
                        state, client_conn, pb::OP_ROLLUP_REGION_FAILED) != 0) {
        ...
        if (_is_rollup_index &&exec_rollup_node(state, client_conn, pb::OP_ROLLUP_REGION_INIT) 
        !=
        0
        )
        {
        ...
        while (true) {
            ...
            if (!first_flag) {
                ...
                if (!_is_global_index && (state.ddl_scan_size == 0 || _start_key == state.ddl_max_pk_key)) {
                    ...
                    break;
                } else if (_is_global_index &&state
                .
                ddl_scan_size < _limit
                )
                {
                ...
                if (!_is_rollup_index) {
                    ...
                    if (!_is_column_ddl) {
                        ...
                        if (index_info_ptr == nullptr ||
                            (index_info_ptr->state != pb::IS_WRITE_LOCAL && index_info_ptr->state !=
                             pb::IS_WRITE_ONLY)) {
                            //说明任务已经完成，或者任务失败，该索引正在被删除。
                            DB_FATAL(
                                "index info ptr is nullptr or index state is not pb::IS_WRITE_LOCAL/pb::IS_WRITE_ONLY");
                            _work.set_status(pb::DdlWorkFail);
                            return -1;
                        }
                    }
        ...
        if (_is_rollup_index &&exec_rollup_node(state, client_conn, pb::OP_ROLLUP_REGION_FINISH) 
        !=
        0
        )
        {
        ...
        DB_NOTICE("task_%s summary: first %s last %s", _task_id.c_str(), first_record_str.c_str(),
