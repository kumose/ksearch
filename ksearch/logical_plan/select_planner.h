...
namespace ksearch {
    ...
    public:
        SelectPlanner(QueryContext *ctx) : LogicalPlanner(ctx),
                                           _select(nullptr) {
        }

        SelectPlanner(QueryContext *ctx, const SmartUniqueIdCtx &uniq_ctx,
                      const SmartPlanTableCtx &plan_state) : LogicalPlanner(ctx, uniq_ctx, plan_state),
                                                             _select(nullptr) {
        }
    ...
    private:
        ...
        bool is_pk_consistency(const std::vector<FieldInfo> &pk_fields_in_factory,
