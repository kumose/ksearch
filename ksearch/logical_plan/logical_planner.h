...
#include <ksearch/sqlparser/dml.h>
...
namespace ksearch {
    ...
    public:
        ...
        LogicalPlanner(QueryContext *ctx, const SmartUniqueIdCtx &uniq_ctx,
                       const SmartPlanTableCtx &plan_state) : _ctx(ctx), _unique_id_ctx(uniq_ctx),
                                                              _plan_table_ctx(plan_state) {
            _factory = SchemaFactory::get_instance();
            if (_ctx != nullptr) {
