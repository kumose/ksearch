...
namespace ksearch {
    ...
    int UpdatePlanner::create_update_node(pb::PlanNode *update_node) {
        ...
        if (_current_tables.size() != 1 || _plan_table_ctx->table_tuple_mapping.count(try_to_lower(_current_tables[0]))
            == 0) {
    ...
    int UpdatePlanner::parse_kv_list() {
        ...
        for (int idx = 0; idx < set_list.size(); ++idx) {
            ...
            if (_factory->has_rollup_index(table_id) && need_rollup_field_ids.count(field_info->id) == 0) {
                DB_WARNING("table: %ld has rollup index, field: %s is not aggregate column", table_id,
                           full_name.c_str());
                return -1;
            }
            ...
            // 更新分区键,走全局索引流程
            if (table_info.partition_ptr != nullptr && table_info.partition_ptr->partition_field_id() == field_info->
                id) {
                _ctx->execute_global_flow = true;
            }
            // 更新主键，走全局索引流程
            ...
            if (_ctx->execute_global_flow) {
                const int64_t meta_id = ::ksearch::get_meta_id(table_info.id);
                if (meta_id != 0) {
            ...
            if (field_info->on_update_value == "(current_timestamp())"
                ...
                if (value_expr.nodes(0).node_type() == pb::NULL_LITERAL) {
                    ...
                    node->mutable_derive_node()->set_string_val(
                        ExprValue::Now(field_info->float_precision_len).get_string());
                }
