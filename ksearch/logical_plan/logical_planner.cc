...
#include <ksearch/logical_plan/logical_planner.h>
#include <memory>
#include <algorithm>
#include <iterator>
#include <boost/algorithm/string.hpp>
#include <ksearch/common/common.h>
#include <ksearch/logical_plan/select_planner.h>
#include <ksearch/logical_plan/insert_planner.h>
#include <ksearch/logical_plan/delete_planner.h>
#include <ksearch/logical_plan/update_planner.h>
#include <ksearch/logical_plan/union_planner.h>
#include <ksearch/logical_plan/ddl_planner.h>
#include <ksearch/logical_plan/load_planner.h>
#include <ksearch/logical_plan/setkv_planner.h>
#include <ksearch/logical_plan/transaction_planner.h>
#include <ksearch/logical_plan/kill_planner.h>
#include <ksearch/logical_plan/prepare_planner.h>
#include <ksearch/expr/predicate.h>
#include <ksearch/sqlparser/parser.h>
#include <ksearch/protocol/mysql_err_code.h>
#include <ksearch/physical_plan/physical_planner.h>
...
namespace ksearch {
    ...
    int LogicalPlanner::construct_in_predicate_node(const parser::FuncExpr *func_item, pb::Expr &expr,
                                                    pb::ExprNode **node) {
                    ...
                    for (int i = 0; i < ctx->plan.nodes_size(); i++) {
                        ...
                        if (table_index_map.count(table_id) > 0) {
                            for (auto &index_name: table_index_map[table_id]) {
                                ...
                                if (indexInfoPtr == nullptr || indexInfoPtr->state != pb::IS_PUBLIC || indexInfoPtr->
                                    index_hint_status != pb::IHS_NORMAL) {
                                    continue;
                                }
    ...
    int LogicalPlanner::add_table(const std::string &database, const std::string &table,
        ...
        if (_plan_table_ctx->table_info.count(try_to_lower(alias_full_name)) == 0) {
            ...
            if (tbl_ptr->engine == pb::DBLINK) {
                is_dblink = true;
                if (tbl_ptr->dblink_info.type() == pb::LT_KSEARCH) {
                    ...
                    external_table_name = ::ksearch::get_add_meta_name(meta_id, external_table_name);
    ...
    */
    ...
    int LogicalPlanner::parse_using_cols(const parser::Vector<parser::ColumnName *> &using_col,
    ...
    //join节点的叶子节点
    int LogicalPlanner::create_join_node_from_table_name(const parser::TableName *table_name,
                                                         JoinMemTmp **join_root_ptr) {
        std::string db;
        std::string table;
    ...
    int LogicalPlanner::create_join_node_from_table_source(const parser::TableSource *table_source,
                                                           JoinMemTmp **join_root_ptr) {
        std::string db;
        std::string table;
        std::string alias;
    ...
    int LogicalPlanner::create_join_node_from_terminator(const std::string db,
        ...
        *join_root_ptr = join_node_mem.release();
        ...
        for (auto &index_name: force_index_names) {
            ...
            auto indexInfoPtr = _factory->get_index_info_ptr(index_id);
            if (indexInfoPtr == nullptr || indexInfoPtr->state != pb::IS_PUBLIC || indexInfoPtr->index_hint_status !=
                pb::IHS_NORMAL) {
                continue;
            }
    ...
    int LogicalPlanner::parse_db_name_from_table_source(const parser::TableSource *table_source,
        ...
        if (table_source->table_name != nullptr) {
            ...
            if (table_with_clause_iter != _ctx->table_with_clause_mapping.end()) {
                // 查询的表在with中
                std::string with_select_stmt = "SELECT * FROM (" + table_with_clause_iter->second + ") " + table_source
                ...
                parser::TableSource *with_table_source = (parser::TableSource *) ((parser::SelectStmt *) parser.result[
                    0])->table_refs;
        ...
        if (table_source->table_name != nullptr && !table_source->table_name->db.empty()) {
            table_full_name = _ctx->user_info->namespace_ + "." + table_source->table_name->db.value + "." +
                              table_source->table_name->table.value;
        } else if (table_source->table_name != nullptr && !_ctx->cur_db.empty()) {
            table_full_name = _ctx->user_info->namespace_ + "." + _ctx->cur_db + "." + table_source->table_name->table.
                              value;
        }
        if (table_full_name != ""
            ...
            if (table_ptr != nullptr && table_ptr->is_view) {
                if (_ctx->is_with == true) {
                    _ctx->stat_info.error_code = ER_VIEW_INVALID;
                    _ctx->stat_info.error_msg << "cant create with-clause on a view, " << table_full_name <<
                            " is a view";
                    return -1;
                }

                std::string view_select_stmt = "SELECT * FROM (" + table_ptr->view_select_stmt + ") " + table_ptr->
                                               short_name;
                parser::SqlParser parser;
                ...
                parser::TableSource *view_table_source = (parser::TableSource *) ((parser::SelectStmt *) parser.result[
                    0])->table_refs;
        ...
        if (view_select_stmt->node_type == parser::NT_SELECT) {
            ...
            for (int i = 0; i < view_list.size() - 1; ++i) {
                if (view_list[i].fields_size() != view_list[i + 1].fields_size()) {
                    ...
                    DB_WARNING("have a different number of columns %u and %u", view_list[i].fields_size(),
                               view_list[i + 1].fields_size());
                    return -1;
                }
    ...
    int LogicalPlanner::parse_view_select_field(parser::SelectField *select_field,
        ...
        if (column_names.size() > 0 && column_names.size() <= column_name_idx) {
            DB_WARNING("column names size %d not equal to column_name_idx size %d", column_names.size(),
                       column_name_idx);
            return -1;
        }

        const std::string &alias_name = column_names.size() > 0
                                            ? column_names[column_name_idx++]->name.value
                                            : select_name;
    ...
    std::vector<pb::SlotDescriptor> &LogicalPlanner::get_agg_func_slot(
        ...
        static std::unordered_set<std::string> need_intermediate_slot_agg = {
            "avg", "rb_or_cardinality_agg", "rb_and_cardinality_agg", "rb_xor_cardinality_agg", "multi_count_distinct",
            "multi_sum_distinct",
    ...
    int LogicalPlanner::create_agg_expr(const parser::FuncExpr *expr_item, pb::Expr &expr,
        ...
        if (_need_multi_distinct &&expr_item
        ->
        distinct
        )
        {
    ...
    int LogicalPlanner::create_window_expr(const parser::WindowFuncExpr *item, pb::Expr &expr,
                                           const CreateExprOptions &options) {
            ...
            if (item->fn_name.to_lower() == "default" && item->children.size() == 1) {
                ...
                if (field_info->default_expr_value.is_null()) {
                    if (_ctx->stat_info.error_code == ER_ERROR_FIRST) {
                        ...
                        _ctx->stat_info.error_msg << "Field '" << col_expr->name.to_lower() <<
            ...
            if (options.is_select_field) {
                if (row_cnt == 0) {
                    ...
                    if (run_memrow) {
                        ...
                        pb::ExprNode *node = expr.add_nodes();
                        construct_literal_expr(
                            VectorizeHelpper::get_vectorized_value(vec_result_table->column(0).get(), 0), node);
                    }
                }
                // in (SubSelect)
            } else {
                        ...
                        for (int64_t row_idx = 0; row_idx < vec_result_table->num_rows(); ++row_idx) {
                            ...
                            if (column_size > 1) {
                                ...
                                for (int column_idx = 0; column_idx < column_size; ++column_idx) {
                                    pb::ExprNode *node = expr.add_nodes();
                                    construct_literal_expr(
                                        VectorizeHelpper::get_vectorized_value(
                                            vec_result_table->column(column_idx).get(), row_idx), node);
                                }
                            } else {
                                pb::ExprNode *node = expr.add_nodes();
                                construct_literal_expr(
                                    VectorizeHelpper::get_vectorized_value(vec_result_table->column(0).get(), row_idx),
                                    node);
                            }
                        }
                    }
                    ...
                    if (is_eq_all) {
                        if (run_memrow) {
                            ...
                            for (uint32_t i = 1; i < vec_result_table->num_rows(); i++) {
                                // = all返回多个不同值
                                if (first_val.compare(
                                        VectorizeHelpper::get_vectorized_value(vec_result_table->column(0).get(), i)) !=
                                    0) {
                                    DB_WARNING("always_false");
                                    always_false = true;
                                }
    ...
    int LogicalPlanner::handle_common_subquery(const parser::ExprNode *expr_item,
                                               pb::Expr &expr,
                                               const CreateExprOptions &options) {
        int ret = create_common_subquery_expr((parser::SubqueryExpr *) expr_item, expr, options,
                                              _is_correlate_subquery_expr);
        if (ret < 0) {
            return -1;
        }
        if (_is_correlate_subquery_expr &&options
        .
        is_select_field
        )
        {
    ...
    int LogicalPlanner::can_use_dblink(SmartTable table) {
        ...
        if (table->dblink_info.type() == pb::LT_KSEARCH) {
            // dblink Ksearch表只支持INSERT/DELETE/UPDATE/SELECT/UNION
            if (_ctx->stmt_type != parser::NT_INSERT
            ...
            _ctx->stat_info.error_code = ER_BAD_TABLE_ERROR;
            _ctx->stat_info.error_msg << "dblink table only supprt KSEARCH/MYSQL";
            return -1;
        }
        // dblink表不支持事务
